...
**B4-C2 state (2026-09-24): IMPLEMENTED LOCALLY / AWAITING REVIEW / NO MIGRATION / NO DEPENDENCY / NO JOURNAL CHANGE / NO GRAPH CHANGE.** A pure Web mapping now turns only reliable C1 source identities into existing destinations: Order SALE, Expense v1/v2, Provider Statement, Provider Payout and Payroll accrual/reversal. Expense, Provider Statement Inbox evidence and payout list APIs gain additive same-owner stable-ID filters solely to make deep-link positioning independent of bounded list windows; Payroll uses its existing `GET /accounting/payroll/runs/:runStableId`. Order adjustment/reversal, employee-payment, CRA remittance, Uber pre-cutover reversal and unknown source types remain identity-only rather than guessing cross-owner keys. The drawer itself still fetches only `/accounting/report/statement-journals`; no foreign-owner API lookup, new financial DTO/read model, Prisma/schema/migration, package, posting change or context edge is added.
