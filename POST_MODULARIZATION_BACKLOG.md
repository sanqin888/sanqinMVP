...
Current state: **B2-P0A PRODUCTION VERIFIED / B2-P0B PRODUCTION VERIFIED + COMPLETE / B2-A MERGED + CI GREEN / B2-B MERGED + CI GREEN / B2-C LOCAL SOURCE IMPLEMENTED / B2-D NOT STARTED**. The readiness audit found a prerequisite reliability gap before Sales Analytics projection work: Orders continued to create durable immutable `order.financial_sale.v1` facts, but the canonical SALE posting service had no runtime consumer after the controlled 2026-09-13 replay.
...
B2-C on local branch `accounting/b2-c-sales-ui-cutover` cuts only the Sales page to that canonical endpoint. It removes the Sales page's P&L + legacy Order-slice reads, adds matching Web wire DTOs, surfaces Gross Sales / Discounts / Net Sales Revenue / Output Tax / Channel Contribution, daily trend, channel and canonical-primary-payment attribution, actual tender mix, provider coverage, attribution-quality warnings and explicit source/adjustment buckets, and performs previous equal-period comparison through the same canonical endpoint. Comparison is based on the backend's effective clamped range and is omitted when the prior range cannot be read exactly. Dashboard intentionally still consumes `/accounting/report/slice`, so final legacy analytics cleanup remains deferred to B2-D. No schema/migration, package, backend authority or architecture direction changes are introduced.
