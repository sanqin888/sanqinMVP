...
Complexity: **M**  
State: **PAYOUT-A MERGED / CI GREEN; PAYOUT-B MERGED / CI GREEN / MIGRATION APPLIED; PAYOUT-C PRODUCTION VERIFIED; PAYOUT-D DEPLOYED / BACKEND DATA-PATH VERIFIED / UI SPOT-CHECK PENDING; PAYOUT-E-A MERGED / CI GREEN; INBOX-ONLY EVIDENCE FOLLOW-UP LOCAL SOURCE READY FOR REVIEW / NO GRAPH CHANGE**
...
PAYOUT-E-A is merged through PR #2490 / squash `1d90e6fd`, CI #6223 green, and introduces only **bank CSV evidence + payout match preview**. It reuses the Accounting Inbox artifact lifecycle and accepts the verified native CIBC headerless four-column export plus strong headered directional bank signatures. June production evidence establishes `FIRST DATA CANADA(K)` as the Clover deposit descriptor; Uber/Fantuan/First Data strings remain provider hints rather than posting authority. Deposits are compared only with payout rows for the same store/bank/currency whose active canonical payout Journal revalidates, with exact amount and a bounded ±3-day date window. Provider-hint contradictions cannot be labelled exact. The current Inbox-only follow-up moves bank preview into the evidence-review owner: a CSV Inbox row classified as `OTHER_DOCUMENT` or still `UNKNOWN` opens the preview for that exact artifact, supports session-only Include/exclude decisions for cross-period receipts, and then returns to the same Inbox item for classification/review confirmation. The `OTHER_DOCUMENT` enum remains unchanged while its UI label becomes “银行流水 / 其他资料”. Settlements no longer hosts bank CSV preview/upload and is reserved for formal payout posting. Durable bank-row reconciliation decisions remain E-B. The GET preview never creates or modifies payout/Journal rows. No schema/migration, provider API, package or context edge is added. Detailed readiness: `docs/architecture/accounting-provider-payout-readiness.md`.
