...
Complexity: **M**  
State: **PAYOUT-A MERGED / CI GREEN; PAYOUT-B MERGED / CI GREEN / MIGRATION APPLIED; PAYOUT-C PRODUCTION VERIFIED; PAYOUT-D DEPLOYED / BACKEND DATA-PATH VERIFIED / UI SPOT-CHECK PENDING; PAYOUT-E-A MERGED / CI GREEN; INBOX-ONLY EVIDENCE FOLLOW-UP MERGED / CI GREEN; SETTLEMENT ROW-DECISION FOLLOW-UP LOCAL SOURCE READY FOR REVIEW / NO GRAPH CHANGE**
...
PAYOUT-E-A is merged through PR #2490 / squash `1d90e6fd`, CI #6223 green, and introduces only **bank CSV evidence + payout match preview**. It reuses the Accounting Inbox artifact lifecycle and accepts the verified native CIBC headerless four-column export plus strong headered directional bank signatures. June production evidence establishes `FIRST DATA CANADA(K)` as the Clover deposit descriptor; Uber/Fantuan/First Data strings remain provider hints rather than posting authority. Deposits are compared only with payout rows for the same store/bank/currency whose active canonical payout Journal revalidates, with exact amount and a bounded ±3-day date window. Provider-hint contradictions cannot be labelled exact. The Inbox-only workflow contraction merged through PR #2492 / squash `c4324edd`, CI #6229 green: a CSV Inbox row classified as `OTHER_DOCUMENT` or still `UNKNOWN` opens preview for that exact artifact, while the `OTHER_DOCUMENT` enum remains unchanged and its UI label is “银行流水 / 其他资料”. The current local follow-up separates evidence review from settlement decisions: Inbox preview no longer exposes Include/Exclude; Settlements consumes only reviewed `CONFIRMED + OTHER_DOCUMENT + CSV` artifacts, owns session-only cross-period Include/Exclude, and allows only an included `UNMATCHED` row with a provider hint to populate the formal payout form. Exact/possible/ambiguous rows cannot create a new payout through this handoff. Durable bank-row reconciliation decisions remain E-B. The GET preview never creates or modifies payout/Journal rows. No schema/migration, provider API, package or context edge is added. Detailed readiness: `docs/architecture/accounting-provider-payout-readiness.md`.
