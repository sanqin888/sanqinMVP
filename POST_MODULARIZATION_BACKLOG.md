...
**B4-D1 state (2026-09-24): IMPLEMENTED LOCALLY / AWAITING REVIEW / NO MIGRATION / NO DEPENDENCY / NO JOURNAL OR P&L ARITHMETIC CHANGE / NO GRAPH CHANGE.** Readiness found the cash-movement path already Journal-only, so the next real gap is the opaque Management P&L adjustment bucket. D1 preserves `adjustmentCents` and `netProfitCents` exactly, adds a reconciled Accounting-owned `adjustmentBreakdown` by source/sourceFactType, and exposes Journal count plus revenue/expense/net-profit effects in Web and Management CSV/PDF. Production evidence is dominated by two current Order reversal Journals and 119 Uber pre-cutover reversal Journals; Provider Statement adjustments already decompose into normal revenue/expense facts. No Orders/Payroll/provider lookup, route/schema/package/posting change or graph change is introduced. Old account-balance HTTP contraction and Sales/Reports stale-copy cleanup remain later B4 work.
