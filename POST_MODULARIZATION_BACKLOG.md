...
2026-09-25 A3-B local implementation adds deterministic rendering-contract coverage without redesigning the printer agent. Customer/kitchen renderers accept test-only fixed-time options while runtime defaults remain current time; customer tests can suppress the optional logo raster path without changing production default behavior; the label JSON handed to the Windows PowerShell driver is extracted as a pure contract function. Importing `printer-server.js` no longer starts the health server or cloud socket, while direct `node printer-server.js` startup remains unchanged. Focused Node built-in tests cover deterministic receipt/kitchen buffers and representative bilingual/discount/surcharge/note/component content plus 70x30 label payload defaults. Production `assets/logo.png` has been supplied for A3-B review but cannot be written as binary through the current MCP workspace tool, so exact logo raster golden coverage remains a binary-asset handoff rather than being silently substituted.
