...
B4 polish tail: the valid `2026-06-01..2026-06-30` Sales request succeeds; its automatically calculated equal-period comparison `2026-05-02..2026-05-31` is intentionally rejected because it predates `accountingStartDate=2026-06-01`, then the UI omits comparison. **B4-D2 now implements the local pre-request coverage guard** so known-out-of-coverage comparisons no longer need to generate expected HTTP 400 / ERROR-log noise; B2 remains closed and D2 still awaits its own review/delivery gate.
...
**B4-D2 state (2026-09-24): IMPLEMENTED LOCALLY / AWAITING REVIEW / ADDITIVE SALES V1 METADATA / NO MIGRATION / NO DEPENDENCY / NO JOURNAL OR SALES MONEY CHANGE / NO GRAPH CHANGE.** Sales exposes its already-resolved `accountingStartDate`; the Web requires the entire prior equal-length range to start on/after that date before issuing the comparison request. Known-out-of-coverage comparisons are omitted locally rather than causing expected 400/error-log noise or relying on a shorter clamped range. Comparison-unavailable copy now distinguishes coverage from actual read failure, and the trend copy no longer promises an equal-period comparison when none can exist. Canonical `/accounting/report/sales` remains the sole money source. The zero-consumer legacy account-balance HTTP route is deliberately left for separate D3 contract contraction.
