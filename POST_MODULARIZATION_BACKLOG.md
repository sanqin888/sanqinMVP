...
**B1-C1 state:** **LOCAL SOURCE COMPLETE / USER REVIEW PENDING / NO MIGRATION / NO GRAPH CHANGE** on `accounting/b1c1-expense-report-cutover`. Authoritative P&L/export, account-balance and cashflow now use canonical Journals including `EXPENSE_DOCUMENT`; legacy Expense Transaction report arithmetic and writer/delete paths are contracted; canonical Expense posting no longer requires the legacy split shadow copy; Upload Library deletion safety now keys off Expense-owned split counts; split audit ownership is explicit; and the architecture gate requires zero production `AccountingTransaction` mutation callers. The model/table and historical compatibility row remain for later separately reviewed destructive contraction. The legacy-comparison HTTP routes `report/expense-journal-parity` and `journal/canonical-expenses/shadow-preview` are retained in this slice; once new post-cutover Expenses stop receiving a legacy copy, those diagnostics are expected to fail closed and are not permanent canonical-health monitors.
