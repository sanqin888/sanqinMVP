...
Complexity: **M**  
State: **PAYOUT-A MERGED / CI GREEN; PAYOUT-B MERGED / CI GREEN / MIGRATION APPLIED; PAYOUT-C PRODUCTION VERIFIED; PAYOUT-D DEPLOYED / BACKEND DATA-PATH VERIFIED / UI SPOT-CHECK PENDING; PAYOUT-E-A LOCAL SOURCE READY FOR REVIEW / NO GRAPH CHANGE**
...
PAYOUT-E-A introduces only **bank CSV evidence + payout match preview**. It reuses the existing Accounting Inbox manual-upload artifact lifecycle and accepts the verified native CIBC headerless four-column export plus strong headered directional bank signatures. June production evidence establishes `FIRST DATA CANADA(K)` as the Clover deposit descriptor; Uber/Fantuan/First Data strings remain provider hints rather than posting authority. Deposits are compared only with payout rows for the same store/bank/currency whose active canonical payout Journal revalidates, with exact amount and a bounded ±3-day date window. Provider-hint contradictions cannot be labelled exact. Duplicate uploads reuse the original artifact ID exposed by Inbox dedupe. The Settlements UI also lets the operator exclude any deposit from the **current settlement preview** without modifying/deleting bank evidence. This supports cross-period receipts such as 2026-06-02 Uber 26,039c and 2026-06-03 Fantuan 65,354c that belong to the prior 2026-05-25..2026-05-31 settlement period. E-A exclusions are deliberately session-only; durable bank-row reconciliation decisions remain E-B. The GET preview never creates or modifies payout/Journal rows. No schema/migration, provider API, package or context edge is added. Detailed readiness: `docs/architecture/accounting-provider-payout-readiness.md`.
