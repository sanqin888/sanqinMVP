...
**B1-C1 state:** **PRODUCTION VERIFIED / MERGED / CI GREEN / NO MIGRATION / NO GRAPH CHANGE** through PR #2452 / squash `cdd3b47a`; final head `dc849d20` passed CI #6099. Production runs the Journal-authoritative Expense cutover; post-cutover Expense `expense_bmwt1anetgvhiglbc6wsjzf8` creates Expense-owned split/payment facts plus canonical Journal and zero legacy Transaction rows.

**B1-C2 state:** **C2A + C2B LOCAL SOURCE COMPLETE / USER REVIEW PENDING / MIGRATION REQUIRED / DESTRUCTIVE APPROVAL RECORDED / NO GRAPH CHANGE** on `accounting/b1c2-final-persistence-contraction` from latest `origin/dev@bba19e2e`. C2A deletes the two legacy diagnostics and their legacy-only service/test/module wiring; no Web/PWA consumer exists. C2B removes the Prisma `AccountingTransaction` model, `AccountingSourceType` enum, Transaction relations, dead Accounting-owned source-type contract and obsolete architecture assertions. Production still physically contains exactly one active compatibility row (7495 cents expense + 974 cents tax = CAD 84.69) until the user-generated migration is reviewed/deployed, so the compatibility register remains active and B2 remains gated on production closure.
