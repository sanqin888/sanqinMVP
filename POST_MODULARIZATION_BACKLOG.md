...
Current state: **B2-P0A PRODUCTION VERIFIED / B2-P0B DEPLOYED + FORWARD VERIFICATION PENDING / B2-A LOCAL SOURCE IMPLEMENTED / B2-B NOT STARTED**. The readiness audit found a prerequisite reliability gap before Sales Analytics projection work: Orders continued to create durable immutable `order.financial_sale.v1` facts, but the canonical SALE posting service had no runtime consumer after the controlled 2026-09-13 replay.
...
P0B merged through PR #2459 as `2c4cb834`; CI #6118 passed all required Architecture/API/Web gates and production `main` is now running that merge. The Accounting-owned processor posts only missing `IMMUTABLE_SALE_SNAPSHOT` facts through the existing canonical posting service, never auto-reconstructs `LEGACY_CURRENT_ORDER`, and preserves Uber statement authority until `liveOrderFactCutoverAt`. The remaining P0B evidence is one new forward POS/Web paid Order proving automatic exactly-one Journal convergence without manual replay.

B2-A on local branch `accounting/b2-a-sales-analytics-contracts` establishes the non-runtime contract/policy foundation. Orders exposes a dedicated sales-attribution reader for canonical SALE plus immutable adjustment/reversal source facts; the public DTO contains only source/order/store identity, occurredAt, channel, canonical primary payment method and evidence, with no money fields. Direct SALE attribution prefers immutable SALE snapshots over explicit historical `LEGACY_CURRENT_ORDER` fallback. Change attribution preserves immutable change identity/channel/time but inherits primary payment method from the original SALE attribution instead of interpreting change before/after tender state; if that original attribution is unavailable, the dimension remains missing. Accounting owns the allowed Sales/provider source-fact vocabulary, Journal account-to-Sales-component sign policy, Journal tender buckets, attribution quality and provider coverage states `COMPLETE / INCOMPLETE / UNKNOWN / NOT_APPLICABLE`. Contribution semantics stay exactly as already approved: Net Sales Revenue minus Platform Commission, Payment Processing Fees and Platform Promotion Expense; tips/other revenue/advertising/chargeback/provider-other-fee stay separately visible. B2-A adds no route/UI cutover, Prisma schema/migration, package, Journal posting change, scanner allowance or new context direction. B2-B is the next code slice after local review.
