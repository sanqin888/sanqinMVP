...
Current state: **B2-P0A PRODUCTION VERIFIED / B2-P0B DEPLOYED + FORWARD VERIFICATION PENDING / B2-A MERGED + CI GREEN / B2-B LOCAL SOURCE IMPLEMENTED / B2-C NOT STARTED**. The readiness audit found a prerequisite reliability gap before Sales Analytics projection work: Orders continued to create durable immutable `order.financial_sale.v1` facts, but the canonical SALE posting service had no runtime consumer after the controlled 2026-09-13 replay.
...
P0B merged through PR #2459 as `2c4cb834`; CI #6118 passed all required Architecture/API/Web gates and production `main` is now running that merge. The remaining P0B evidence is one new forward POS/Web paid Order proving automatic exactly-one Journal convergence without manual replay.
...
B2-B on local branch `accounting/b2-b-canonical-sales-projection` adds a read-only canonical `GET /accounting/report/sales`. Monetary values come only from Journal lines for the approved Sales/provider source-fact vocabulary. Orders contributes descriptive dimensions only through `ORDER_SALES_ATTRIBUTION_READER`; missing Orders attribution is surfaced as `UNATTRIBUTED`, never reconstructed from `Order.totalCents`. Accounting-owned provider-document identity and Uber historical-replacement Journal anchors are fail-closed if broken. The report exposes total/daily/channel/primary-payment/tender/source projections plus explicit provider coverage, while `/accounting/report/slice`, Sales UI and Dashboard remain unchanged. No schema/migration, package, posting-policy, scanner-allowance or context-direction change is introduced. B2-C is the next slice after B2-B review/CI.
