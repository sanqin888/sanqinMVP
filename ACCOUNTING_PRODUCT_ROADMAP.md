...
Status: **PAYOUT-A LOCAL SOURCE READY FOR REVIEW — B4-B MERGED / CI GREEN — B4-A MERGED / CI GREEN — B3 PRODUCTION VERIFIED / CLOSED — EFA PRODUCTION VERIFIED / CLOSED — B2 PRODUCTION VERIFIED / CLOSED — B1 CLOSED / B0 3V-B PRODUCTION VERIFICATION STILL PENDING — DO NOT REOPEN PHASE 9**  
...
**PAYOUT-A local source state (2026-09-23):** **READY FOR USER REVIEW / NO RUNTIME WRITER / NO MIGRATION / NO DEPENDENCY / NO GRAPH CHANGE** on branch `feat/accounting-payout-a-foundation` from `origin/dev@cbd8bb1d`. The Accounting-owned `accounting.provider_payout.v1` contract is now frozen independently from monthly provider statements: provider/store/business-date/destination BANK/amount/CAD/reference only, with no statement ID or statement-period binding. Its deterministic Journal draft is `TRANSFER`, `PAYMENT`, `Dr BANK / Cr provider PLATFORM_WALLET`, protected by account prerequisites plus a frozen authority hash. Provider pending account IDs are shared by settlement and payout policies so Clover/Uber/Fantuan cannot drift to different assets. Cash Movement now admits TRANSFER Journals and derives movement only from CASH/BANK lines: CASH↔BANK transfers net to zero, while PLATFORM_WALLET→BANK receipts become visible. Production read-only evidence before the change found zero active TRANSFER Journals, so this query correction does not alter existing production report numbers. PAYOUT-A exposes no controller/service/runtime writer and adds no persisted fact yet. Detailed readiness: `docs/architecture/accounting-provider-payout-readiness.md`.
