...
Status: **PAYOUT-C LOCAL SOURCE READY FOR REVIEW — PAYOUT-B MERGED / CI GREEN / MIGRATION MERGED + REVIEWED — PAYOUT-A MERGED / CI GREEN — B4-B MERGED / CI GREEN — B4-A MERGED / CI GREEN — B3 PRODUCTION VERIFIED / CLOSED — EFA PRODUCTION VERIFIED / CLOSED — B2 PRODUCTION VERIFIED / CLOSED — B1 CLOSED / B0 3V-B PRODUCTION VERIFICATION STILL PENDING — DO NOT REOPEN PHASE 9**  
...
**PAYOUT-C local source state (2026-09-23):** **READY FOR USER REVIEW / NO MIGRATION / NO DEPENDENCY / NO GRAPH CHANGE** on `feat/accounting-payout-c-runtime-ui` from `origin/dev@da7edc2b`. A dedicated authenticated Accounting payout controller exposes bounded payout history plus the existing payout-specific write service; no generic Journal route is reused. `/accounting/settlements` now includes a separate Provider bank receipts panel that reads active CAD BANK accounts, accepts provider/store/actual bank date/actual amount/reference, generates a retry-stable client payout ID, and requires an explicit real-bank confirmation before POST. Statement Net payout is not copied or linked. Changing any material form fact invalidates the confirmation/stable-ID attempt. Recent payout history shows destination bank and canonical Journal anchor. PAYOUT-C still adds no bank import or automatic matching; those remain later PAYOUT-E scope. Detailed readiness/implementation: `docs/architecture/accounting-provider-payout-readiness.md`.
