...
Status: **B2 IN PROGRESS — B1 CLOSED / B2-P0A PRODUCTION VERIFIED / B2-P0B PRODUCTION VERIFIED + COMPLETE / B2-A MERGED + CI GREEN / B2-B MERGED + CI GREEN / B2-C LOCAL SOURCE IMPLEMENTED / B2-D NOT STARTED / B0 3V-B PRODUCTION VERIFICATION STILL PENDING — DO NOT REOPEN PHASE 9**  
...
**2026-09-22 readiness state:** **B1 DEPENDENCY SATISFIED / B2-P0A PRODUCTION VERIFIED / B2-P0B PRODUCTION VERIFIED + COMPLETE / B2-A MERGED + CI GREEN / B2-B MERGED + CI GREEN / B2-C LOCAL SOURCE IMPLEMENTED / B2-D NOT STARTED**. The B2 readiness audit found that immutable Orders SALE facts remained durable after the 2026-09-13 historical Journal replay, but `AccountingCanonicalSalePostingService.postCanonicalSale()` had no production runtime caller. This was a continuity gap, not a Journal redesign requirement.
...
**B2-C — Sales UI cutover:** local branch `accounting/b2-c-sales-ui-cutover` moves only the Accounting Sales page from the legacy P&L + `/accounting/report/slice` combination to `/accounting/report/sales`. The page shows canonical Gross Sales, Discounts, Net Sales Revenue, Output Tax, approved Channel Contribution, daily trend, by-channel and canonical-primary-payment attribution, actual Journal tender mix, provider financial coverage, attribution quality and explicit source/adjustment buckets. Equal-period comparison uses the same canonical endpoint and is calculated from the effective clamped report range; if the prior period is outside Accounting coverage it is omitted instead of comparing unequal periods. Dashboard intentionally remains on `/accounting/report/slice`, so the legacy route/type cannot yet be retired. No backend, schema/migration, package, Journal policy or context direction changes are introduced.

Accounting Journal/canonical financial facts continue to own every monetary amount. B2-D should cut Dashboard to the canonical Sales projection and then assess final legacy `/accounting/report/slice` / `OrderReportingFactsModule` cleanup. B2-P0 is now fully production-verified.
