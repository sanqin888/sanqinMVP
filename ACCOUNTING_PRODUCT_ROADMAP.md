...
Status: **PAYOUT-E-B1 LOCAL SOURCE READY FOR REVIEW / MIGRATION REQUIRED; PAYOUT-E-A + INBOX-ONLY + SETTLEMENT ROW-DECISION FOLLOW-UPS MERGED / CI GREEN / DEPLOYED — PAYOUT-D DEPLOYED / BACKEND DATA-PATH VERIFIED / UI SPOT-CHECK PENDING — PAYOUT-C PRODUCTION VERIFIED — PAYOUT-B MERGED / CI GREEN / MIGRATION APPLIED — PAYOUT-A MERGED / CI GREEN — B4-B MERGED / CI GREEN — B4-A MERGED / CI GREEN — B3 PRODUCTION VERIFIED / CLOSED — EFA PRODUCTION VERIFIED / CLOSED — B2 PRODUCTION VERIFIED / CLOSED — B1 CLOSED / B0 3V-B PRODUCTION VERIFICATION STILL PENDING — DO NOT REOPEN PHASE 9**  
...
**PAYOUT-E-B1 state (2026-09-23):** **LOCAL SOURCE READY FOR USER REVIEW / MIGRATION REQUIRED / NO PAYOUT OR JOURNAL WRITER CHANGE / NO DEPENDENCY / NO GRAPH CHANGE.** Adds durable Accounting-owned bank-row reconciliation decisions keyed by immutable artifact row + store + destination bank, with deterministic stable ID, parser-bound fingerprint, operator/time audit and `EXCLUDED / READY_FOR_POSTING / MATCH_EXISTING_PAYOUT` state. Confirmation reparses server-side evidence and derives decision semantics from the current E-A match preview; unresolved possible/ambiguous matches fail closed. First-load UI becomes conservative: only exact existing payout matches start included; all unmatched/possible/ambiguous rows start excluded until explicitly selected. Reopening the same CSV restores confirmed choices. E-B2 remains responsible for atomically binding a READY bank row to exactly one canonical payout. Detailed readiness/implementation: `docs/architecture/accounting-provider-payout-readiness.md`.
