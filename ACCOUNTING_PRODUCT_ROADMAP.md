...
Status: **PAYOUT-B LOCAL SOURCE READY FOR REVIEW / MIGRATION REQUIRED — PAYOUT-A MERGED / CI GREEN — B4-B MERGED / CI GREEN — B4-A MERGED / CI GREEN — B3 PRODUCTION VERIFIED / CLOSED — EFA PRODUCTION VERIFIED / CLOSED — B2 PRODUCTION VERIFIED / CLOSED — B1 CLOSED / B0 3V-B PRODUCTION VERIFICATION STILL PENDING — DO NOT REOPEN PHASE 9**  
...
**PAYOUT-B local source state (2026-09-23):** **READY FOR USER REVIEW / MIGRATION REQUIRED / NO HTTP OR UI ENTRY / NO DEPENDENCY / NO GRAPH CHANGE** on `feat/accounting-payout-b-persistence` from `origin/dev@b919990f`. A new Accounting-owned `AccountingProviderPayout` persistence model stores the frozen v1 fact plus its canonical Journal anchor. `AccountingProviderPayoutService` follows the existing Payroll settlement pattern: exact-fact idempotent retry, conflicting stable-ID reuse rejection, active provider-pending/BANK prerequisite reads, and payout-specific Journal posting plus anchor update inside one Serializable Accounting transaction. The generic Journal create/update/delete paths cannot create or mutate `accounting.provider_payout.v1`. No monthly-statement relation or current-pending-balance gate is added, because real daily/weekly bank deposits may precede monthly statement ingestion; later reconciliation owns balance anomalies. The service is registered internally but intentionally has no controller/runtime caller until the additive migration is generated, reviewed and merged. Detailed readiness/implementation: `docs/architecture/accounting-provider-payout-readiness.md`.
