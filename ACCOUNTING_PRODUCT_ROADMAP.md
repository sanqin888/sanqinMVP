...
Status: **PAYOUT-E-A MERGED / CI GREEN; INBOX-ONLY EVIDENCE FOLLOW-UP LOCAL SOURCE READY FOR REVIEW — PAYOUT-D DEPLOYED / BACKEND DATA-PATH VERIFIED / UI SPOT-CHECK PENDING — PAYOUT-C PRODUCTION VERIFIED — PAYOUT-B MERGED / CI GREEN / MIGRATION APPLIED — PAYOUT-A MERGED / CI GREEN — B4-B MERGED / CI GREEN — B4-A MERGED / CI GREEN — B3 PRODUCTION VERIFIED / CLOSED — EFA PRODUCTION VERIFIED / CLOSED — B2 PRODUCTION VERIFIED / CLOSED — B1 CLOSED / B0 3V-B PRODUCTION VERIFICATION STILL PENDING — DO NOT REOPEN PHASE 9**  
...
**PAYOUT-E-A state (2026-09-23):** **MERGED / CI GREEN / PREVIEW-ONLY / NO MIGRATION / NO NEW WRITER / NO DEPENDENCY / NO GRAPH CHANGE** through PR #2490 / squash `1d90e6fd`, CI #6223 green. E-A reuses Accounting `SourceArtifact` evidence, recognizes the verified native CIBC headerless four-column export plus strong headered directional schemas, and treats Uber/Fantuan/`FIRST DATA CANADA(K)` strings as provider hints only. Matching is same bank/store/CAD, exact amount, bounded ±3 days, and only against payout rows whose active canonical `TRANSFER/PAYMENT/accounting.provider_payout.v1` Journal anchor revalidates. The current local follow-up moves the preview into the owning Inbox workflow: Accounting Inbox is the only Accounting file-upload surface; a CSV row classified as `OTHER_DOCUMENT` or still `UNKNOWN` can preview bank receipts directly, apply session-only Include/exclude decisions, then be classified/reviewed before the operator continues to Settlements for formal posting. The `OTHER_DOCUMENT` persistence value is unchanged while its UI label becomes “银行流水 / 其他资料”. Settlements no longer contains a bank CSV preview or upload surface. Durable bank-row decisions remain E-B scope. Detailed readiness/implementation: `docs/architecture/accounting-provider-payout-readiness.md`.
