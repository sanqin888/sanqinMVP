...
**B4-D1 local state (2026-09-24):** **IMPLEMENTED LOCALLY / AWAITING REVIEW / NO MIGRATION / NO DEPENDENCY / NO JOURNAL OR P&L ARITHMETIC CHANGE / NO GRAPH CHANGE.** Readiness confirmed that Cash Movement is already Journal-only, so D1 does not rebuild it. The remaining reporting gap is Management P&L adjustment explainability: non-Provider-Statement `ADJUSTMENT` Journals are currently collapsed into one signed `adjustmentCents`. Production read-only evidence shows two active Order reversal Journals with revenue net effect `-3,497c` and 119 Uber pre-cutover reversal Journals with revenue net effect `-288,573c`; six Provider Statement Journals are already decomposed through ordinary revenue/expense lines. D1 keeps the existing fact, period and net-profit arithmetic unchanged and adds a reconciled `adjustmentBreakdown` grouped by `source + sourceFactType`, exposing Journal count, revenue net change, expense net change and net-profit effect. Web, Management CSV and Management PDF render the explanation; the service fails closed if breakdown net effect does not equal existing `adjustmentCents`. No owner API, foreign Prisma read, schema/package, route, posting authority or context edge is added. The legacy account-balance route and Sales/Reports copy cleanup remain later B4 follow-ups. Per repository workflow, no local lint/build/test command has been run before review.
