...
Status: **PAYOUT-E-A LOCAL SOURCE READY FOR REVIEW — PAYOUT-D DEPLOYED / BACKEND DATA-PATH VERIFIED / UI SPOT-CHECK PENDING — PAYOUT-C PRODUCTION VERIFIED — PAYOUT-B MERGED / CI GREEN / MIGRATION APPLIED — PAYOUT-A MERGED / CI GREEN — B4-B MERGED / CI GREEN — B4-A MERGED / CI GREEN — B3 PRODUCTION VERIFIED / CLOSED — EFA PRODUCTION VERIFIED / CLOSED — B2 PRODUCTION VERIFIED / CLOSED — B1 CLOSED / B0 3V-B PRODUCTION VERIFICATION STILL PENDING — DO NOT REOPEN PHASE 9**  
...
**PAYOUT-E-A local source state (2026-09-23):** **READY FOR REMOTE DELIVERY / PREVIEW-ONLY / NO MIGRATION / NO NEW WRITER / NO DEPENDENCY / NO GRAPH CHANGE** on `feat/accounting-payout-e-a-bank-match-preview` from `origin/dev@1666b3ed`. E-A reuses existing Accounting manual-upload `SourceArtifact` evidence and now recognizes the verified native CIBC headerless four-column export in addition to strong headered directional schemas. Real June CIBC evidence confirmed Uber/Fantuan EFT descriptions plus `FIRST DATA CANADA(K)` as the Clover settlement descriptor, so those strings are provider hints only. Matching is same bank/store/CAD, exact amount, bounded ±3 days, and only against payout rows whose active canonical `TRANSFER/PAYMENT/accounting.provider_payout.v1` Journal anchor revalidates. Exact status requires same date plus no provider-hint contradiction. Duplicate uploads reuse `duplicateOfArtifactStableId`. The Settlements preview adds an operator-controlled Include checkbox per bank deposit: a row can be excluded from the current settlement preview without editing/deleting bank evidence or posting anything. This is intentionally session-only in E-A; durable exclusion/reconciliation decisions remain E-B scope. The uploaded June sample demonstrates why this is required: the 2026-06-02 Uber 26,039c and 2026-06-03 Fantuan 65,354c bank receipts belong to the prior 2026-05-25..2026-05-31 provider settlement period and therefore must be manually excluded from a June settlement selection despite their June bank dates. No payout or Journal is automatically created. Detailed readiness/implementation: `docs/architecture/accounting-provider-payout-readiness.md`.
