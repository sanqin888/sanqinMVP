...
2026-09-24 follow-up: a Clover statement semantic-detail correction is in **LOCAL REVIEW ONLY**
on `accounting/clover-statement-semantic-v7`. It separates Monthly Equipment Bill base/HST from
card/network fees, maps the base to the existing `expense_software` category, adds exact FEES
detail reconciliation, and preserves category on settlement Journal lines. It has no
schema/dependency change and does not remediate already materialized historical documents; that is
a separate Human Review effective-snapshot follow-up. No PR/CI/deployment state is claimed yet.
