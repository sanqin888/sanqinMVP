...
**B4-D3 local state (2026-09-24):** **IMPLEMENTED LOCALLY / AWAITING REVIEW / HTTP CONTRACT CONTRACTION / NO MIGRATION / NO DEPENDENCY / NO JOURNAL OR REPORT ARITHMETIC CHANGE / NO GRAPH CHANGE.** Readiness confirmed that B4-A had already removed the only Web consumer; repo-wide runtime search found no other caller, and the former `accountBalanceReport()` implementation was a standalone operational Journal-line widget not reused by P&L, Cash Movement, Trial Balance, Balance Movement, annual reporting or exports. Production API logs covering the latest 168 hours contain only Nest's startup route-mapping line for `/accounting/report/account-balance`, with no actual request evidence. D3 removes authenticated `GET /accounting/report/account-balance`, the now-dead service method and unused Web DTO, deletes its single-purpose characterization spec, and converts architecture coverage to explicit non-reintroduction guards while retaining canonical Cash Movement characterization. This is an intentional public HTTP contract contraction; no schema/package/posting/owner boundary or context graph changes are introduced. Per repository workflow, no local lint/build/test command has been run before review.
