...
**B4-D2 local state (2026-09-24):** **IMPLEMENTED LOCALLY / AWAITING REVIEW / ADDITIVE SALES V1 METADATA / NO MIGRATION / NO DEPENDENCY / NO JOURNAL OR SALES MONEY CHANGE / NO GRAPH CHANGE.** B2's deferred polish tail is now addressed without reopening B2: the canonical Sales report exposes its already-resolved `accountingStartDate` as additive range metadata, and the Web computes the equal-length previous period before issuing a second Sales request. If that full prior range begins before `accountingStartDate`, comparison is omitted locally instead of intentionally provoking a 400 and API ERROR-log noise or accepting a clamped unequal period. The UI distinguishes coverage omission from a genuine comparison-read failure, shows Accounting start metadata and no longer claims that KPI comparison is always present. The comparison still uses only `GET /accounting/report/sales`; no P&L/legacy slice fallback, extra owner query, schema/package, posting change or context edge is added. The legacy `GET /accounting/report/account-balance` contraction remains a separate later B4-D3 HTTP-contract task. Per repository workflow, no local lint/build/test command has been run before review.
