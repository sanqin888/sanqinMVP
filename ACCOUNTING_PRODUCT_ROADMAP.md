...
Status: **PAYOUT-D LOCAL SOURCE READY FOR REVIEW — PAYOUT-C PRODUCTION VERIFIED — PAYOUT-B MERGED / CI GREEN / MIGRATION APPLIED — PAYOUT-A MERGED / CI GREEN — B4-B MERGED / CI GREEN — B4-A MERGED / CI GREEN — B3 PRODUCTION VERIFIED / CLOSED — EFA PRODUCTION VERIFIED / CLOSED — B2 PRODUCTION VERIFIED / CLOSED — B1 CLOSED / B0 3V-B PRODUCTION VERIFICATION STILL PENDING — DO NOT REOPEN PHASE 9**  
...
**PAYOUT-D local source state (2026-09-23):** **READY FOR USER REVIEW / READ-ONLY / NO MIGRATION / NO DEPENDENCY / NO GRAPH CHANGE** on `feat/accounting-payout-d-reconciliation-v2` from latest `origin/dev@ac9ffcc2`. A new Accounting-only Provider Pending reconciliation projection reads canonical Journal lines for Clover/Uber/Fantuan pending accounts and produces a date-aware roll-forward: opening Pending + canonical Order movement + provider Statement movement + authority adjustments + other movement − actual payouts = closing Pending. It classifies Uber pre-cutover replacement reversals separately so historical Order authority and statement authority are not double-counted. Provider financial coverage is displayed as evidence status but does not alter Journal arithmetic. Negative Pending, unexpected other Journal movement and reversed payout direction are surfaced as warnings rather than hidden or blocked. `/accounting/settlements` gets a read-only reconciliation panel and `GET /accounting/provider-pending-reconciliation`; no provider API, bank import, write path, schema or package change is added. Detailed readiness/implementation: `docs/architecture/accounting-provider-payout-readiness.md`.
