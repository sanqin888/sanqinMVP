...
Status: **B2 IN PROGRESS — B1 CLOSED / B2-P0A PRODUCTION VERIFIED / B2-P0B DEPLOYED + FORWARD VERIFICATION PENDING / B2-A LOCAL SOURCE IMPLEMENTED / B2-B NOT STARTED / B0 3V-B PRODUCTION VERIFICATION STILL PENDING — DO NOT REOPEN PHASE 9**  
...
**2026-09-22 readiness state:** **B1 DEPENDENCY SATISFIED / B2-P0A PRODUCTION VERIFIED / B2-P0B DEPLOYED + FORWARD VERIFICATION PENDING / B2-A LOCAL SOURCE IMPLEMENTED / B2-B NOT STARTED**. The B2 readiness audit found that immutable Orders SALE facts remained durable after the 2026-09-13 historical Journal replay, but `AccountingCanonicalSalePostingService.postCanonicalSale()` had no production runtime caller. This was a continuity gap, not a Journal redesign requirement.
...
**B2-P0B — Continuous Canonical SALE Posting:** PR #2459 merged as `2c4cb834`; CI #6118 passed Architecture plus all API/Web lint/build/strict/tests, and production `main` is now running `2c4cb834`. The Accounting-owned convergence processor performs startup full recovery in bounded seven-day chunks, frequent recent-window reconciliation, periodic full reconciliation, batch Journal-anchor exclusion and bounded writes through the existing canonical posting service. Only `IMMUTABLE_SALE_SNAPSHOT` facts are eligible automatically; `LEGACY_CURRENT_ORDER` remains controlled-replay-only. Uber remains deferred while Accounting provider coverage has no `liveOrderFactCutoverAt`. Deployment is confirmed; the remaining P0B gate is one new forward POS/Web paid Order proving immutable SALE fact -> exactly one canonical SALE Journal without manual replay.

**B2-A — Sales Analytics Contracts + Attribution Policy:** local branch `accounting/b2-a-sales-analytics-contracts` establishes the implementation boundary without changing current report/UI behavior. Orders publishes a dedicated non-monetary sales-attribution reader keyed by canonical Order financial `sourceFactStableId`, covering SALE plus immutable adjustment/reversal facts. Direct SALE attribution prefers immutable sale snapshots with explicit `LEGACY_CURRENT_ORDER` historical fallback. Change attribution keeps its immutable change source identity/channel/time, while canonical primary payment method is inherited from the original SALE attribution rather than guessed from change before/after tender state; its evidence remains explicit and a missing original SALE attribution stays missing. The public DTO contains only descriptive identity/dimension/evidence fields: no `*Cents`, `Order.totalCents` or payment-breakdown amounts cross the boundary. Accounting defines the canonical Journal account -> Sales component sign policy, allowed Sales/provider source-fact vocabulary, Journal tender buckets, attribution-quality states and provider coverage states `COMPLETE / INCOMPLETE / UNKNOWN / NOT_APPLICABLE`. The existing roadmap formula remains authoritative: `Channel Contribution = Net Sales Revenue - Platform Commission - Payment Processing Fees - Platform Promotion Expense`; Tips, Other Operating Revenue, Advertising, Chargeback and provider-other-fee remain separately visible and are not silently folded into Contribution. No route, UI, Journal schema/policy, Prisma schema/migration, package, context direction or scanner allowance is changed in B2-A.

Accounting Journal/canonical financial facts continue to own every monetary amount. B2-B may now compose Journal amounts with the new Orders descriptive attribution and Accounting-owned provider coverage, but the legacy `/accounting/report/slice` remains active until the later Sales/Dashboard cutovers. P0B forward verification remains required before B2 closeout, but the user explicitly allowed non-runtime B2-A source work to proceed while the store awaited the next real order.
