...
Status: **PAYOUT-E-A MERGED / CI GREEN; INBOX-ONLY EVIDENCE FOLLOW-UP MERGED / CI GREEN; SETTLEMENT ROW-DECISION FOLLOW-UP LOCAL SOURCE READY FOR REVIEW — PAYOUT-D DEPLOYED / BACKEND DATA-PATH VERIFIED / UI SPOT-CHECK PENDING — PAYOUT-C PRODUCTION VERIFIED — PAYOUT-B MERGED / CI GREEN / MIGRATION APPLIED — PAYOUT-A MERGED / CI GREEN — B4-B MERGED / CI GREEN — B4-A MERGED / CI GREEN — B3 PRODUCTION VERIFIED / CLOSED — EFA PRODUCTION VERIFIED / CLOSED — B2 PRODUCTION VERIFIED / CLOSED — B1 CLOSED / B0 3V-B PRODUCTION VERIFICATION STILL PENDING — DO NOT REOPEN PHASE 9**  
...
**PAYOUT-E-A state (2026-09-23):** **MERGED / CI GREEN / PREVIEW-ONLY / NO MIGRATION / NO NEW WRITER / NO DEPENDENCY / NO GRAPH CHANGE** through PR #2490 / squash `1d90e6fd`, CI #6223 green. E-A reuses Accounting `SourceArtifact` evidence, recognizes the verified native CIBC headerless four-column export plus strong headered directional schemas, and treats Uber/Fantuan/`FIRST DATA CANADA(K)` strings as provider hints only. Matching is same bank/store/CAD, exact amount, bounded ±3 days, and only against payout rows whose active canonical `TRANSFER/PAYMENT/accounting.provider_payout.v1` Journal anchor revalidates. The Inbox-only workflow contraction merged through PR #2492 / squash `c4324edd`, CI #6229 green: Accounting Inbox is the only Accounting file-upload surface; a CSV row classified as `OTHER_DOCUMENT` or still `UNKNOWN` can preview bank receipts directly, and the `OTHER_DOCUMENT` persistence value remains unchanged while its UI label is “银行流水 / 其他资料”. The current local follow-up tightens ownership one step further: Inbox preview is evidence-only and has no Include/Exclude controls; Settlements reads only reviewed `CONFIRMED + OTHER_DOCUMENT + CSV` artifacts, owns session-only settlement Include/Exclude, and may populate the formal payout form only from an included `UNMATCHED` row with a provider hint. Exact/possible/ambiguous matches cannot create a new payout through this handoff. Durable bank-row decisions remain E-B scope. Detailed readiness/implementation: `docs/architecture/accounting-provider-payout-readiness.md`.
