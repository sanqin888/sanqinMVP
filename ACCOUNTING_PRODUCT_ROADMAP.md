...
**B4-C2 local state (2026-09-24):** **IMPLEMENTED LOCALLY / AWAITING REVIEW / NO MIGRATION / NO DEPENDENCY / NO JOURNAL CHANGE / NO GRAPH CHANGE.** Readiness audited the canonical source identities before implementation: production currently contains nine active `sourceFactType` values, while code also defines currently-unseen `order.financial_adjustment.v1`, `payroll.cra_remittance.v1` and `payroll.run.reversal.v1`. C2 adds a pure Web source-navigation adapter over the C1 identity only. Stable destinations are mapped for Order SALE → existing order detail, Expense v1/v2 → Expense records, Provider Statement → the existing Settlements evidence card, Provider Payout → the existing Settlements bank-receipt row, and Payroll accrual/reversal → the existing Payroll run UI. Expense records, Provider Statement Inbox evidence and provider-payout history receive only additive same-owner exact stable-ID query filters so an older source cannot disappear behind bounded list windows; Payroll reuses its already-existing Accounting-owned run-by-ID endpoint. Order adjustment/reversal, Payroll employee-payment/CRA, Uber pre-cutover reversal and unknown types remain identity-only because their source stable IDs do not reliably identify an existing destination key. The Journal drawer performs no Orders/Payroll/Expense/provider owner fetch and creates no page-local financial DTO. Per repository workflow, no local lint/build/test command has been run before review.
