...
Status: **B2 IN PROGRESS — B1 CLOSED / B2-P0A PRODUCTION VERIFIED / B2-P0B PRODUCTION VERIFIED + COMPLETE / B2-A MERGED + CI GREEN / B2-B MERGED + CI GREEN / B2-C MERGED + CI GREEN / B2-D LOCAL SOURCE IMPLEMENTED + REVIEW PENDING / B2-E CLEANUP READINESS AUDITED / B0 3V-B PRODUCTION VERIFICATION STILL PENDING — DO NOT REOPEN PHASE 9**  
...
**2026-09-22 readiness state:** **B1 DEPENDENCY SATISFIED / B2-P0A PRODUCTION VERIFIED / B2-P0B PRODUCTION VERIFIED + COMPLETE / B2-A MERGED + CI GREEN / B2-B MERGED + CI GREEN / B2-C MERGED + CI GREEN / B2-D LOCAL SOURCE IMPLEMENTED + REVIEW PENDING / B2-E CLEANUP READINESS AUDITED**. The B2 readiness audit found that immutable Orders SALE facts remained durable after the 2026-09-13 historical Journal replay, but `AccountingCanonicalSalePostingService.postCanonicalSale()` had no production runtime caller. This was a continuity gap, not a Journal redesign requirement.
...
**B2-D — Dashboard Canonical Sales cutover:** **LOCAL SOURCE IMPLEMENTED / REVIEW PENDING** on `accounting/b2-d-dashboard-canonical-sales`. Dashboard keeps its existing Accounting-owned dashboard summary/expense/attention cards but replaces its two legacy Orders paid-total sales summaries with the existing `AccountingSalesAnalyticsReport` from `GET /accounting/report/sales`. The overview intentionally uses only `byChannel[*].summary.netSalesRevenueCents` and `byPrimaryPaymentMethod[*].summary.netSalesRevenueCents`; the payment section is explicitly labelled primary-payment revenue attribution and is not presented as tender mix. The full Sales-only daily, provider-coverage, tender and source/adjustment detail remains on the Sales page. No backend authority, schema/migration, package, Journal posting, settlement, Orders financial-fact or attribution-owner change is introduced.

**B2-E cleanup readiness:** after the B2-D Web cutover, live Web/PWA consumers of `/accounting/report/slice` and `AccountingOrderDimensionSlice` are zero. `AccountingService.dimensionSlice()` is the sole production caller of `readPaidTotalDimensionsForRange()`, and the paid-total DTOs are dedicated to that method. However, `OrderReportingFactsModule` must remain because `apps/api/src/reports/reports.module.ts` still consumes the same Orders reporting module for `readMetricsForRange()` and `readItemsForRange()`. A later explicit B2-E contraction may remove the Accounting slice route/service wiring, paid-total-only contract/method/types and obsolete characterization/architecture pins, while retaining the shared Orders reporting module for operational Reports. No B2-E deletion is included in B2-D.
