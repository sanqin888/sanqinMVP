...
Status: **B2 IN PROGRESS — B1 CLOSED / B2-P0A PRODUCTION VERIFIED / B2-P0B DEPLOYED + FORWARD VERIFICATION PENDING / B2-A MERGED + CI GREEN / B2-B LOCAL SOURCE IMPLEMENTED / B2-C NOT STARTED / B0 3V-B PRODUCTION VERIFICATION STILL PENDING — DO NOT REOPEN PHASE 9**  
...
**2026-09-22 readiness state:** **B1 DEPENDENCY SATISFIED / B2-P0A PRODUCTION VERIFIED / B2-P0B DEPLOYED + FORWARD VERIFICATION PENDING / B2-A MERGED + CI GREEN / B2-B LOCAL SOURCE IMPLEMENTED / B2-C NOT STARTED**. The B2 readiness audit found that immutable Orders SALE facts remained durable after the 2026-09-13 historical Journal replay, but `AccountingCanonicalSalePostingService.postCanonicalSale()` had no production runtime caller. This was a continuity gap, not a Journal redesign requirement.
...
**B2-B — Canonical Journal Sales Projection:** local branch `accounting/b2-b-canonical-sales-projection` adds the new read-only `GET /accounting/report/sales` projection while leaving `/accounting/report/slice`, the Sales UI and Dashboard untouched. Every amount comes from canonical Accounting Journal lines selected by the B2-A Sales/provider source-fact allowlist. Orders contributes only descriptive channel/primary-payment attribution through `ORDER_SALES_ATTRIBUTION_READER`; missing Orders dimensions remain visible under `UNATTRIBUTED` instead of falling back to `Order.totalCents`. Provider documents remain Accounting-owned and identify Uber/Fantuan/Clover statement attribution; missing provider-document links or broken Uber historical-replacement anchors fail closed. Historical Uber revenue remains Journal-authoritative by combining the original SALE, its explicit pre-cutover reversal and the provider-statement Journal. The projection exposes totals, daily series, channel, primary-payment attribution, actual Journal tender mix, source breakdown and explicit provider coverage. No schema/migration, package, Journal posting policy, scanner allowance or context direction changes are introduced.

Accounting Journal/canonical financial facts continue to own every monetary amount. B2-C may cut the Sales UI to `/accounting/report/sales` only after B2-B source review/CI. The legacy `/accounting/report/slice` remains active until the later Sales and Dashboard cutovers. P0B forward verification remains required before B2 closeout.
