...
|---|---|---|---|---|
| `brand-store.business-config.v1` | active | BusinessConfig → BrandConfig + StoreConfig snapshot; POS StoreStatus/Connectivity reads are canonical, POS compatibility writes plus Admin/Uber legacy reads remain | All field owners assigned; difference report zero for one business cycle; old reads/writes zero | Before Phase 2 exit |
