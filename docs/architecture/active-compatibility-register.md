...
| `brand-store.default-store-identity.v1` | active | implicit `default` store → explicit `storeStableId`; canonical POS and Admin Store contexts are explicit while singular staff routes remain stale-bundle compatibility | Rows backfilled; implicit resolution metric zero; schema default and fallback routes removed after observation | Before Phase 2 exit |
