...
|---|---|---|---|---|
| `payments.pos-card-legacy.v1` | active / pre-production | direct paid Order → Unified Payment Core + Terminal + finalize | POS ↔ Clover realtime/recovery complete; real-device acceptance; canonical charged-total/surcharge/tip + reversal authority complete; durable Accounting payment-fact cutover deliberately recorded; one settlement cycle reconciled; pre-cutover Accounting facts resolved; clean production stability window; legacy calls zero | Phase J cleanup after Terminal synchronization/cutover stability |
...
New in-store CARD SALE facts freeze additive Orders-owned `posCardExecutionEvidence=LEGACY_DIRECT_PAID|UNIFIED_PAYMENT_CORE` from the existing persisted payment-breakdown shape. Accounting uses that immutable provenance first. Older SALE facts that predate the field are resolved from existing owner facts: a canonical Payments CARD SALE fact proves Unified/strict handling; no Payments CARD SALE fact means the legacy direct-paid route under this registered compatibility. This provenance does not replace the separate durable Accounting `providerPaymentFactCutoverAt`: the POS rollout flag remains route-choice infrastructure, while the persisted cutover becomes the one-way financial-history boundary and must not move backward on a later flag cutback. At/after that durable boundary, missing canonical Payments facts fail closed into reconciliation rather than falling back to Order CARD or Closeout guessing. Legacy/order-declared compatibility relaxes refund/negative settlement evidence only; CARD collection/additional charge stays strict. Phase J exit additionally requires provider-proven charged total, surcharge and tip authority (plus matching reversal facts), deliberate durable cutover recording, resolved pre-cutover Accounting facts and zero legacy invocation before the legacy compatibility and routes are removed together. Slice E1 source plus the additive nullable `tipCents` migration are now on dev with CI #6448 green. Slice E2 locally extends the same provider authority through managed refund/void: refunded tip is independently proven and published as nullable `tipRefundCents`, while webhook evidence remains null when it cannot prove tip. Neither slice sets the durable cutover; production Web Ecommerce remains frozen.
