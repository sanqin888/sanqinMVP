...
2026-09-21 Post-Modularization Accounting **B1-B Expense owner-read + atomic Journal authority** is **LOCAL SOURCE COMPLETE / USER REVIEW PENDING / NO MIGRATION / NO GRAPH CHANGE / NO REPORT CUTOVER** on branch `accounting/b1b-expense-journal-authority` from `origin/dev@7e54853a`. Expense API/C0 canonical split authority moves to `AccountingExpenseSplit`; legacy Transaction rows remain only for split parity and current financial-report arithmetic. Expense API/Web cut directly to `splitStableId`; no old-PWA `txStableId` compatibility field is retained. Complete reviewed payment allocations post a canonical Expense Journal inside the same Serializable Expense write; unknown-payment confirmation remains unposted and later payment completion atomically adds allocations + Journal. The Journal service exposes an Expense-specific in-transaction authority that revalidates persisted document/split/payment identities before write. No context edge, direct-import allowance, scanner baseline, public SCC, Prisma schema, provider behavior or package dependency changes. B1-C still owns report cutover and legacy Transaction contraction.
