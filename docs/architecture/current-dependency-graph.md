...
Phase 9 Slice 8P-A Payroll readiness/schema contract analysis is **ANALYSIS COMPLETE / REVIEW PENDING / NO SOURCE IMPLEMENTATION / NO MIGRATION** from `origin/dev@6d216eab`. The reviewed design keeps Payroll as an Accounting-internal vertical and closes the core B1 contract around PayrollEmployer, immutable employer/employee config versions, PayrollEmployee, same-employer tax-year opening evidence, PayrollRun lifecycle/YTD snapshots, explicit Payroll Journal source/write-authority direction and later settlement boundaries. Production CoA still has 21 active accounts and no Payroll control accounts; `expense_labor` remains only a reporting category. The proposed B1 source slice is schema/contracts/architecture only, with statutory formulas, HTTP/UI, Journal posting, CoA provisioning, settlement tables and PDF dependency explicitly deferred. This analysis introduces no graph/baseline change; the authoritative baseline remains Foundation **1** / External **1** / Identity **2** / Runtime **4**, total **8**, public SCC empty.
