...
2026-09-22 Accounting **EFA-B1 Expense funding attribution foundation** is **LOCAL SOURCE IMPLEMENTED / REVIEW PENDING / MIGRATION REQUIRED / NO GRAPH CHANGE** on `accounting/efa-b1-funding-foundation` from `origin/dev@eeca118e`. The additive Accounting-owned schema seam introduces Expense funding versioning, nullable split -> funding-account attribution and an account-level Management-Expense inclusion policy while preserving historical Expense v1 allocations/Journals and the generic Journal schema. Current v1 runtime posting/reporting is intentionally unchanged in B1. The single-user Accounting PWA does not require long-lived old-write compatibility for the later v2 cutover. No new context direction, direct-import allowance, scanner baseline, SCC, package or provider path is introduced; B3 Trial Balance waits for EFA closeout. **MIGRATION REQUIRED** before production promotion; detailed audit/design is `docs/architecture/accounting-expense-funding-attribution.md`.
