...
2026-09-10 Phase 8 Slice 8.3A is implemented locally after explicit architecture authorization. Orders now owns a provider-neutral `ORDER_EXTERNAL_FACTS_READER` and narrow `OrderExternalFactsModule`; `ubereats.module.ts` maps it to Uber action/sync/operations query ports for API and worker, and the worker does not import `OrdersModule`. All nine pure canonical Order read operations move out of Uber persistence. The three reads coupled to existing writes remain inside their original transactions—two action-completion reads and one cancellation read—so 8.3B/8.3C atomicity is unchanged. The application-facing existing-order chain uses `orderStableId`, while the cancellation transaction resolves `Order.id` internally. Deleting the old Uber sync Prisma repository contracts `external-channels -> runtime-data-ci-ops` **24 -> 23**, reducing External total **31 -> 30**; External -> Orders remains **1**, public SCC remains empty, and no schema/migration or dependency change is introduced. This is LOCAL SOURCE only; remote CI and deployment are not claimed.
