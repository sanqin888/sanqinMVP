...
2026-09-19 post-closeout Identity/Staff hardening is **SOURCE IMPLEMENTED / LOCAL REVIEW APPROVED / REMOTE SUBMISSION AUTHORIZED / NO MIGRATION** without reopening Phase 4: `StaffAdministrationService.updateStaff()` now protects the existing last-active-admin invariant with a PostgreSQL/Prisma `Serializable` transaction plus whole-use-case `P2034` retry. The Staff owner/public boundary, Admin transport adapter, stable-ID contract and direct-import graph are unchanged; expected baseline remains Foundation **1** / External **1** / Identity **2** / Runtime **4**, total **8**, public SCC empty. The final-tail audit classifies the Admin Members STAFF/ADMIN overlap as explicit development/test compatibility to be contracted later when membership-system testing no longer needs those identities.
...
A separate post-modularization Accounting product roadmap now exists at `ACCOUNTING_PRODUCT_ROADMAP.md`. It is **PLANNED / START GATE NOT YET SATISFIED** and must not reopen Phase 9. Implementation begins only after Identity/Staff last-active-admin atomicity is closed or explicitly dispositioned and the repository-wide final modularization tail audit formally classifies all remaining ownership/concurrency/direct-edge tails. Its approved sequence is Expense -> canonical Journal contraction, canonical Sales Analytics, Trial Balance, then the zero-opening **资产负债变动表 / Balance Movement Statement**; a formal Balance Sheet waits for a later real fiscal-year opening balance.
...
A separate, already-documented **L3 reliability** tail remains in Commerce/Fulfillment: private in-memory `order.paid.verified` still drives Uber Direct dispatch, so provider success followed by local `externalDeliveryId` persistence/process failure is not durably replay-safe. This is a real durability/idempotency improvement candidate, but it is not a cross-context ownership/cycle/concurrency blocker and Phase 5 intentionally kept it outside its closed modularization scope. Printer-agent workspace packaging, the no-op `NotificationProcessor`, and similar dead/ops hygiene are likewise not owner-boundary blockers.

Therefore the repository-wide modularization tail is **fully dispositioned at the source/architecture level** once this Staff atomicity hardening passes reviewed PR CI: no remaining historical baseline count should be reduced solely for numeric cleanliness. The Admin Members STAFF/ADMIN overlap is an explicitly deferred development/test compatibility cleanup, the two Payment compatibilities remain provider/cutover gates, and the Uber Direct item remains a separate L3 reliability candidate rather than a modularization blocker.
