...
2026-09-15 Phase 9 Accounting image OCR + post-confirm binary retention is **SOURCE IMPLEMENTED / LOCAL REVIEW GATE / NO GRAPH CHANGE EXPECTED** on branch `phase9-accounting-image-retention`, based on `origin/dev@9c358405`. OCR and retention stay wholly inside Accounting: original bytes are the OCR source; post-confirm compression is a separately audited image-storage lifecycle using the existing Sharp/Tesseract stack. A dedicated TransactionClient-only retention writer owns the new Accounting-internal persistence while the orchestration service reaches it through the existing `AccountingOperationsService`; there is no new context import or `PrismaService` boundary. Confirmed image Expense attachments use an Accounting-local stable artifact-content URL so derivative acceptance never rewrites posted Transaction/Expense URLs. A new Accounting-only read query exposes a persistent image-optimization queue for `ORIGINAL_PRESENT`, `CANDIDATE_READY`, and `PURGE_PENDING`, allowing browser refresh/reopen recovery without adding another ownership edge. The new retention enum/table requires a user-generated migration, but no package dependency, external/public context edge, scanner allowance, provider wire contract, Journal authority or posting policy changes. Direct-import baseline remains Foundation **2**, External **1**, Identity **2**, Runtime **6**, total **11**, public SCC empty pending authoritative CI.
