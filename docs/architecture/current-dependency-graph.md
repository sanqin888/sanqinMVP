...
2026-09-22 Accounting **B2-D Dashboard Canonical Sales cutover** is **LOCAL SOURCE IMPLEMENTED / REVIEW PENDING / EXPECTED NO GRAPH CHANGE** on branch `accounting/b2-d-dashboard-canonical-sales`. Dashboard keeps its existing Accounting-owned summary/expense/attention read path and replaces only the channel/payment sales overview with the already-established `AccountingSalesAnalyticsReport` from `/accounting/report/sales`. Channel and canonical primary-payment rows display Journal-owned `netSalesRevenueCents`; tender/provider/source/daily detail stays on the full Sales page. No API owner edge, schema/migration, package, scanner allowance, SCC, posting or settlement mutation is added.

The accompanying **B2-E cleanup readiness audit** finds zero live Web/PWA consumers of `/accounting/report/slice` / `AccountingOrderDimensionSlice` after B2-D. The only production caller of `readPaidTotalDimensionsForRange()` is then `AccountingService.dimensionSlice()`, so that paid-total-only surface is a later contraction candidate. `OrderReportingFactsModule` itself remains required by `apps/api/src/reports/reports.module.ts` for `readMetricsForRange()` and `readItemsForRange()`; therefore B2-E must remove the Accounting dependency without deleting the shared Orders reporting module. No B2-E cleanup is performed in B2-D.
