...
2026-09-12 Phase 9 Slice 5D readiness is **COMPLETE / NO GRAPH CHANGE** at `origin/dev@8cacdf60`. The audit confirms the 5B Journal writer is ready for canonical fact consumption, but the provisional `Order.totalCents` revenue path cannot be promoted as-is. Production read-only evidence shows historical PaymentTransaction coverage is absent, Store Balance top-up/use is real, most existing OrderAmendment rows lack authoritative occurrence timestamps, and WECHAT_ALIPAY needs an explicit Accounting funds-flow policy. The next safe slice is 5D-A Canonical Financial Facts Boundary: Orders / Payments / Benefits remain fact owners and Accounting consumes versioned public facts; 5D-A itself must not add Journal posting, new direct implementation imports, Prisma changes or scanner allowances. The current merged Accounting / Reporting / Analytics baseline therefore remains Foundation **2**, External **1**, Identity **2**, Runtime **6**, total **11**, with `legacyPublicCycleComponents=[]`.
