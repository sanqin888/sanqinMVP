...
2026-09-21 post-modularization Accounting Document Recognition Reliability Slice B implements ordinary Expense source-amount reconciliation plus a versioned Expense Human Review authority entirely inside Accounting. Poppler/Textract Expense extraction exposes fail-visible `MATCHED / MISMATCH / INSUFFICIENT` financial consistency, geometry-backed native-PDF amount pairing, and a Bell-style regression preventing HST/discount mispairing. `AccountingExpenseReviewRevision` binds human-effective Expense fields to Inbox version + ParseRun hash and final Expense materialization requires the current confirmed review when machine values are inconsistent or machine-observed facts are corrected. The additive schema requires a user-generated migration before production promotion, but introduces **NO GRAPH CHANGE**: no cross-context capability direction, direct import, scanner allowance or SCC is added. The authoritative Accounting direct-import baseline remains Foundation **1** / External **1** / Identity **2** / Runtime **4**, total **8**, public SCC empty; Phase 9 remains closed.
