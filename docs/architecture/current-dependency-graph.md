...
2026-09-21 Accounting **B1-C2 Expense compatibility contraction** is **C2A + C2B SOURCE COMPLETE / MIGRATION REQUIRED / DESTRUCTIVE APPROVAL RECORDED / NO GRAPH CHANGE** on branch `accounting/b1c2-final-persistence-contraction-v2` from `origin/dev@5c84100b`. PR #2455 preserves the production/readiness evidence. C2A removes both legacy comparison routes and their legacy-only implementation; C2B removes `AccountingTransaction`, `AccountingSourceType` and Transaction relations while architecture guards prevent reintroduction. No context direction or scanner allowance changes. Final compatibility closure remains blocked on the separately user-generated destructive migration and production verification.
