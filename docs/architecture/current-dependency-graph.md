...
2026-09-22 Accounting **EFA-C Expense write/UI cutover** is **LOCAL SOURCE IMPLEMENTED / REVIEW PENDING / NO NEW MIGRATION / NO GRAPH CHANGE** on `accounting/efa-c-expense-write-ui-cutover` from `origin/dev@6674ab1c`. Current Expense and Inbox writes stay inside Accounting but move to explicit version-2 split funding; historical v1 allocation reads and completion remain available. The Web adapter moves payment-account selection onto each category row, adds split-level completion for confirmed-unposted v2 Expenses, and records/query dual-reads v1/v2 funding. Accounting Settings exposes create/view/update configuration for `includeFundedExpensesInManagementReports`, but no reporting arithmetic consumes it until EFA-D. There is no new context direction, direct-import allowance, scanner baseline, SCC, package/provider change, Prisma schema change or JournalLine funding dimension. Production deployment is gated on first applying the already-reviewed B1 additive migration and reinstalling the single-user Accounting PWA.
