...
Phase 9 Slice 7-B Accounting actor/stable-user identity contraction is **LOCAL SOURCE / REVIEW GATE / MIGRATION REQUIRED / NO GRAPH CHANGE EXPECTED** on `refactor/phase9-slice7b-accounting-actor-identities`. It distinguishes human-only persisted `*UserStableId` fields from the Journal/Audit actor-reference space that legitimately contains either a user stable ID or registered `system:*` actor key. Journal `createdBy/updatedBy` and Audit `operator` persistence become `*ActorRef`; Transaction/Expense/PeriodClose human fields become explicit `*UserStableId`. Existing Audit Web/PWA response/query naming remains externally compatible until the planned Slice 8B cleanup, with no dual-write path. This is entirely Accounting-owned persisted-contract cleanup: no financial posting, provider, Orders/Payments/Loyalty ownership, context direction or scanner allowance changes are introduced, so the expected baseline remains Foundation **1** / External **1** / Identity **2** / Runtime **6**, total **10**, public SCC empty.
