...
2026-09-21 Post-Modularization Accounting **B1-A Expense split ownership foundation** is **SOURCE + COMPANION MIGRATION MERGED TO DEV / CI GREEN / PRODUCTION DEPLOYMENT NOT CLAIMED / NO GRAPH CHANGE**. Source merged in PR #2448 as `8614633a` after CI #6082; user-generated migration `20260921224139_post_mod_accounting_b1a_expense_split_ownership` is on `dev@7e54853a` and SQL review found only the expected additive table/index/FK operations. No backfill/drop/rename/enum change is present.

2026-09-21 Post-Modularization Accounting **B1-B Expense owner-read + atomic Journal authority** is **MERGED / CI GREEN / PRODUCTION VERIFICATION PENDING / NO MIGRATION / NO GRAPH CHANGE / NO REPORT CUTOVER** through PR #2449 / squash `324a16eb`; final head `acb8db42` passed CI #6086. Expense API/C0 canonical split authority uses `AccountingExpenseSplit`; complete reviewed payment allocations post a canonical Expense Journal inside the same Serializable Expense write, while unknown-payment confirmation remains unposted until completion. The Expense-specific in-transaction authority revalidates persisted document/split/payment identities before write. The post-merge production preflight still shows no `AccountingExpenseSplit` table and zero confirmed Expense/legacy Expense Transaction/canonical Expense Journal facts, so production verification is not claimed.

2026-09-21 Post-Modularization Accounting **B1-C0 Expense report parity preview** is **LOCAL SOURCE COMPLETE / USER REVIEW PENDING / READ-ONLY / NO CUTOVER / NO MIGRATION / NO GRAPH CHANGE** on branch `accounting/b1c-expense-report-cutover` from `origin/dev@324a16eb`. It adds a read-only parity endpoint comparing legacy and canonical Expense category-level P&L, input tax, account movement, CASH/BANK cashflow, split persistence and Journal anchor coverage. Empty production data is deliberately not accepted as cutover proof. Authoritative reports and the legacy Transaction writer remain unchanged until B1-C1 receives real production parity evidence. No context edge, direct-import allowance, scanner baseline, public SCC, schema or dependency changes.
