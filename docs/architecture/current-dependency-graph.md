...
Phase 9 Slice 8 readiness audit is **COMPLETE / READY FOR SLICE 8A** at `origin/dev@17b48d7c`, and Slice **8A-1 Period + Journal capability extraction is SOURCE READY FOR REMOTE CI / NO MIGRATION** on `refactor/phase9-slice8a1-period-journal-capabilities`. The approved sequence remains **8A Accounting backend internal capability split -> 8P Payroll -> 8B Accounting Web vertical-contract cleanup -> Phase 9 closeout**. 8A-1 physically moves Store-local timezone/accounting-start/Period locking into `AccountingPeriodService` and double-entry Journal mutation/idempotency/authority/revalidation into `AccountingJournalService`; canonical SALE/change/provider-settlement services and `AccountingOperationsService` no longer depend on broad `AccountingService` for those capabilities. The existing Controller route shapes remain unchanged. A single Accounting-local `ACCOUNTING_DB` token reuses the existing PrismaService composition binding. Remote architecture gate #5831 showed the ownership move also contracts Accounting -> Runtime direct debt **6 -> 5**; the Brand/Store migrated-consumer registry now follows `accounting-period.service.ts`, and the monotonic baseline is lowered rather than widened. Generic `AccountingTransaction` CRUD remains for 8A-3 and `pnlReport()` / `dashboard()` remain on legacy facts until 8A-4. No new public edge, scanner allowance, Prisma schema/migration or SCC is introduced; the authoritative limits are now Foundation **1** / External **1** / Identity **2** / Runtime **5**, total **9**, public SCC empty.
