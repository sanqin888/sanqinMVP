...
Phase 9 Slice 8 readiness audit is **COMPLETE / READY FOR SLICE 8A**. Slice **8A-1 Period + Journal capability extraction is MERGED / PR CI GREEN / NO MIGRATION** through PR #2380 / squash `8bc1f77e`; final head `655918d8` passed CI #5834. Slice **8A-2 Expenses / Inbox / Chart / settlement-read capability split is MERGED / PR CI GREEN / NO MIGRATION** through PR #2381 / squash `eaff4c0a`; functional code head `83d4473d` passed CI #5840. Slice **8A-3 generic single-entry Transaction surface contraction is MERGED / PR CI GREEN / NO MIGRATION** through PR #2382 / exact head `9c803efc` / squash `14caf98a`; CI #5843 passed Architecture and all API/Web gates. Generic authenticated `/accounting/tx` CRUD and the retired legacy revenue guard are absent, while Expense remains the only production mutator of `AccountingTransaction`. Slice **8A-4 Financial Reports canonical fact cutover is MERGED / PR CI GREEN / NO MIGRATION** through PR #2383 / exact head `6b0df942` / squash `66a1873b`; CI #5847 passed Architecture and all API/Web gates. P&L/dashboard/annual/cashflow/account-balance and report exports belong to `AccountingFinancialReportsService` and combine canonical Journal facts with only confirmed Expense category splits; `EXPENSE_DOCUMENT` Journal entries are excluded to prevent double counting, recoverable HST stays outside P&L expense, provider-statement lines contribute only posted account-class effects, and cashflow uses actual CASH/BANK movements rather than P&L totals or platform-receivable balances. `AccountingService` is now only the separate Orders `paidAt + totalCents` dimension projection plus Audit-log reads. Slice **8A-5 Controller vertical split + closeout guard has IMPLEMENTATION CI GREEN / FINAL DOCS HEAD PENDING EXACT-HEAD CI / NO MIGRATION** through PR #2384 on `refactor/phase9-slice8a5-accounting-controller-vertical-split`, implementation head `7f64cb07`, from merged `origin/dev@66a1873b`; initial CI #5850 failed API lint only on eight Prettier findings after Architecture/Web passed, and formatting-only follow-up CI #5851 passed Architecture plus all API/Web lint/build/strict/tests: the 16-dependency god `AccountingController` is deleted; the unchanged authenticated `/accounting/**` route set is distributed across twelve explicit vertical controllers under the same staff-auth policy and one `AccountingModule`; a closeout regression pins the controller set, allowed capability dependencies, exact method/path set, Prisma-free transport boundary and absence of the retired broad controller/facade. The approved sequence remains **8A Accounting backend internal capability split -> 8P Payroll -> 8B Accounting Web vertical-contract cleanup -> Phase 9 closeout**. CI #5851 confirmed the direct-import baseline remains Foundation **1** / External **1** / Identity **2** / Runtime **4**, total **8**, public SCC empty; only the final documentation-only head remains subject to the exact-head CI gate, with no new public edge, scanner allowance, Prisma schema/migration, dependency or compatibility path introduced.

Phase 9 Accounting Inbox Expense Review currency/prefill/layout follow-up is **SOURCE COMPLETE / DELIVERY AUTHORIZED / REMOTE CI PENDING / NO MIGRATION** on `fix/accounting-inbox-recognition-prefill-ui` from `origin/dev@bd09197d`. It is Web-adapter-only: the editable currency always initializes to CAD, any recognized total is prefilled as an operator aid regardless of detected currency, foreign/ambiguous recognition is surfaced in red for manual correction, recognized source-amount evidence keeps its detected foreign-currency label, and the payment-allocation editor moves immediately before Memo. Quick-line aggregation still refuses foreign/ambiguous recognition so source-currency line amounts are not silently treated as CAD. The existing Accounting API/persistence/payment-allocation contract is unchanged. Static ownership review therefore introduces no context direction, public edge, direct-import allowance, scanner allowance, SCC or baseline change; the last CI-confirmed baseline remains Foundation **1** / External **1** / Identity **2** / Runtime **4**, total **8**, public SCC empty.
