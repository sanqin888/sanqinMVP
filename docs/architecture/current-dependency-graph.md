...
Phase 9 Slice 8 readiness audit is **COMPLETE / READY FOR SLICE 8A**. Slice **8A-1 Period + Journal capability extraction is MERGED / PR CI GREEN / NO MIGRATION** through PR #2380 / squash `8bc1f77e`; final head `655918d8` passed CI #5834. Slice **8A-2 Expenses / Inbox / Chart / settlement-read capability split is MERGED / PR CI GREEN / NO MIGRATION** through PR #2381 / squash `eaff4c0a`; functional code head `83d4473d` passed CI #5840. Slice **8A-3 generic single-entry Transaction surface contraction is MERGED / PR CI GREEN / NO MIGRATION** through PR #2382 / exact head `9c803efc` / squash `14caf98a`; CI #5843 passed Architecture and all API/Web gates. Generic authenticated `/accounting/tx` CRUD and the retired legacy revenue guard are absent, while Expense remains the only production mutator of `AccountingTransaction`. Slice **8A-4 Financial Reports canonical fact cutover is SOURCE READY FOR REMOTE VALIDATION / NO MIGRATION** on `refactor/phase9-slice8a4-canonical-financial-reports` from `origin/dev@14caf98a`. P&L/dashboard/annual/cashflow/account-balance and report exports now belong to `AccountingFinancialReportsService` and combine canonical Journal facts with only confirmed Expense category splits; `EXPENSE_DOCUMENT` Journal entries are excluded to prevent double counting, recoverable HST is kept outside P&L expense, provider-statement lines contribute only their posted account-class effects, and cashflow uses actual CASH/BANK movements rather than P&L totals or platform-receivable balances. `AccountingService` contracts to the separate Orders `paidAt + totalCents` dimension projection plus Audit-log reads, so the legacy dimension surface cannot become canonical revenue by accident. The monolithic Controller remains intact until 8A-5. The approved sequence remains **8A Accounting backend internal capability split -> 8P Payroll -> 8B Accounting Web vertical-contract cleanup -> Phase 9 closeout**. All capabilities reuse the Accounting-local `ACCOUNTING_DB` composition seam. Current expected direct-import upper bound remains Foundation **1** / External **1** / Identity **2** / Runtime **4**, total **8**, public SCC empty pending 8A-4 PR CI; no new public edge, scanner allowance, Prisma schema/migration, dependency or compatibility path is introduced.
