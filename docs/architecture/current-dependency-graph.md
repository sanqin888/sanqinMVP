...
Phase 9 Expense Review Slice is **LOCAL SOURCE / REVIEW GATE / MIGRATION REQUIRED / NO GRAPH CHANGE EXPECTED** on `refactor/accounting-expense-payment-allocations`. A fresh read-only check found `AccountingExpenseDocument = 0` and `AccountingTransaction = 0`, so the explicitly authorized empty-Accounting contraction removes the old `ExpenseDocument -> single AccountingAccount` relation outright and replaces it with Accounting-owned `AccountingExpensePaymentAllocation` rows; no legacy read/write, backfill or primary-account compatibility is introduced. Category split Transactions remain Expense/category facts and no longer carry payment account ownership; confirmed Expense account-balance outflow is read from payment allocations. Web Expense Review/manual Expense entry consume the same Accounting contract, while the quick-category calculator remains payment-agnostic and the Aggregate action uses its existing explanatory CAD guard. Review selection is now Web-local stable-ID-derived state: `reviewingInboxItemStableId` resolves against the latest Inbox `items`, so refreshed ParseRun/Textract evidence replaces the open review snapshot without any API, polling, database or dependency-boundary change; missing items clear the selection. This changes only owner-local Accounting persistence plus Web adapter code, adds no context direction/public edge/direct-import allowance/scanner-baseline change and does not expand into Expense -> Journal cutover. The expected direct-import baseline therefore remains Foundation **1** / External **1** / Identity **2** / Runtime **6**, total **10**, public SCC empty pending remote CI. `schema.prisma` is changed locally by the payment-allocation portion, but no migration was created or run and the database was not changed; Task C itself has no Prisma effect, and user-generated migration review remains required before production promotion.
