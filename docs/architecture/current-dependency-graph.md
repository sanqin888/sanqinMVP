...
Phase 9 Slice 8 readiness audit is **COMPLETE / READY FOR SLICE 8A**. Slice **8A-1 Period + Journal capability extraction is MERGED / PR CI GREEN / NO MIGRATION** through PR #2380 / squash `8bc1f77e`; final head `655918d8` passed CI #5834. Slice **8A-2 Expenses / Inbox / Chart / settlement-read capability split is MERGED / PR CI GREEN / NO MIGRATION** through PR #2381 / squash `eaff4c0a`; functional code head `83d4473d` passed CI #5840 across Architecture, API/Web lint/build/strict/tests. The approved sequence remains **8A Accounting backend internal capability split -> 8P Payroll -> 8B Accounting Web vertical-contract cleanup -> Phase 9 closeout**. 8A-1 physically moved Store-local timezone/accounting-start/Period locking into `AccountingPeriodService` and double-entry Journal mutation/idempotency/authority/revalidation into `AccountingJournalService`. 8A-2 now deletes `AccountingOperationsService` rather than wrapping it: Chart/category/account ownership moves to `AccountingChartService`, Expense document/split/payment-allocation ownership to `AccountingExpenseService`, Unified Inbox orchestration to `AccountingInboxService`, and settlement/provider-financial reads to `AccountingProviderSettlementQueryService`; provider settlement preview depends directly on the narrow query + Chart capabilities. A minimal `AccountingFinancialReportsService` carries only the unchanged legacy dashboard read so Operations can disappear without growing `AccountingService`; canonical report fact cutover remains 8A-4. 8A-2 left the existing Controller route shapes unchanged; 8A-3 now contracts only the generic Transaction routes while the full Controller vertical split remains 8A-5. All new capabilities reuse the single Accounting-local `ACCOUNTING_DB` composition binding. PR #2381 CI #5836 first reached the architecture gate and showed another monotonic contraction: Accounting -> Runtime direct debt **5 -> 4**. The baseline is lowered rather than widened, and CI #5840 confirmed Foundation **1** / External **1** / Identity **2** / Runtime **4**, total **8**, public SCC empty. Slice **8A-3 generic single-entry Transaction surface contraction is SOURCE READY FOR USER REVIEW / NO MIGRATION** on `refactor/phase9-slice8a3-generic-transaction-contraction` from merged `origin/dev@eaff4c0a`. It removes the authenticated generic `/accounting/tx` CRUD/read surface, `TxBody` / `UpsertTxDto` / generic payload validation and `AccountingService.createTx/listTx/updateTx/deleteTx`; `AccountingTransaction` remains only as Expense split persistence and as a temporary legacy read source for P&L/dashboard/export until 8A-4. The obsolete `assertNoLegacyOrderRevenueAccrual()` runtime guard is removed with its canonical execution calls, while architecture regression now requires generic routes/CRUD/guard to stay absent and limits production `AccountingTransaction` mutation to `AccountingExpenseService`. Read-only production verification still finds zero AccountingTransaction rows and zero retired AUTO_ORDER accrual rows. No public edge, scanner allowance, Prisma schema/migration, dependency or compatibility path is introduced; current expected upper bound remains Foundation **1** / External **1** / Identity **2** / Runtime **4**, total **8**, public SCC empty pending remote CI. Report/export/account-balance canonical-fact semantics remain 8A-4.
