...
2026-09-21 Post-modularization Accounting Document Recognition Reliability Slice A is **LOCAL SOURCE COMPLETE / USER REVIEW PENDING / NO MIGRATION** on `accounting/structured-csv-result-hash`. It only restores the existing Inbox Core successful-ParseRun `resultHash` invariant for structured-expense CSV and ambiguous provider-recognition CSV by reusing Accounting-owned deterministic JSON hashing. CSV parsing/classification, batch fail-closed behavior, Provider API routing, persistence ownership and Journal authority are unchanged. **No dependency-graph or scanner-baseline change:** no new context direction, public edge, direct import, architecture allowance or SCC is introduced; the closed modularization baseline remains unchanged.
