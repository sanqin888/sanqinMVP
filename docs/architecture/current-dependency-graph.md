...
| payments-clover | architecture-foundation 15; commerce-orders-fulfillment 4; identity-customer-benefits 13; messaging-notifications 2; runtime-data-ci-ops 8; store-operations-pos-print 11 |
...
Phase 6 Slice 1C is currently local / review pending. Orders now exposes the unchanged V2 preparation contract through `PAYMENT_ORDER_PREPARATION`, backed by `OrdersService` via `useExisting`, and `PaymentCheckoutAttemptService` consumes that public port instead of deep-importing `orders.service`. This removes exactly one production direct import: the monotonic baseline contracts `payments-clover -> commerce-orders-fulfillment` **5 -> 4**, Payments/Clover total outgoing direct debt **54 -> 53**, and the public SCC baseline remains expected empty. Confirmed-payment finalization, OrdersModule composition imports and the production Web Clover legacy controller remain intentionally outside this slice.
