...
2026-09-22 Accounting **B2-P0 Canonical SALE continuity hardening** is **P0A PRODUCTION VERIFIED / P0B DEPLOYED + FORWARD VERIFICATION PENDING / NO GRAPH CHANGE**. P0A caught up the post-2026-09-13 gap through the existing guarded replay and production read-only verification showed 131 immutable SALE facts / 131 exactly-one canonical SALE Journal anchors / zero missing / zero duplicate for the catch-up window. P0B merged through PR #2459 as `2c4cb834`; CI #6118 passed all Architecture/API/Web gates and production `main` now runs `2c4cb834`. The processor stays inside the existing Accounting -> Orders and Accounting -> Brand/Store public directions: it reads Orders financial facts through `ORDER_FINANCIAL_FACTS_READER`, excludes existing Accounting Journal anchors, writes only through the existing canonical SALE posting service and reads provider coverage inside Accounting to preserve Uber statement authority until `liveOrderFactCutoverAt`. No Orders -> Accounting edge, direct foreign-owner Prisma read, queue/table, schema/migration, package, scanner allowance or public SCC change was introduced. One new forward POS/Web paid Order remains required to verify automatic exactly-one Journal convergence without replay.

2026-09-22 Accounting **B2-A Sales Analytics contracts + attribution policy** is **LOCAL SOURCE IMPLEMENTED / CI PENDING / EXPECTED NO GRAPH CHANGE** on branch `accounting/b2-a-sales-analytics-contracts`. Orders adds a dedicated public `ORDER_SALES_ATTRIBUTION_READER` plus module whose DTO contains only descriptive canonical Order-financial source/order/store identity, occurredAt, channel, primary payment method and evidence; it contains no monetary fields. Direct SALE attribution prefers immutable SALE evidence over explicit `LEGACY_CURRENT_ORDER` fallback. Immutable adjustment/reversal source facts retain their own source identity/channel/time while primary payment attribution is inherited from the original SALE evidence, never from change monetary state. Accounting adds a pure allowed Sales/provider source-fact vocabulary, Journal account-to-Sales-component/sign policy, Journal tender buckets, attribution-quality classification and fail-visible provider coverage states. Accounting may reference the new Orders contract only through `../orders/public-api`; there is no new owner direction, direct Prisma crossing, scanner allowance, SCC, schema/migration, route or UI cutover. The existing Accounting -> Orders public direction therefore remains unchanged in topology.
