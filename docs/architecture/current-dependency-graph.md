...
2026-09-16 Phase 9 Expense Inbox ownership + CAD functional-currency hardening is **SOURCE IMPLEMENTED / LOCAL REVIEW GATE / NO GRAPH CHANGE EXPECTED** on `phase9-expense-inbox-cad-boundary`. Accounting Inbox now owns evidence-backed Expense review through category aggregation and confirmation; the temporary product-item calculator moves from Expenses into the Inbox UI and still persists only category totals. Expense confirmation collapses the former two-step pending-materialization path into one Accounting-owned Serializable transaction that creates the confirmed ExpenseDocument, CAD category Transactions/Audits and confirmed materialization link atomically. Source-document currency/amount stays recognition evidence in the existing parse/extraction JSON while booked Expense/Transaction money is explicitly CAD and non-CAD payment accounts fail closed; this avoids introducing a multi-currency ledger while supporting USD-billed/CAD-settled evidence such as Cloudflare. The Expenses page retains only confirmed-expense visibility plus explicit no-evidence manual CAD entry. No Prisma schema/migration, dependency manifest, Textract runtime, context import, scanner allowance, Journal authority, provider settlement or revenue-posting policy changes; expected baseline remains Foundation **1**, External **1**, Identity **2**, Runtime **6**, total **10**, public SCC empty pending GitHub Actions.
