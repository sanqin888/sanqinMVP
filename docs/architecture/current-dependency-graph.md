...
Slice 8B-B Inbox / Expense / Settlement Web contracts is **LOCAL SOURCE COMPLETE / REVIEW PENDING / NO MIGRATION / NO LOCAL CI CLAIMED** on `refactor/phase9-slice8b-b-inbox-expense-settlement-contracts`. Inbox wire DTOs, shared chart/account shapes, Expense document responses and provider-settlement responses now live under the Accounting Web contract surface; `inbox-model.ts` is UI-only, Expenses no longer carries page-local wire copies, and Settlement production sources no longer depend on the Inbox UI model or the deleted `settlement-model.ts`. This is Web source contraction only: no API response/route behavior, persistence, Journal/Expense/Inbox/provider-settlement semantics, package dependency, context direction or scanner allowance changes. Expected baseline remains Foundation **1** / External **1** / Identity **2** / Runtime **4**, total **8**, public SCC empty.
