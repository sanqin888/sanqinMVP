...
2026-09-21 post-modularization Accounting Document Recognition Slice 3V-A is **SOURCE IMPLEMENTED / LOCAL REVIEW / NO GRAPH CHANGE** on branch `accounting/document-recognition-3v-a` from `origin/dev@4d68379e`. Native-PDF usability classification, persisted routing evidence, Poppler layout verification and the provider-financial v5 fail-closed rule remain entirely inside Accounting; the new routing policy is pinned by the existing Prisma-free Accounting architecture guard. The current whole-PDF Textract fallback is not broadened: only the historical completely blank native-text case remains eligible until separate Slice 3V-B replaces it with bounded page rasterization, and the existing Provider API exclusion is unchanged. No new cross-context import, public capability direction, scanner allowance, compatibility path, schema/migration, package/runtime dependency or public SCC is introduced. The authoritative Accounting direct-import baseline therefore remains Foundation **1** / External **1** / Identity **2** / Runtime **4**, total **8**, with public SCC empty. Phase 9 remains closed.
