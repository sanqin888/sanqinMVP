...
Slice 8B-E Accounting Web contract architecture guard + 8B source closeout is **MERGED / CI GREEN / PRODUCTION DEPLOYED / NO MIGRATION** through PR #2412 / final head `8163a8e0` / squash `11bf2589`; PR CI #5955, post-merge CI #5956 and dev verification CI #5957 all passed. Production `main` now runs `11bf2589`. The last inline Inbox delete response and Reconciliation-local Uber report DTO moved to the Accounting contract surface. The recursive Web guard requires named `/accounting/*` `apiFetch<T>` responses to be imported from `contracts/*`, rejects inline/page-local response DTOs, keeps Inbox/Payroll UI-only modules private to their verticals, keeps deleted mixed-model files absent and pins Audit `operatorActorRef`. Source-closeout counts are page-local named Accounting response ownership **0**, inline response DTOs **0**, Settlement -> Inbox UI-model imports **0**, and Audit legacy Web label **0**. No runtime semantics, persistence, package dependency, context direction or scanner allowance changed. Final baseline remains Foundation **1** / External **1** / Identity **2** / Runtime **4**, total **8**, public SCC empty.

Phase 9 overall is therefore **SOURCE / ARCHITECTURE CLOSED — PRODUCTION CLOSEOUT PENDING** rather than source-incomplete. Final closure still requires the §18.21 production-test-fixture work package and authoritative read-only parity snapshot. The 2258-cent historical exception remains fail-closed, with the current persisted amendment row accurately identified as `RETENDER` carrying `refundCents=2258` / `deltaCents=-2258`, not a currently persisted `FULL_REFUND` type. Payroll 8P-E is **PRODUCTION VERIFIED WITH EXPLICIT DEFERRED EVIDENCE**: real CRA-remittance settlement waits for the next real remittance, posted-run reversal/correction waits for a real case, and production period-close remains source/CI-characterized rather than synthetically forced. Uber July/provider-report completion remains a provider-evidence gate, not architecture debt.
