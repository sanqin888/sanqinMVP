...
2026-09-14 Phase 9 C1-A.1 governance is **DOCUMENTED / FOLLOW-UP IMPLEMENTATION LOCAL / NO GRAPH CHANGE**. The approved direction reuses the existing `payments.pos-card-legacy.v1` rollout meaning for Accounting rather than introducing another CARD route-choice policy: legacy in-store CARD changes may use explicitly labeled Orders-declared settlement authority, while later Terminal-mode CARD changes remain strict on Payments reversal evidence. Historical replay is resolved from owner facts instead of a wall-clock boundary: new Orders SALE facts freeze legacy-vs-Unified execution provenance, while older unprovenanced in-store CARD SALEs use the presence or absence of a canonical Payments CARD SALE fact. The same documentation batch adds a Phase 9 closeout data-hygiene gate requiring all deliberately created test fixtures and their dependent Order/change/payment/Loyalty/Accounting artifacts to be inventoried and resolved before the final authoritative closeout snapshot. This is documentation/governance only: no source import, public edge, scanner allowance, Prisma/schema/migration or runtime behavior changes, so the recorded baseline remains Foundation **2**, External **1**, Identity **2**, Runtime **6**, total **11**, public SCC empty.

2026-09-14 Phase 9 C1-A.1 implementation is **LOCAL / REVIEW PENDING / NOT YET REMOTE**. It adds no Accounting -> POS dependency. Orders adds only an additive owner-fact field, and Accounting continues to consume Orders + Payments public financial facts already present in C1-A. Orders adds an optional immutable `posCardExecutionEvidence` field to `OrderFinancialFactV1` and derives it from the existing persisted in-store CARD payment-breakdown shape, so later cutback/re-enable cannot reinterpret newly created legacy vs Unified CARD SALEs. Accounting resolves CARD settlement authority from explicit Orders execution provenance first; for older facts, a canonical Payments CARD SALE proves Unified/strict handling, while no Payments CARD SALE means the legacy direct-paid route. Legacy/order-declared mode may relax only CARD refund/negative settlement evidence; post-sale CARD collection remains strict. No new public edge, SCC/cycle, scanner allowance, Prisma/schema/migration or rollout configuration is introduced. The direct baseline therefore remains Foundation **2**, External **1**, Identity **2**, Runtime **6**, total **11**; public SCC remains empty subject to authoritative CI verification after remote submission.
