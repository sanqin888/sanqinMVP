...
2026-09-16 Phase 9 Textract Expense Recognition Provider is **SOURCE IMPLEMENTATION IN PROGRESS / NO GRAPH CHANGE EXPECTED** on `phase9-textract-expense-recognition`. AWS Textract `AnalyzeExpense` is introduced only behind the Accounting-local recognition boundary and an opt-in `ACCOUNTING_TEXTRACT_ENABLED` flag. Images use a transient auto-oriented/high-confidence receipt-cropped bounded JPEG for Textract and fall back to existing Tesseract on provider/preparation failure; the immutable SourceArtifact and post-confirm image-retention lifecycle are unchanged. Native-text PDFs stay on local Poppler/provider parsing; only empty-text scanned PDFs may try the synchronous Textract fallback, with no S3/SNS/SQS/async-job path. The mapper treats `SUBTOTAL/TAX/TOTAL` as editable candidates, keeps `AMOUNT_PAID` and provider currency as non-authoritative evidence, fails conflicting dates to review, and stores only aggregate line-item diagnostics rather than SKU accounting rows. The explicitly authorized `@aws-sdk/client-textract` is the only new package; there is no Prisma schema/migration, public API, context import, scanner allowance, Journal authority or posting-policy change. Expected direct-import baseline remains Foundation **1** / External **1** / Identity **2** / Runtime **6**, total **10**, public SCC empty pending dependency lock regeneration and GitHub Actions.
