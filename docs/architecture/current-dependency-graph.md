...
2026-09-08 Phase 6 Slice 4C is **LOCAL / REVIEW PENDING** and also makes **no graph/baseline change**. `CloverProviderConfig` and all in-context consumers now separate the live Web Ecommerce compatibility configuration from Unified merchant/OAuth/Platform and Terminal device/REST Pay configuration. Missing Unified/Terminal variables fail closed with no fallback to the production Web merchant/token/base URL; Platform/OAuth use only `CLOVER_UNIFIED_*`, Terminal uses only its `CLOVER_TERMINAL_*` device/base family plus the temporary pre-4D Terminal token, and production webhook ingress stays on the live Web merchant/auth scope. This is runtime/provider configuration isolation inside Payments/Clover, so `payments-clover -> commerce-orders-fulfillment` remains **1**, Payments/Clover total direct debt remains **41**, and the public SCC/context graph is unchanged. Slice 4D remains the next source step to remove the temporary Terminal static token and converge Terminal on the database-backed Unified merchant OAuth lifecycle.
