...
2026-09-10 Phase 8 Slice 8.6A local source contracts the last two application-layer Foundation logging imports without changing merchant behavior. `uber-merchant-provisioning.service.ts` and `uber-merchant-store-mapping.service.ts` now depend on application-owned `UBER_DIAGNOSTIC_LOG_PORT`; `UberTelemetryService` implements that log-only capability with `AppLogger`, while `diagnosticLog()` performs no `OpsEvent` persistence. The monotonic machine allowance therefore contracts `external-channels -> architecture-foundation` **4 -> 2**, reducing External direct debt **29 -> 27**. The remaining two Foundation direct imports are infrastructure-only `getLogContext()` and `getUploadsAccountingDir()`. External -> Orders remains **0**, Runtime **23**, Identity **2**, and public SCC remains empty. GitHub Actions remains authoritative after user review.
