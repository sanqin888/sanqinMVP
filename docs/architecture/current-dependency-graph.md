...
2026-09-22 Accounting **B2-E Legacy Sales slice cleanup** is **MERGED / CI GREEN / PRODUCTION VERIFIED / NO GRAPH-COUNT CHANGE** through PR #2465 / final head `bff7899a` / squash `3448c271`; CI #6136 passed all required API/Web/Architecture gates. The zero-consumer `/accounting/report/slice` route, `AccountingOrderDimensionSlice`, `AccountingService.dimensionSlice()`, Accounting -> `OrderReportingFactsModule` composition dependency and paid-total-only `readPaidTotalDimensionsForRange()` contract/types are removed. The shared Orders reporting module remains required by `apps/api/src/reports/reports.module.ts` for operational `readMetricsForRange()` / `readItemsForRange()`, so the Accounting -> Orders context direction still exists through canonical financial facts and Sales attribution and the repository graph count remains unchanged. Production deployment of `3448c271` showed Dashboard and canonical `/accounting/report/sales` returning 200, zero post-deploy `/accounting/report/slice` traffic, no Web runtime errors, and balanced read-only canonical Sales Journal populations; the operator reported no visible anomaly. **B2 Canonical Sales Analytics is therefore PRODUCTION VERIFIED / CLOSED.** No schema/migration, package, scanner allowance, SCC, posting or settlement mutation was introduced.

2026-09-22 Accounting **EFA-B1 Expense funding attribution foundation** is **MERGED / CI GREEN / MIGRATION REVIEWED + APPLIED TO PRODUCTION / NO GRAPH CHANGE** through PR #2468 / squash `2250b22d`; PR CI #6145 passed. User-generated migration `20260922183548_accounting_efa_b1_funding_attribution_foundation` was reviewed as additive-only, committed to `dev` as `3e445345`, and applied successfully in production at `2026-09-22 22:51:15Z`. The additive Accounting-owned schema seam introduces Expense funding versioning, nullable split -> funding-account attribution and an account-level Management-Expense inclusion policy while preserving historical Expense v1 allocations/Journals and the generic Journal schema.

2026-09-22 Accounting **EFA-B2 canonical Expense v2 posting** is **MERGED / CI GREEN / NO MIGRATION / NO GRAPH CHANGE** through PR #2469 / `6674ab1cdd6bcba78998698cde69469c40b0b03d`; CI #6150 passed. It adds `CanonicalExpenseFactV2`, persists source authority through split-level funding, groups one Expense into 1..N balanced Journals by funding account with deterministic account-scoped v2 idempotency, validates active CAD operational ASSET funding accounts, keeps v1 authority frozen, and reuses the existing caller-owned Serializable Expense transaction / Journal period-lock / idempotent-replay paths. No new context direction, direct-import allowance, scanner baseline, SCC, package, provider path or Prisma migration is introduced; the later EFA closeout is now production-verified and B3 may proceed to readiness audit.

2026-09-22 Accounting **EFA-C Expense write/UI cutover** is **MERGED / CI GREEN / PRODUCTION DEPLOYED / INCLUDED-ACCOUNT WRITE PATH VERIFIED / NO NEW MIGRATION / NO GRAPH CHANGE** through PR #2470 / final head `483109e6` / squash `42e25b04`; CI #6155 passed API/Web/Architecture gates. Current Expense and Inbox writes stay inside Accounting but move to explicit version-2 split funding; historical v1 allocation reads and completion remain available. The Web adapter moves payment-account selection onto each category row, adds split-level completion for confirmed-unposted v2 Expenses, and records/query dual-reads v1/v2 funding. Accounting Settings exposes create/view/update configuration for `includeFundedExpensesInManagementReports`. Production `expense_nha2w24tprp8s74921k3ge9p` persisted v2 split funding to `account_primary_bank`, proving the active cutover write path. There is no new context direction, direct-import allowance, scanner baseline, SCC, package/provider change, Prisma schema change or JournalLine funding dimension.

2026-09-22 Accounting **EFA-D Management reporting cutover** is **PRODUCTION VERIFIED / CLOSED / NO NEW MIGRATION / NO GRAPH CHANGE** through PR #2471 / final head `ddb01f74` / squash `2d3abc0e`; CI #6158 passed. Reporting remains inside the existing Accounting owner: Dashboard/P&L/category/trend and Management/Boss exports use an explicit Management projection scope that filters only Expense v2 Journal groups by the funding account's management-inclusion policy; raw transaction CSV stays canonical. Account balance/movement and actual cash flow keep their existing independent canonical Journal paths, while input-tax recognition is accumulated before the Management P&L filter. Production verification passed for included Primary Bank, excluded CIBC and mixed CIBC + Primary Bank v2 Expenses. The mixed document created exactly two balanced account-scoped Journals, the excluded CIBC group remained canonical while disappearing from Management Expense, both new verified v2 documents have zero legacy payment-allocation rows, and Management-vs-canonical arithmetic reconciles exactly. **EFA is closed and B3 is ready for readiness audit.** No new context direction, direct-import allowance, scanner baseline, SCC, package/provider path, Prisma schema/migration or JournalLine dimension is introduced.

2026-09-22 Accounting **B3-A canonical Trial Balance core** is **MERGED / CI GREEN / NO MIGRATION / NO GRAPH CHANGE** through PR #2475 / PR head `8a9280a5` / squash `9c92eeda`; PR CI #6171 and merged-head CI #6172 passed. Readiness inspection proves the Journal/CoA is sufficient and B3 remains an Accounting-owned read-model project. B3-A adds Accounting-local contract/policy/service/tests and an AccountingModule provider: it reads canonical Journal lines and account metadata, applies business-timezone/accounting-start range semantics, explicit `OPENING_BALANCE` treatment, account-class normal sides, per-currency whole-ledger scope and period-close metadata, while rejecting any imbalance. It deliberately does not consume P&L Management filtering, Orders/Payments/Loyalty/Uber facts, store filters or provider parser arithmetic. No new context direction, direct-import allowance, scanner baseline, public SCC, package dependency, Prisma schema/migration or posting behavior is introduced. Detailed readiness evidence: `docs/architecture/accounting-b3-trial-balance-readiness.md`.

2026-09-22 Accounting **B3-B Trial Balance HTTP/public contract** is **SOURCE IMPLEMENTED / LOCAL REVIEW PENDING / NO MIGRATION EXPECTED / NO GRAPH CHANGE** on latest `origin/dev@9c92eeda`. The existing authenticated `AccountingReportsController` adds only `GET /accounting/report/trial-balance`, accepts `from?` / `to?` / `currency?`, returns the existing versioned `AccountingTrialBalanceReportV1`, and delegates unchanged query values to `AccountingTrialBalanceService.project`. CAD defaulting, date/clamp policy, Journal reads and all monetary projection remain exclusively in B3-A; transport characterization preserves B3-A BadRequest/Conflict semantics and guards against route-local Prisma, Management projection, ExpenseDocument or Orders arithmetic. No Web/PWA UI, export, Balance Movement, old account-balance contraction, schema/migration, store filter, FX conversion, posting change, new context direction, scanner allowance or SCC is introduced.
