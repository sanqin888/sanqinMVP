...
Phase 9 Slice 6D-A legacy provider-settlement contraction is **SOURCE / LOCAL REVIEW** on `refactor/phase9-6d-a-legacy-settlement-contraction` after the operator executed the real June replay. Production read-only verification found exactly 1 balanced provider-settlement Journal at `294164 / 294164` cents plus 83 balanced historical Uber reversal Journals at `294474 / 294474`, while `PlatformSettlementRecord = 0` and `AccountingTransaction = 0`. The source contraction removes only the obsolete flat `PlatformSettlementRecord` / `SettlementPlatform` schema surface, its CSV importer and `PlatformSettlementRecord -> AccountingTransaction` reconciliation API/service, plus the matching Web normalized-settlement/difference consumer. Uber report artifact visibility remains, and canonical Provider Financial / Journal settlement ownership is unchanged. This is a destructive Prisma contraction and therefore **MIGRATION REQUIRED** before production promotion; the broader `AccountingTransaction.orderId`, `AccountingSourceType.ORDER/UBER/FANTUAN` and no-legacy-revenue guard remain deferred to the later stable-ID/Prisma contract contraction. No context direction or scanner allowance is added; the expected direct-import baseline remains Foundation **1** / External **1** / Identity **2** / Runtime **6**, total **10**, public SCC empty.
