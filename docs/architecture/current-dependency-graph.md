...
Read-only production audit during 4C found a separate existing functional debt: both current `UserAddress.addressStableId` rows use the historical `a...` prefix, while Orders' existing `normalizeStableId()` accepts only `c...` CUID values. That mismatch is documented but intentionally not fixed in this boundary-only slice.
