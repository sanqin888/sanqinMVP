...
2026-09-22 Accounting **B2-P0 Canonical SALE continuity hardening** is **P0A PRODUCTION VERIFIED / P0B LOCAL SOURCE IMPLEMENTED / CI PENDING / EXPECTED NO GRAPH CHANGE**. P0A caught up the post-2026-09-13 gap through the existing guarded replay and production read-only verification now shows 131 immutable SALE facts / 131 exactly-one canonical SALE Journal anchors / zero missing / zero duplicate for the catch-up window. P0B stays inside the existing Accounting -> Orders and Accounting -> Brand/Store public directions: an Accounting-owned processor reads Orders financial facts through `ORDER_FINANCIAL_FACTS_READER`, excludes existing Accounting Journal anchors, writes only through the existing canonical SALE posting service and reads provider coverage inside Accounting to preserve Uber statement authority until `liveOrderFactCutoverAt`. No Orders -> Accounting edge, direct foreign-owner Prisma read, queue/table, schema/migration, package, scanner allowance or public SCC change is introduced by source design; GitHub Architecture/CI remains pending after local review.
