...
| payments-clover | architecture-foundation 15; commerce-orders-fulfillment 4; identity-customer-benefits 13; messaging-notifications 2; runtime-data-ci-ops 8; store-operations-pos-print 5 |
...
Phase 6 Slice 2B is currently in CI remediation. The three Payment/POS controllers now consume the existing `PosDeviceGuard` through `pos/public-api.ts`, and the Unified Payment Core composition module imports the existing `PosDeviceModule` from that owner public surface. An initial attempt to re-export `PosModule` through the same barrel passed architecture/lint/build/strict but failed API tests because eager `PosModule` loading created broad runtime circular initialization (`ZodValidationPipe is not a constructor` / invalid guard decorators). The safe design therefore retains the existing direct `PosModule` Nest composition import rather than manufacturing another facade or making the lightweight POS public barrel load the full module. The monotonic baseline contracts `payments-clover -> store-operations-pos-print` **9 -> 5**, reducing Payments/Clover total outgoing direct debt **51 -> 47**; the four removed edges are the three `PosDeviceGuard` deep imports plus `PosDeviceModule`. `PosCardPaymentFeatureConfig`, the legal `PosModule` composition edge, full-refund `PosOrdersService`, refund policy, confirmed-payment atomicity and Web Clover remain outside further contraction in this slice.
