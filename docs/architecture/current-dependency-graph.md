...
| payments-clover | architecture-foundation 15; commerce-orders-fulfillment 4; identity-customer-benefits 13; messaging-notifications 2; runtime-data-ci-ops 8; store-operations-pos-print 2 |
...
2026-09-08 Phase 6 Slice 2D is a docs-only ownership/cutover decision and therefore makes **no graph or baseline change**: `payments-clover -> store-operations-pos-print` remains **2** and Payments/Clover total remains **44**. `PosCardPaymentFeatureConfig` is classified as temporary `payments.pos-card-legacy.v1` compatibility, not as a lasting POS public policy or Payments capability. It should not be hidden behind a manufactured permanent facade merely to lower the scanner count; it is removed together with the legacy direct-paid CARD path and route-choice branches after POS ↔ Clover Terminal realtime synchronization/recovery, real-device acceptance, settlement proof, production stability and zero legacy invocation satisfy the registered cutover gate. The direct `PosModule` import remains legal Nest composition.
