...
Phase 9 Accounting / Reporting / Analytics Slice 0 readiness audit is complete at `origin/dev@1a69bd7d`; Slices 1-5B are merged/CI-green through PRs #2281/#2283/#2284/#2285/#2288, with Slice 5B squash merge `cbe8ad6f` after CI #5516 passed and its migration now deployed/runtime-smoke-verified on the running VM. Slice 5C readiness merged via PR #2290 / `7419c982`; 5C-A Unified Inbox Core merged via PR #2291 / `913e88aa` after CI #5526 passed. Slice 5C-B acquisition cutover is local-source-complete from that merged base and removes one Accounting -> Runtime direct import by taking Gmail ingestion off direct `PrismaService`; the monotonic source baseline is now Foundation **2**, External **1**, Identity **2**, Runtime **6**, total **11**, pending remote CI confirmation. The sequence remains Unified Inbox/provider financial evidence -> canonical financial facts/revenue posting -> platform settlement/reconciliation, with **2026-06-01** as the common Clover/Uber Eats/Fantuan financial-history boundary.
...
2026-09-12 Phase 9 Slice 5C readiness is **MERGED / NO GRAPH CHANGE** through PR #2290 / squash merge `7419c982`; 5C-A Unified Inbox Core is **MERGED / CI GREEN** through PR #2291 / squash merge `913e88aa` after CI #5526. 5C-A adds Accounting-owned SourceArtifact / ParseRun / InboxItem / TrustedSender / ProviderFinancialDocument / ProviderFinancialLine / provider-coverage persistence, stable-ID materialization linkage and an additive migration without Journal posting. Historical financial coverage remains **2026-06-01 for Clover, Uber Eats and Fantuan**. 5C-B acquisition cutover is now local-source-complete from `913e88aa`: Gmail body/attachments and current Web manual upload use Unified Inbox acquisition; the legacy receipt-upload route is removed under explicit user authorization because Accounting is not yet in operational use and no intake will occur before the new path is ready. Gmail ingestion no longer imports `PrismaService` or the old direct Accounting upload-path helper, so the monotonic baseline contracts Runtime debt **7 -> 6** and Foundation debt **3 -> 2**, total **13 -> 11**; no new public SCC or scanner allowance is expected, pending remote CI confirmation. The 5C-A migration has not yet been applied on the running VM.
