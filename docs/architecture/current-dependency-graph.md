...
2026-09-11 Phase 9 Slice 4 is **SOURCE COMPLETE / REMOTE CI FOLLOW-UP** on `refactor/phase9-slice4-reporting-orders-boundary`. Reporting owns a projection-ready `REPORTING_ORDER_FACTS_QUERY` outbound contract, while Orders owns the public `ORDER_REPORTING_FACTS_READER` that executes existing Order/OrderItem persistence queries and decodes immutable `componentsJson` before facts cross the boundary. `ReportsModule` is a registered composition root for Orders -> Reporting wiring. Homepage now depends on its own `HOMEPAGE_SALES_RANKING_QUERY`; `HomepageContentModule` is the registered composition root that adapts Reporting top-items into that Brand-owned port, removing the public Brand -> Reporting edge that CI #5505 correctly detected in the first remote attempt. The source baseline contracts Accounting/Reporting/Analytics -> Orders **1 -> 0**, Accounting/Reporting/Analytics -> Runtime **9 -> 7**, Brand -> Accounting/Reporting/Analytics **2 -> 0**, and Brand -> Runtime **4 -> 3**; Accounting/Reporting/Analytics direct debt becomes Foundation **3**, External **1**, Identity **2**, Runtime **7**, total **13**. Both cross-owner bindings are composition-only, so `legacyPublicCycleComponents` is expected to remain empty. No Reporting projection tables, migration/backfill/replay worker, Revenue Posting semantics, Uber provider behavior or Web Clover code is introduced.
