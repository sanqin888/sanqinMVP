...
2026-09-21 Accounting **B1-C1 authoritative Expense report cutover** is **PRODUCTION VERIFIED / MERGED / CI GREEN / NO MIGRATION / NO GRAPH CHANGE** through PR #2452 / squash `cdd3b47a`; final head `dc849d20` passed CI #6099. Production runs the Journal-authoritative cutover and post-cutover Expense evidence confirms zero legacy Transaction writes.

2026-09-21 Accounting **B1-C2 Expense compatibility contraction** is **LOCAL SOURCE COMPLETE / USER REVIEW PENDING / MIGRATION REQUIRED / NO GRAPH CHANGE** on branch `accounting/b1c2-final-persistence-contraction` from latest `origin/dev@bba19e2e`. C2A retires the two legacy Expense comparison HTTP diagnostics and their legacy-only implementation/test wiring. C2B removes `AccountingTransaction`, `AccountingSourceType`, their Prisma relations and the dead Accounting-owned source-type contract while strengthening architecture guards around their absence. This changes no context direction or scanner allowance. Production still retains the single CAD 84.69 compatibility row until the separately user-generated destructive migration is reviewed and deployed, so compatibility closure is not yet claimed.
