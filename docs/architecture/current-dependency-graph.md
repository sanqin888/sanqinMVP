...
2026-09-21 Accounting **B1-C1 authoritative Expense report cutover** is **LOCAL SOURCE COMPLETE / USER REVIEW PENDING / NO MIGRATION / NO GRAPH CHANGE** on branch `accounting/b1c1-expense-report-cutover` from `origin/dev@f164be7a`. Authoritative P&L/export, account-balance and cashflow read canonical Journals including `EXPENSE_DOCUMENT`; new Expense writes no longer create/delete legacy `AccountingTransaction` rows; canonical Expense posting depends only on Expense-owned persisted facts rather than the legacy split shadow; split audit identity is explicit; architecture requires zero production Transaction mutation callers. Historical compatibility persistence and both legacy-comparison diagnostic HTTP routes remain for later explicit contraction. No context edge, scanner baseline, public SCC, schema or dependency change is introduced.
