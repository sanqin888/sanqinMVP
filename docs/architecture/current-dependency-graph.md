...
2026-09-14 Phase 9 Slice 5D-C1-A is **LOCAL SOURCE / REVIEW READY / PUBLIC EDGE ADDED, DIRECT BASELINE UNCHANGED BY DESIGN** on `phase9-5d-c1a-accounting-change-shadow-preview`. AccountingModule composes `OrderFinancialChangeFactsModule` and `PaymentFinancialFactsModule` through owner public surfaces, establishing the intended one-way Accounting -> Payments read capability alongside the existing Accounting -> Orders/Loyalty reads. Orders change facts add only a stable-Order batch operation on the already-existing public Accounting -> Orders edge, allowing whole-Order evidence matching before range filtering; Payments adds analogous stable-Order batch reads. `PaymentFinancialFactsModule` itself is narrowed to Prisma-backed financial-fact reader composition rather than importing the full `PaymentsModule`, so this public read edge does not transitively activate Clover provider/execution infrastructure. Production Accounting code still has no Orders/Payments/Loyalty deep import, and source review finds no reverse Payments/Orders/Loyalty -> Accounting import. The direct-import debt baseline is intended to remain Foundation **2**, External **1**, Identity **2**, Runtime **6**, total **11**, and public SCC is intended to remain empty; these scanner results are not claimed locally and await GitHub Actions after review. No Prisma/schema/migration, Journal write, provider execution path, Web Ecommerce cutover or compatibility allowance is added.
