...
2026-09-22 Accounting **B2-C Sales UI cutover** is **LOCAL SOURCE IMPLEMENTED / CI PENDING / EXPECTED NO GRAPH CHANGE** on branch `accounting/b2-c-sales-ui-cutover`. The Web Sales page becomes a consumer of the existing canonical `/accounting/report/sales` contract and stops reading P&L plus `/accounting/report/slice` for its own display. Dashboard deliberately remains a legacy `/accounting/report/slice` consumer, so `AccountingOrderDimensionSlice`, `OrderReportingFactsModule` and the backend route remain in place. This is a Web consumer cutover only: no backend owner edge, schema/migration, package, scanner allowance, SCC or Journal mutation is added.
