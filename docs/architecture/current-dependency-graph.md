...
2026-09-21 Post-Modularization Accounting **B1-A Expense split ownership foundation** is **LOCAL SOURCE COMPLETE / USER REVIEW PENDING / MIGRATION REQUIRED / NO GRAPH CHANGE** on branch `accounting/b1a-expense-split-ownership` from `origin/dev@502647b8`. It adds an Accounting-owned `AccountingExpenseSplit` child model because legacy `AccountingTransaction` EXPENSE rows are currently both report compatibility data and the only durable category/tax split carrier. New/confirmed Expense writes atomically persist both representations under registered compatibility `accounting.expense-split-ownership.v1`; Expense UI/reports remain on the legacy read path, while the existing C0 preview compares normalized category/amount/tax split hashes and blocks `SPLIT_PERSISTENCE_MISMATCH`. Read-only production preflight found 0 confirmed Expenses / 0 active legacy Expense transactions / 0 canonical Expense Journals, so no backfill is currently required. No context edge, direct-import allowance, scanner debt count, public SCC, Journal authority, provider behavior or package dependency changes; the only persistence expansion is the additive split table, which requires a user-generated/reviewed companion migration before promotion.
