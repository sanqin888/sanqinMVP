...
2026-09-16 Phase 9 Textract Expense Recognition Provider is **MERGED / PR CI GREEN / POST-MERGE CI GREEN / DEPLOYED / REAL RECEIPT SMOKE VERIFIED / DATE-ARBITRATION FOLLOW-UP LOCAL / NO GRAPH CHANGE** through PR #2360 / final head `cea260e0` / squash merge `8cd2168f`; PR CI #5767 and post-merge CI #5768 passed. AWS Textract `AnalyzeExpense` remains behind the Accounting-local recognition boundary and opt-in `ACCOUNTING_TEXTRACT_ENABLED`. Images use a transient auto-oriented/high-confidence receipt-cropped bounded JPEG for Textract and fall back to existing Tesseract on provider/preparation failure; the immutable SourceArtifact and post-confirm image-retention lifecycle are unchanged. Native-text PDFs stay on local Poppler/provider parsing; only empty-text scanned PDFs may try the synchronous Textract fallback, with no S3/SNS/SQS/async-job path. Production re-upload of the known Foody `$42.38` receipt confirmed `AWS_TEXTRACT`, applied crop `624x1920`, matched subtotal/tax/total and all three line-item prices, while the provider's incorrect USD suggestion remained non-canonical (`sourceCurrency = null`). The same smoke exposed a date-arbitration defect: provider candidates normalized to both `2026-09-01` and `2026-09-08`, but the generic full-text date matched the wrong provider candidate and bypassed the intended conflict fail-close. The local follow-up on `phase9-textract-date-arbitration` makes any distinct normalized-date conflict authoritative over the generic match and leaves the date null for Inbox review. The explicitly authorized `@aws-sdk/client-textract` remains the only package addition; there is no Prisma schema/migration, public API, context import, scanner allowance, Journal authority or posting-policy change. Direct-import baseline remains Foundation **1** / External **1** / Identity **2** / Runtime **6**, total **10**, public SCC empty.
