...
Phase 6 Slice 1B is local identity-contract work only and does not change this graph. `PreparedPaymentOrderSnapshot` is normalized to V2 before any public port contraction: persisted preparation removes internal `User.id`, `Coupon.id`, `OrderItem.id` and `UserCoupon.id`, makes `storeStableId` explicit, and preserves assigned-coupon selection as a non-identity boolean while Benefits resolves its own records through stable identities. Unified-payment idempotency hashing also excludes internal `selectedUserCouponId` and `checkoutIntentId`, retaining only stable order facts plus the assigned-coupon business intent. V1 read compatibility is intentionally not retained for the coordinated non-business-hours cutover; the production `PaymentCheckoutAttempt` table was re-audited at **0 rows** before implementation. The concrete `PaymentCheckoutAttemptService -> OrdersService.preparePaymentOrder()` edge remains for the next slice, so `payments-clover -> commerce-orders-fulfillment` stays **5**, Payments/Clover total stays **54**, and the public SCC baseline stays empty.
