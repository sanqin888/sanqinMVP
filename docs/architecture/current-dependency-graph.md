...
Phase 9 Accounting / Reporting / Analytics Slice 0 readiness audit is complete at `origin/dev@1a69bd7d`; Slices 1-4 are merged/CI-green through PRs #2281/#2283/#2284/#2285, with Slice 4 merge evidence synchronized on `dev@703a4269`. The 2026-09-12 Slice 5 planning decision revises the next sequence to Double-entry readiness/core -> historical provider-statement ingestion -> canonical financial facts/revenue posting -> platform settlement/reconciliation; this planning-only change does not alter the machine graph or baseline.
...
2026-09-12 Phase 9 Slice 5 planning is **DESIGN-ONLY / NO GRAPH CHANGE**. The approved direction now inserts a minimal double-entry Accounting core before canonical Revenue Posting, treats provider statements as settlement/reconciliation evidence rather than flat income rows, and defines historical Uber Eats/Fantuan backfill from **2026-06-01** at monthly statement level only. The future Accounting Inbox reuses the existing nightly Gmail ingestion boundary with UI-managed trusted senders and content-based document classification; the existing Accounting Web manual file-upload window must become a first-class `MANUAL_UPLOAD` acquisition path through that same Inbox/parser/review pipeline, while provider API remains the third acquisition source. No source, Prisma schema/migration, scanner allowance, dependency manifest, provider-wire behavior or baseline count changes in this planning batch; Accounting/Reporting/Analytics remains Foundation **3**, External **1**, Identity **2**, Runtime **7**, total **13**, with `legacyPublicCycleComponents=[]`.

2026-09-12 Phase 9 Slice 5A readiness is **COMPLETE** and Slice 5B is **LOCAL SOURCE COMPLETE / USER REVIEW** from `origin/dev@3ac08a39`. The reviewed target is an Accounting-owned double-entry Journal + Chart-of-Accounts core only; current Expense/Revenue/statement/report/Web paths remain on their existing contracts until later slices. Current Accounting-owned records are explicitly disposable and require no historical compatibility/backfill solely for preservation. Clover evidence also refines 5C: monthly processing statements are settlement evidence, while the daily Closeout arrives as structured **email body** content (not a production PDF attachment) and is batch-control reconciliation evidence only. Static source review found no new cross-context import, scanner allowance or public cycle; the journal remains inside the existing Accounting persistence boundary. The direct-debt target therefore remains Foundation **3**, External **1**, Identity **2**, Runtime **7**, total **13**; scanner confirmation is pending GitHub Actions after remote-delivery authorization.
