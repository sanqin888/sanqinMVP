...
2026-09-22 Accounting **EFA-D Management reporting cutover** is **MERGED / CI GREEN / PRODUCTION DEPLOYED / PARTIAL VERIFICATION / NO NEW MIGRATION / NO GRAPH CHANGE** through PR #2471 / final head `ddb01f74` / squash `2d3abc0e`; CI #6158 passed. Reporting remains inside the existing Accounting owner: Dashboard/P&L/category/trend and Management/Boss exports use an explicit Management projection scope that filters only Expense v2 Journal groups by the funding account's management-inclusion policy; raw transaction CSV stays canonical. Account balance/movement and actual cash flow keep their existing independent canonical Journal paths, while input-tax recognition is accumulated before the Management P&L filter. The included-account production v2 Journal is balanced; excluded-account and mixed-account Management-vs-canonical verification remain pending. No new context direction, direct-import allowance, scanner baseline, SCC, package/provider path, Prisma schema/migration or JournalLine dimension is introduced.
