...
2026-09-15 Phase 9 Accounting manual-upload filename encoding hardening is **SOURCE IMPLEMENTED / LOCAL REVIEW GATE / NO GRAPH CHANGE EXPECTED** on branch `phase9-accounting-filename-encoding`, based on `dev@376cea32`. Production read-only evidence confirmed browser UTF-8 filenames were being interpreted as Latin-1 at the Multer multipart `originalname` boundary (`三秦肉夹馍` -> `ä¸ç§¦èå¤¹é¦`). The fix is Accounting-local: new `MANUAL_UPLOAD` filenames are conservatively normalized before detection/persistence/parser recognition. Existing mojibake test rows are intentionally not compatibility-normalized and will be permanently deleted through the Upload library; Gmail/Provider API filename semantics are unchanged. No schema/migration, package dependency, context edge, scanner allowance, Journal authority or posting-policy change is introduced; the expected direct-import baseline remains Foundation **1**, External **1**, Identity **2**, Runtime **6**, total **10**, public SCC empty pending authoritative CI rerun.
