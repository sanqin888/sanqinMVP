...
2026-09-14 Phase 9 C1-A.1 governance is **DOCUMENTED / IMPLEMENTATION PENDING / NO GRAPH CHANGE**. The approved direction reuses the existing `payments.pos-card-legacy.v1` rollout meaning for Accounting rather than introducing another CARD route-choice policy: legacy in-store CARD changes may use explicitly labeled Orders-declared settlement authority, while later Terminal-mode CARD changes remain strict on Payments reversal evidence. Historical replay must use immutable change `occurredAt` against a recorded compatibility time boundary. The same documentation batch adds a Phase 9 closeout data-hygiene gate requiring all deliberately created test fixtures and their dependent Order/change/payment/Loyalty/Accounting artifacts to be inventoried and resolved before the final authoritative closeout snapshot. This is documentation/governance only: no source import, public edge, scanner allowance, Prisma/schema/migration or runtime behavior changes, so the recorded baseline remains Foundation **2**, External **1**, Identity **2**, Runtime **6**, total **11**, public SCC empty.
