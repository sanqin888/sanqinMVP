...
2026-09-22 Accounting **EFA-B1 Expense funding attribution foundation** is **MERGED / CI GREEN / MIGRATION REVIEWED + COMMITTED TO DEV / PRODUCTION APPLICATION PENDING / NO GRAPH CHANGE** through PR #2468 / squash `2250b22d`; PR CI #6145 passed. User-generated migration `20260922183548_accounting_efa_b1_funding_attribution_foundation` is reviewed as additive-only and committed to `dev` as `3e445345`. The additive Accounting-owned schema seam introduces Expense funding versioning, nullable split -> funding-account attribution and an account-level Management-Expense inclusion policy while preserving historical Expense v1 allocations/Journals and the generic Journal schema. Production migration application remains a later gate.
...
2026-09-22 Accounting **EFA-C Expense write/UI cutover** is **MERGED / CI GREEN / NO NEW MIGRATION / NO GRAPH CHANGE** through PR #2470 / final head `483109e6` / squash `42e25b04`; CI #6155 passed API/Web/Architecture gates. Current Expense and Inbox writes stay inside Accounting but move to explicit version-2 split funding; historical v1 allocation reads and completion remain available. The Web adapter moves payment-account selection onto each category row, adds split-level completion for confirmed-unposted v2 Expenses, and records/query dual-reads v1/v2 funding. Accounting Settings exposes create/view/update configuration for `includeFundedExpensesInManagementReports`. There is no new context direction, direct-import allowance, scanner baseline, SCC, package/provider change, Prisma schema change or JournalLine funding dimension. Production deployment remains gated on first applying the already-reviewed B1 additive migration and reinstalling the single-user Accounting PWA.

2026-09-22 Accounting **EFA-D Management reporting cutover** is **LOCAL SOURCE IMPLEMENTED / REVIEW PENDING / NO NEW MIGRATION / NO GRAPH CHANGE** on `accounting/efa-d-management-reporting-cutover` from `origin/dev@42e25b04`. Reporting remains inside the existing Accounting owner: Dashboard/P&L/category/trend and Management/Boss exports use an explicit Management projection scope that filters only Expense v2 Journal groups by the funding account's management-inclusion policy; raw transaction CSV stays canonical. Account balance/movement and actual cash flow keep their existing independent canonical Journal paths, while input-tax recognition is accumulated before the Management P&L filter. No new context direction, direct-import allowance, scanner baseline, SCC, package/provider path, Prisma schema/migration or JournalLine dimension is introduced.
