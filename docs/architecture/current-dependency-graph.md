...
Slice 4B is remote on PR #2203 from `refactor/phase5-slice4b-catalog-persistence-boundary` against `origin/dev@6e2da654`. Initial CI #5230 exposed only lint issues; rerun CI #5231 then passed Architecture/lint/build/strict before API Jest exposed one missed `tx.menuItem.findMany` hidden-item read inside `createInternal()`. The current follow-up moves that final Catalog read through the stable-ID-only `CATALOG_ORDER_FACTS_READER` before the Orders transaction and strengthens the scanner to reject any `.menuItem.` delegate in the protected Orders consumers. Catalog exposes the same public capability for hidden-item facts, immutable OrderItem materialization facts and current label/packaging configuration. Orders retains the Web-vs-POS hidden-item policy, immutable snapshot assembly and physical label-plan decisions; only persistence ownership moves behind Catalog.
