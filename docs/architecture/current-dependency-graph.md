...
2026-09-08 Phase 6 post-4A planning records **no additional graph/baseline change**. Slice 4B is limited to internal Clover Platform gateway ownership cleanup and removal of unused Payments module exports; Slice 4C then separates live Web Ecommerce configuration from Unified merchant/OAuth/Platform and Terminal device configuration; Slice 4D converges Platform/Terminal credential consumption on the database-backed Unified merchant OAuth lifecycle. These planned changes must not create new bounded-context edges: Platform/Terminal concrete infrastructure remains Payments-owned and unavailable to orchestration/POS/Orders. Sandbox/device verification follows 4D before the protected Web Unified Payment readiness/cutover work.
