...
Phase 9 Accounting / Reporting / Analytics Slice 0 readiness audit is complete at `origin/dev@1a69bd7d`; Slices 1-5B are merged/CI-green through PRs #2281/#2283/#2284/#2285/#2288, with Slice 5B squash merge `cbe8ad6f` after CI #5516 passed and its migration now deployed/runtime-smoke-verified on the running VM. Slice 5C readiness merged via PR #2290 / `7419c982`; 5C-A Unified Inbox Core merged via PR #2291 / `913e88aa` after CI #5526 passed; 5C-B Acquisition Cutover merged via PR #2292 / `6f1093a8` after CI #5530 confirmed the monotonic baseline Foundation **2**, External **1**, Identity **2**, Runtime **6**, total **11** with no public SCC; 5C-C Provider Financial Parsing + History merged via PR #2293 / `ce399a37` after final head `592bfdc0` passed CI #5535. Slice 5D readiness is complete, 5D-A Canonical Financial Facts Boundary is **MERGED / CI GREEN** through PR #2296 / `7f35878f`, and 5D-B0 Revenue Posting Boundary Hardening is **MERGED / CI GREEN** through PR #2297 / `124cd76c` after final head `064e098e` passed CI #5549. The current 5D-B1A Store Balance Liability CoA is **LOCAL SOURCE IMPLEMENTATION / REVIEW PENDING** from `origin/dev@fd32edcf`: it adds only the stable default account `account_store_balance_liability` (`LIABILITY`, CAD, active) plus an additive data-only Prisma migration and cumulative CoA seed characterization. `AccountingAccount` already supports this class/type shape, so no structural Prisma schema change is needed; the migration writes no Journal/opening balance/history. No new context direction or public API is introduced, and the expected Accounting baseline remains Foundation **2**, External **1**, Identity **2**, Runtime **6**, total **11** with no public SCC. The confirmed posting policies remain Points -> Sales Discount, Daily Special -> nominal/original Sales Revenue minus a separate Sales Discount, Store Balance -> liability, WECHAT_ALIPAY -> Cash, and legacy undated refund/amendment exceptions -> 5D-C. The sequence remains Unified Inbox/provider financial evidence -> canonical financial facts/revenue posting -> platform settlement/reconciliation. Clover/Uber Eats/Fantuan formal provider-financial history remains **2026-06-01**; the existing UI-managed `accountingStartDate` is also the required future POS/Web financial-statistics lower bound and will be set to **2026-06-01** by the user before canonical posting.
...
2026-09-12 Phase 9 Slice 5D-B1A is **LOCAL SOURCE IMPLEMENTATION / REVIEW PENDING / NO GRAPH CHANGE EXPECTED** from `origin/dev@fd32edcf`. The user explicitly authorized migration work. The existing `AccountingAccount` schema already supports nullable operational account type plus `LIABILITY`, so B1A does not alter `schema.prisma`; it extends the Accounting-owned default CoA with `account_store_balance_liability` / `储值余额负债` and adds one additive data-only Prisma migration. The migration is stable-ID idempotent, defaults the account to active CAD liability, and creates no opening Journal or historical balance because the cleaned Store Balance opening/post-cutover principal baseline is zero. CoA architecture characterization now validates cumulative seed migrations rather than requiring every later default account to appear in the already-applied 5B migration. No runtime/public dependency is introduced; Accounting remains Foundation **2**, External **1**, Identity **2**, Runtime **6**, total **11**, public SCC expected empty. Remote CI and production migration application are not claimed before user review.
