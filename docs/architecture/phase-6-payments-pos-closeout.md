...
Current implementation base: `origin/dev@fcc0b7c1`  
Current status: **MODULARIZATION SOURCE / ARCHITECTURE CLOSEOUT COMPLETE; TERMINAL DEVICE ACCEPTANCE + WEB CUTOVER DEFERRED**
...
12. **Unified Clover OAuth credential convergence — COMPLETED in Slice 4D** — Terminal no longer reads or accepts a static `CLOVER_TERMINAL_OAUTH_TOKEN`; it injects the existing `CloverMerchantAccessTokenService`, resolves credentials for `CLOVER_UNIFIED_MERCHANT_ID` at request time, force-refreshes once after HTTP 401, and distinguishes pre-send credential failure from post-send network uncertainty. PR #2245 final head `c665b469` passed PR CI #5361, squash-merged as `1cc4a829`, and post-merge CI #5362 passed API and Web. Platform v3 and Terminal REST Pay therefore converge on the same database-backed authorization/refresh lifecycle without changing production Web Ecommerce execution or public Payments/POS contracts.
13. **Test Merchant / device sandbox bring-up and POS Terminal verification — PARTIALLY VERIFIED** — the production SanQ runtime now has an `ACTIVE` Test Merchant binding for `4750_Yonge_Street`; OAuth callback, database credential encryption/decryption, Platform v3 merchant/payment reads, required read permissions, automatic access/refresh-token rotation, and post-restart credential recovery are actively verified while `POS_CLOVER_TERMINAL_PAYMENT_ENABLED` remains disabled. Cloud Pay Display/device availability, controlled sandbox Sale/reconciliation/refund/recovery and full POS Terminal acceptance remain pending because no Clover Dev Kit is currently available. Full POS E2E against the production SanQ runtime must still be deliberately scheduled because SanQ-side Payment/Order facts write to the production database even when Clover is sandbox.
14. **Web Clover cutover readiness audit — COMPLETED / CONDITIONAL PASS FOR SHADOW READ / NO-GO FOR CUTOVER** — the 2026-09-09 read-only audit confirmed that Ecommerce `POST /v1/charges` must remain the protected Web execution API while Platform REST v3 becomes the eventual canonical payment/refund truth. Existing v3 payment reads already validate provider/external IDs, amount, currency, `additionalCharges`, `CREDIT_SURCHARGE`, charged total, refunds and card facts, but `WEB_ECOMMERCE` status/reconciliation still reads Ecommerce v1 today. This cannot be replaced transparently: a successful v1 charge that is not yet visible in Platform v3 must become `UNKNOWN/RECONCILING` without a second charge and without creating a paid Order until v3 confirms canonical success. The Unified Payment data model is channel-neutral enough for Web, but the current Orders preparation/finalization implementation still explicitly accepts only `in_store`; `CheckoutIntent` also remains an active Web session/contact/locale/delivery/3DS context source and therefore cannot be deleted as part of an initial payment-truth migration. Read-only production evidence at audit time found 11 `CheckoutIntent` rows (5 pending / 3 failed / 3 completed), 6 with Clover payment/external IDs, zero recorded surcharge metadata, 41 Web CARD Orders with zero persisted positive `creditCardSurchargeCents`, and one historical refunded Web CARD Order with no linked CheckoutIntent. The active Unified authorization is still the Test Merchant, so production Web payment IDs cannot be meaningfully shadow-compared through that merchant binding.
15. **Web Unified Payment Core migration / legacy cleanup — DEFERRED BY OPERATOR DECISION** — no Web payment source change, shadow production query, payment lifecycle migration, refund migration or legacy cleanup will begin now. Resume only after the Test App/device acceptance work is complete and the app is installed/authorized against the operating production Clover merchant, at which point a fresh readiness audit must confirm production Platform v3 read scope/correlation before any shadow comparison or authority cutover. The intended first future migration slice remains a read-only v3 canonical shadow for live Web payments; it must not alter `/v1/charges` execution, Order creation, front-end contracts or refund behavior until parity evidence and the dedicated cutover gate pass.

### 2026-09-09 Web Clover readiness findings and freeze boundary

The audit separates **execution** from **canonical truth** rather than treating Clover API version numbers as a replacement ladder:

```text
Web CARD / Apple Pay / Google Pay
  -> Ecommerce POST /v1/charges        # protected execution path
  -> paymentId / externalPaymentId
  -> Platform REST v3 payment read     # future canonical truth
  -> UNKNOWN/RECONCILING when v3 is not yet authoritative/visible
  -> confirmed canonical success
  -> Unified finalization
```

The future v3 authority is especially important for surcharge and refund facts. Ecommerce v1 status mapping currently falls back to `chargedTotalCents = amount` when no explicit total is exposed, while the existing Platform gateway expands `additionalCharges` and counts only `CREDIT_SURCHARGE` as SanQ card surcharge. Phase G must therefore stop treating a v1-derived or fixed-rate surcharge estimate as ledger truth, but it must do so only after production-merchant parity evidence exists.

Until the production Clover merchant is the active Unified authorization, the guarded freeze is explicit: do not modify Web `/v1/charges` execution, CARD/Apple Pay/Google Pay tokenization, 3DS/session/pricing-token/contact-verification behavior, Web paid-Order creation, Web external-payment refund handling, production webhook merchant scope, or persisted Web payment/surcharge facts. Documentation/read-only audits and non-payment modularization may continue.
...
Phase 6 source/architecture closeout requires public SCC to remain empty, no new bounded-context cycle, meaningful owner-leakage contraction, stable-ID-only prepared-payment boundary, an explicit safe decision for the confirmed-payment transaction seam, clear Clover infrastructure ownership, scanner/tests preventing regression, and a documented plan for the protected Web Clover legacy seam. Those source/architecture criteria are now satisfied. This does **not** claim the deferred Clover provider work is `PRODUCTION VERIFIED / CLOSED`: Terminal device financial acceptance remains governed by `payments.pos-card-legacy.v1`, and the protected Web cutover remains governed by `payments.web-checkout-v1.v1`. If either cutover is later executed, its compatibility exit criteria, settlement/production evidence and legacy cleanup remain mandatory before that payment migration can be closed.
...
- Do **not** add `POS_CARD_PAYMENT_FEATURE_POLICY`, re-export the concrete config through `pos/public-api.ts`, or move the rollout flag/config into Payments merely to remove a direct-import count. Phase 9 C1-A.1 interprets legacy-vs-Unified Accounting behavior from Orders/Payments financial facts and does not expose the POS rollout config as a public Accounting policy.
...
Status: **MERGED / CI GREEN; DEPLOYED; NON-DEVICE CLOVER SANDBOX VERIFIED**
...
- The pre-implementation read-only production audit found one `CloverMerchantAuthorization` in `PENDING_BINDING`, zero `ACTIVE` authorizations, and zero `PaymentTransaction` / `PaymentCheckoutAttempt` rows. Therefore merging/deploying 4D by itself could not make the Terminal route executable; it remained fail closed until deliberate Unified OAuth binding and later rollout steps.
...
### Delivery and non-device sandbox verification

- PR #2245 final head `c665b469` passed PR CI #5361, squash-merged to `dev` as `1cc4a829`, and post-merge CI #5362 passed both API and Web. The merged source was then deployed to the production SanQ runtime with Terminal routing still disabled.
- Unified sandbox OAuth completed against the configured Test Merchant/Test App and persisted an `ACTIVE` authorization bound to `4750_Yonge_Street`. The verified sandbox app permission set is Merchant READ + Payments READ + Orders READ with Ecommerce enabled; no Clover write permission was added. Orders READ is required by the current canonical payment read because `PAYMENT_EXPAND` includes `order` and mapping may fall back to `order.currency`.
- The first canonical Platform v3 query before Orders READ was added reached Clover with the stored credential but returned HTTP 403. After updating the Test App permission, reinstalling it on the Test Merchant and reauthorizing, the same read returned HTTP 200; a deliberately nonexistent `externalPaymentId` mapped to `CLOVER_PLATFORM_PAYMENT_NOT_FOUND`, proving the database-backed access token could be decrypted and used by the canonical Platform gateway without financial side effects.
- Natural access-token expiry then exercised the normal refresh path: `tokenVersion` advanced `2 -> 3`, `refreshedAt` was written, both access/refresh expiry values rotated forward, and the same Platform v3 query continued to return HTTP 200. Recreating the API process afterwards preserved `tokenVersion=3` and successfully reused the rotated encrypted credential from PostgreSQL, proving key-ring reload plus post-restart decrypt/reuse.
- The explicit HTTP 401 -> force-refresh -> one retry branch is not intentionally induced against the valid deployed credential; focused automated coverage remains the evidence for that exceptional recovery path. Cloud Pay Display/device availability and all Terminal financial operations remain unverified because no Clover Dev Kit is currently available.
...
1. **VERIFIED** — keep Terminal rollout disabled and complete Test App Preview -> OAuth -> `ACTIVE` store binding for `4750_Yonge_Street`.
2. **VERIFIED (non-device)** — verify Platform v3 merchant/payment reads with the stored OAuth credential, including the current Orders READ requirement, natural token refresh/rotation and reuse of the rotated encrypted credential after API recreation.
3. **PENDING DEV KIT** — verify Cloud Pay Display/Test device availability using the sandbox Terminal base, device ID and RAID.
4. **PENDING DEV KIT** — run a controlled sandbox Sale and confirm the same payment through Platform v3 canonical read/reconciliation before allowing Order finalization.
5. **PENDING DEV KIT** — exercise timeout/UNKNOWN/recovery, cancel, void/refund and duplicate/idempotency scenarios before the complete POS acceptance matrix.
6. **PENDING DEV KIT / EXPLICIT TEST WINDOW** — only in a deliberate non-business/test window enable the POS Terminal route for full E2E. Using Clover sandbox does **not** isolate SanQ persistence: a full POS flow against the production runtime can create production-database `PaymentTransaction`, `PaymentCheckoutAttempt`, Order, print and reporting facts. If test-data isolation is required, use a separately approved staging runtime/database instead of pretending provider sandbox also isolates SanQ data.
7. **READINESS AUDIT COMPLETE / CUTOVER DEFERRED** — the read-only Web Clover audit completed on 2026-09-09 without switching traffic or relaxing the guarded `/v1/charges` path. By operator decision, Phase G migration/activation and production v3 shadow reads remain deferred until Test App/device acceptance is complete and Unified authorization has moved to the operating production Clover merchant, followed by a fresh production-merchant correlation audit.
...
Slice 3A is merged and CI-green through PR #2239 / CI #5343, final head `782da646`, squash merge `239d8f74`. Its authorized contraction migration `20260908155000_contract_payment_checkout_order_db_ids` has since been applied successfully in production (finished at 2026-09-09 02:08:48 UTC); no Terminal traffic cutover is implied by that migration deployment.

Slice 3B is merged and CI-green through PR #2240 / CI #5346, final head `6eb4b38c`, squash merge `893fde49`. Slice 4A merged through PR #2241 after final head `d38dce42` passed PR CI #5348; squash merge `00768897`. The post-4A sandbox/config sequencing decision merged through PR #2242 after final head `deb2e8c6` passed CI #5350; squash merge `e12217a3`. Slice 4B merged through PR #2243 as `aa765f9d`; merged-dev CI #5354 passed API and Web. Slice 4C merged through PR #2244 after final head `31efc862` passed PR CI #5358; squash merge `bc96c706`, and merged-dev CI #5359 also passed API and Web. Slice 4D merged through PR #2245 after final head `c665b469` passed PR CI #5361; squash merge `1cc4a829`, and post-merge CI #5362 passed API and Web. The merged 4D runtime is deployed, and the non-device Unified Clover sandbox credential/Platform verification described above is complete; device/Terminal financial acceptance remains pending.
...
Slice 4D delivery and the non-device Test Merchant bring-up are complete with `POS_CLOVER_TERMINAL_PAYMENT_ENABLED` still disabled. The hardware-dependent remainder is now explicit: Cloud Pay Display/device availability, controlled sandbox Sale/reconciliation, timeout/UNKNOWN recovery, cancel/void/refund, idempotency and the full POS Terminal acceptance matrix all wait for a Clover Dev Kit and an explicit test window. Clover sandbox still does not isolate SanQ persistence, so full POS E2E against the production runtime requires deliberate scheduling or a separately approved staging runtime/database.

The **read-only protected Web Clover cutover readiness audit is now complete**. Its result is `CONDITIONAL PASS FOR FUTURE SHADOW READ / NO-GO FOR CUTOVER`: Platform v3 is the correct future canonical truth, but production Web execution/status/finalization cannot be switched safely by a transport-only change, and the active Unified authorization still targets the Test Merchant rather than the operating production Clover merchant. By operator decision, all Web Unified Payment migration, production shadow reads, refund migration and legacy cleanup are now deferred until Test App/device acceptance is complete and the app is installed/authorized against the production merchant.

This is the Phase 6 **modularization source/architecture closeout point**. Source debt contracted from **57 -> 41**, the remaining direct Payments -> Orders seam is the explicitly protected Web compatibility path, the remaining Payments -> POS pair is legal composition plus registered POS CARD cutover compatibility, and public SCC remains empty. The repository may proceed to non-payment bounded-context work without reopening these payment seams. Phase 6 is deliberately **not** labeled `PRODUCTION VERIFIED / CLOSED`; the two payment compatibility records remain the authoritative continuation gates for later Terminal and Web cutover work.
