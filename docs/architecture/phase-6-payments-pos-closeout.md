...
Current implementation base: `origin/dev@1cc4a829`  
Current status: **SLICE 4D MERGED / CI GREEN; NON-DEVICE CLOVER SANDBOX VERIFIED; DEVICE VERIFICATION PENDING**
...
7. While items 3-6 are hardware-blocked, a **read-only Web Clover cutover readiness audit may proceed** so long as it does not switch Web traffic or relax the guarded `/v1/charges` path. Phase G migration/activation remains blocked until the dedicated Web readiness gate and the remaining Terminal/device acceptance evidence are both satisfied.
