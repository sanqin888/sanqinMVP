...
14. **Web Clover cutover readiness audit** — a read-only readiness audit may proceed while device work is blocked, because it does not activate Unified Web traffic. It must still preserve the guarded production Web `/v1/charges` path and may not authorize Phase G migration or legacy cleanup; actual Web cutover remains gated on the dedicated readiness PASS plus the remaining Terminal/device acceptance evidence.
