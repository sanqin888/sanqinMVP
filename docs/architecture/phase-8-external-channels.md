...
Status: **PHASE 8 SOURCE / ARCHITECTURE CLOSED — CONSOLIDATED TEST STORE ACTIVE VERIFICATION PASSED FOR ALL EXERCISABLE CAPABILITIES; SLICE 8.4 FINANCIAL-REPORT LIVE REPLAY REMAINS CODE READY / LIVE TEST BLOCKED BY UBER CAPABILITY**  
...
Root cause: Slice 8.2A moved draft/read schedule consumers behind `UBER_BUSINESS_SCHEDULE_QUERY_PORT`, but `PublishUberMenuUseCase` still generated a private seven-day `00:00–23:59` availability array. The forward-fix removes that hard-coded path. `UberMenuSnapshotPrismaAdapter` now consumes the same application-owned business-schedule port already backed by Store-owned `STORE_SCHEDULE_READER`, converts the real recurring hours through `toUberServiceAvailability`, and includes that result in `UberMenuPublishSnapshot`; Dry Run and formal Publish both consume `snapshot.serviceAvailability`. Existing timezone/tax validation and error codes are preserved, and empty/invalid schedules still fail closed before provider upload. Focused regression/architecture coverage pins a real `08:00–22:30` example and forbids the `23:59` hard-coded publish fallback from returning. PR `#2277` final head `300c4367` passed PR CI `#5483`, squash-merged to `dev` as `574c5beb`, and merged-head CI `#5484` passed API + Web again.

This forward-fix makes **no dependency-graph or machine-baseline change**: External -> Foundation remains **2**, Orders **0**, Identity **2**, Runtime **23**, total direct debt **27**, and public SCC remains empty. It introduces no Prisma/schema/migration, provider wire schema/API-path, OAuth, payment/Clover or compatibility change; the intentional provider-visible behavior correction is limited to `service_availability` values now reflecting the configured Store schedule instead of the erroneous 24-hour fallback. Post-deploy verification confirmed Dry Run and formal Publish both carried the canonical Store schedule (Sunday `12:00–21:00`, Monday-Friday `08:00–22:30`, Saturday `12:00–22:00`), tax rate `13`, and Uber Store discovery returned `America/Toronto`, matching Store configuration.

#### Slice 8.6B — Documentation + consolidated closeout evidence

8.6B is documentation-only and records the final merged/source state; it introduces no source, schema, migration, dependency, provider-wire or machine-baseline change. The final source tail is PR `#2275` / `f8896493` for 8.6A plus PR `#2277` / `574c5beb` for the menu-publish schedule forward-fix. PR `#2277` CI `#5483` and merged-head CI `#5484` passed the architecture gate plus complete API/Web validation.

Consolidated 2026-09-11 Test Store active verification from the final deployed state passed every capability that the current Sandbox/Test Store can exercise:

- Dry Run and formal menu Publish used the canonical Store schedule, `13%` tax and `America/Toronto`; the formal publish version succeeded rather than publishing the former seven-day 24-hour fallback.
- Item and option restore-source-price operations both reached their dedicated endpoints and persisted `ubereats_menu_price_restored` audit evidence; the observed source item price was `749` cents and the observed option source delta was `0`.
- Scheduled order `00fb7f7f-5946-4b08-829a-e81f0302175a` ACCEPT succeeded with Uber HTTP `200`; the later scheduled notification reused the same durable ACCEPT action (`attemptCount=2`) without a second canonical `order.accepted`. Scheduled activation, `order.prep_started` and initial print handoff each occurred once, READY succeeded with HTTP `200`, and the canonical order completed.
- Immediate order `cf1a6de2-56fb-425f-a6b1-6cce829a2f92` ACCEPT and READY each succeeded with Uber HTTP `200` / `attemptCount=1`; canonical acceptance, preparation and initial print handoff each occurred once and the order completed.
- Manual DENY for `5bf2a7f0-3b5c-49b0-bdad-eb643859ac19` succeeded with Uber HTTP `200`; the canonical order converged to its terminal refunded/cancelled state when provider failure evidence arrived, without a provider-success/local-transition split.
- 8.6A prep-time smoke updated the existing `1080`-second value with provider HTTP `200`, emitted the expected diagnostic log, and did not create a diagnostic-only `OpsEvent`.
- Post-verification read-only checks found no active `UberOrderAction` or `UberWebhookInbox` leases and no stuck current processing rows. One best-effort `webhookInbox` wake request exceeded its `500ms` timeout during the scheduled-order test, but the durable inbox row was processed once and the worker began normal processing immediately afterward; it is recorded as a non-blocking runtime observation rather than a lost-work condition.

Slice 8.4 financial-report replay remains explicitly **`CODE READY / LIVE TEST BLOCKED BY UBER CAPABILITY`**: the current environment has no live `eats.report.success` evidence and `UberFinancialReport` remains empty. This is not represented as a live pass. Structured Allergy payload capability, `orders.customer_order_edit`, Production Client scopes/whitelisting/webhooks, Production Store provisioning, the production pilot, and the eventual complete Test Store data cleanup remain separate provider/cutover gates outside this source/architecture closeout.

Final Phase 8 architecture state is `external-channels -> architecture-foundation = 2`, `external-channels -> commerce-orders-fulfillment = 0`, `external-channels -> identity-customer-benefits = 2`, `external-channels -> runtime-data-ci-ops = 23`, for **27** remaining External direct-import debt statements; `legacyPublicCycleComponents=[]` and the public SCC baseline remains empty. The two Foundation statements are intentional infrastructure-only seams (`getLogContext()` and accounting uploads layout), while the two Identity statements are the retained Auth HTTP/session constant seam and legal `AuthModule` composition seam. Phase 8 is therefore closed for its approved source/architecture scope with all currently exercisable Test Store closeout verification passed; this does not claim Uber Production Verification, Production Store cutover or pilot completion.
