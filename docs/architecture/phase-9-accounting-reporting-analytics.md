...
Status: **SLICE 0 READINESS AUDIT COMPLETE — SLICE 1/2/3/4/5B/5C/5D-A/5D-B0 MERGED / CI GREEN — SLICE 5D-B1A DEPLOYED / DB VERIFIED — SLICE 5D-B1B LOCAL SOURCE IMPLEMENTATION / REVIEW PENDING**  
...
B1B deliberately **does not expose the new post method through AccountingController or scheduler yet**. The legacy `automation/order-accrual` route remains the only active revenue-posting entry point until the next replay/parity/cutover slice, so deploying B1B alone cannot dual-write Revenue or start historical backfill. Focused pure-policy/service tests cover CASH, WECHAT_ALIPAY, CARD+surcharge, Uber receivable, mixed/pure Store Balance, points-only sales, source/pricing/start-date blocks and the stable system actor. Architecture coverage pins public-only owner imports; the B1B implementation itself deliberately adds no HTTP posting route or scheduler caller. No Prisma schema/migration, provider/Web Clover execution behavior, Reporting read model or old `AccountingTransaction` path is changed. Direct-import debt is expected to remain Foundation **2**, External **1**, Identity **2**, Runtime **6**, total **11**; B1B intentionally adds the public Accounting -> Orders financial-fact capability edge and reuses the already-public Identity/Loyalty direction without introducing a public SCC. Remote CI is not claimed before user review.

The next step after B1B review/CI/merge is the controlled replay/parity slice: add range preview/backfill orchestration from the configured Toronto start date, define the explicit handling workflow for the 101 `MANUAL_OVERRIDE` Daily-Special rows and five post-sale-mutation exceptions, compare eligible results, then retire/demote the provisional `Order.totalCents -> AccountingTransaction` accrual path only after parity. Historical refund/reversal posting remains 5D-C.
