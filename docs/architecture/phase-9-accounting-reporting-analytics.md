...
Slice 5D-B2B is **MERGED / CI GREEN / NO CANONICAL REPLAY EXECUTED** through PR #2313; final head `6171aa96` passed CI #5595 before squash merge `72395609` into `dev`. The first two PR runs exposed formatting-only API lint failures; the final reviewed head passed Architecture, API lint/build/strict/Jest, shared strict and Web lint/build/strict/tests. No deployment or production Journal replay is claimed by this source merge.
...
**Production gate remains explicit.** The post-B2A.1 preview body has been user-reviewed and the expected 1210 replay-ready records were confirmed. Before the first canonical historical replay, execution must use the exact reviewed fresh `planHash` and will recompute the range, enforce zero-cent parity, balanced debit/credit totals, mutation-only blocked rows with exact stable-ID acknowledgement, and zero retired `AUTO_ORDER*` Revenue rows. Immediately before execution, verify the legacy and Journal tables still contain no unexpected Revenue writes. The first replay must then be followed by read-only Journal counts/amount/source-fact verification and a second same-plan replay to demonstrate idempotent convergence. `accounting.order-revenue-journal-cutover.v1` may close only after that evidence exists and the removed legacy endpoint is confirmed unavailable.
