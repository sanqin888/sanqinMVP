...
State at local review: **SOURCE COMPLETE / NOT PUSHED / CI NOT RUN / PRODUCTION PREVIEW NOT YET RERUN** on branch `phase9-5d-b2b-approved-historical-override`.
...
No Prisma schema/migration, Journal write, AccountingTransaction write, provider/Clover behavior, Reporting read model, dependency direction or architecture baseline changes in B2A.1. Focused source coverage pins the manifest size at 105, rejects an unlisted future stable ID, verifies both sides of the `MAX(current base, historical effective base)` policy, preserves historical option value, and verifies an approved legacy candidate becomes replay-eligible while existing unapproved `MANUAL_OVERRIDE` characterization remains conservative. After merge/deploy, the read-only production preview must be rerun before B2B write-authority cutover; expected direction is **1210 READY / 5 POST_SALE_MUTATION BLOCKED / 0 pricing overrides blocked**, with fresh plan hash, zero-cent parity and balanced Journal draft totals required as evidence rather than assumed from source.
