...
Status: **SLICE 0 READINESS AUDIT COMPLETE — SLICE 1/2/3/4/5B/5C/5D-A/5D-B0/5D-B1B/5D-B2A MERGED / CI GREEN — SLICE 5D-B1A DEPLOYED / DB VERIFIED — 5D-B2A PRODUCTION PREVIEW NEXT**  
...
After B2A merge, the next operational gate is to deploy this read-only preview and run a production range from **2026-06-01 Toronto** through an explicit exclusive end date with required `storeStableId=4750_Yonge_Street`. The result must show `parityDeltaCents = 0`, aggregate Journal debit = credit, and an exception inventory consistent with the known historical baseline (previously 101 Daily-Special `MANUAL_OVERRIDE` plus five post-sale-mutation rows, subject only to explainable dataset drift). Only then should B2B contract the provisional accrual writer and enable idempotent canonical Journal replay. Historical refunds/reversals remain 5D-C.
