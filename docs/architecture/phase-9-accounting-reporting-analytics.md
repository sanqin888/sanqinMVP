...
C1-B is **SOURCE IMPLEMENTED / LOCAL REVIEW GATE / NOT YET REMOTE** on branch `phase9-c1b-canonical-change-write-cutover`. The write boundary stays entirely inside Accounting: `AccountingCanonicalChangePreviewService` remains read-only and continues to build the fresh owner-fact plan; a separate `AccountingCanonicalChangeExecutionService` adds `POST /accounting/journal/canonical-changes/replay`, requires a lowercase SHA-256 `expectedPlanHash`, rebuilds the shadow plan at execution time, and returns HTTP 409 if the reviewed plan is stale. Only entries with `status=READY`, `classification=READY` and a non-null deterministic Journal draft are passed to the writer. `READY_NOOP` produces no Journal, and `BLOCKED` candidates are counted but never passed to any write path. Unmatched Payments reversals remain reconciliation evidence only and are not turned into Journals.
...
Prepared first-write production verification, after CI/merge/deploy and a fresh shadow preview, uses the same four currently READY facts: CASH partial **1355 / 1355**, CASH full reversal **1581 / 1581**, Store Balance full reversal **946 / 946**, and legacy CARD full reversal **846 / 846**, totaling **4728 / 4728**. Expected first execution is four canonical change JournalEntries, approximately thirteen JournalLines from the current drafts, and four CREATE Audits with write authority; an identical second execution must leave Journal/Line/Audit counts unchanged and return the same four entries as idempotent replays. Before any write, the operator must use the freshly deployed C1-B shadow `planHash`; the C1-A.1 production hash above is historical evidence only and must not be reused because C1-B intentionally adds matched-evidence IDs to the plan hash.
