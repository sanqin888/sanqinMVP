...
5D-C1-A is **LOCAL SOURCE / REVIEW READY** on `phase9-5d-c1a-accounting-change-shadow-preview`, implemented from `origin/dev@fde52e19`. Final local static review is complete; no CI, deployment or runtime verification is claimed before user review. The slice wires Accounting to the already-public Orders change facts, Orders original SALE facts, Payments financial/reversal facts and Loyalty financial facts only through owner `public-api` modules. Accounting continues to own only the matching/classification/Journal-draft policy and its own original-SALE Journal-anchor lookup; it does not read Orders, Payments or Loyalty Prisma internals.
