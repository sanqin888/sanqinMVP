...
5D-C0A is **LOCAL SOURCE / REVIEW PENDING**. It is the forward Orders-owner fact boundary only: no Accounting Journal consumer/write, no Payments reversal-fact work, no historical exception posting/backfill and no Prisma schema/migration are included.

Orders adds a separate public `ORDER_FINANCIAL_CHANGE_FACTS_READER` capability for append-only `orders.financial / order.financial_adjustment.v1` and `order.financial_reversal.v1` `OpsEvent` facts. These facts freeze stable Order/Store identity, authoritative business occurrence evidence, before/after monetary state and explicit settlement declaration in the same Orders transaction that confirms the post-sale change. Ordinary `RETENDER` / `VOID_ITEM` / `SWAP_ITEM` / `ADDITIONAL_CHARGE` use `ADJUSTMENT`; confirmed `FULL_REFUND` and provider-confirmed external cancellation use `REVERSAL`. External cancellation now fails closed when its provider occurrence time is absent instead of inventing a timestamp. Daily-Special nominal/discount presentation remains nullable in change facts when Orders cannot prove it from the mutable state; Accounting must later combine owner facts and fail closed rather than infer historical nominal pricing.

The amendment settlement contract now explicitly separates `previousOrderPaymentMethod` / `resultingOrderPaymentMethod` from `declaredSettlementPaymentMethod`. Only `RETENDER` may rewrite the whole Order payment method. A `VOID_ITEM` / `SWAP_ITEM` / `ADDITIONAL_CHARGE` settlement method is evidence for that change only and no longer reclassifies the Order's tender. Any non-zero tender refund or additional charge must carry a declared settlement method; the POS picker therefore also appears for positive VOID/SWAP differences. `redeemReturnCents` is frozen separately so a gross amendment refund is not silently treated as entirely external tender. Orders settlement evidence is explicitly not provider-money truth: CARD and Store Balance movements still require Payments/Loyalty corroboration before any 5D-C Journal posting.

The existing five historical mutation Orders and three refunded historical exceptions are not backfilled by C0A. The three SWAP amount deltas remain candidates for a later explicitly reviewed historical reconstruction; the two RETENDER rows still lack original tender/time evidence; two confirmed historical full refunds still lack authoritative exact occurrence time; and the orphan refunded Order remains `REFUNDED_STATE_WITHOUT_REVERSAL_EVIDENCE`. 5D-C0B must next establish Payments-owned reversal facts/correlation, including managed REFUND/VOID and provider reverse-sync deltas, before Accounting shadow preview/posting can begin.

Architecture remains within existing ownership: change readers use Orders-owned persistence and are exported only through the Orders public surface; Accounting is not wired as a consumer in C0A. No new direct-import allowance, public SCC, package dependency, provider execution behavior or Web Clover production-payment cutover is introduced.
