...
State at local review: **SOURCE COMPLETE / NOT PUSHED / CI NOT RUN / NO CANONICAL REPLAY EXECUTED** on branch `phase9-b2b-write-cutover`.
...
**Production gate remains explicit.** Before this source state may be used for the first canonical historical replay, retain/review the actual post-B2A.1 preview response proving the fresh plan hash, expected 1210 READY + five `POST_SALE_MUTATION` blocked + zero other pricing blocks, zero-cent parity and balanced debit/credit totals. Immediately before execution, verify the legacy and Journal tables still contain no unexpected Revenue writes. The first replay must then be followed by read-only Journal counts/amount/source-fact verification and a second same-plan replay to demonstrate idempotent convergence. `accounting.order-revenue-journal-cutover.v1` may close only after that evidence exists and the removed legacy endpoint is confirmed unavailable.
