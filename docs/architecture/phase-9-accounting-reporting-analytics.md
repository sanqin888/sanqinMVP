...
When the Terminal route is enabled for production, CARD settlement after the cutover boundary must require canonical Payments/provider evidence exactly as the current strict C1-A policy does; missing evidence remains `WAITING_FOR_PAYMENT_EVIDENCE`. Historical determinism is mandatory: before production cutover, the system must freeze an effective-from timestamp for the existing POS CARD compatibility so future replay can classify each change by its immutable `occurredAt`, rather than by whatever the current boolean flag happens to be. This timestamp is migration metadata for the existing compatibility, **not a second route-choice boolean**. Pre-cutover CARD changes remain legacy/order-declared on later replay; post-cutover CARD changes remain strict forever. The eventual Phase J compatibility contraction removes the legacy direct-paid route, flag/config, legacy refund/Accounting fallback and their cutover metadata together only after the existing exit criteria and all pre-cutover Accounting facts are reconciled.

C1-A.1 is currently **DOCUMENTED / IMPLEMENTATION PENDING**. The intended implementation must preserve owner boundaries: Accounting may consume only a narrow rollout/compatibility capability composed from the existing flag semantics, never POS implementation internals or `process.env` independently. Draft/plan determinism and Audit/provenance must include the resolved evidence authority. Focused coverage must prove at least: legacy pre-cutover in-store CARD refund -> READY without Payments reversal; post-cutover CARD refund without Payments evidence -> BLOCKED; post-cutover exact Payments reversal -> READY; historical pre-cutover replay remains stable after the live flag is enabled; CASH and Store Balance behavior is unchanged.
