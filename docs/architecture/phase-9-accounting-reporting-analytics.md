...
5D-C1-A is **MERGED / CI GREEN / DEPLOYED / CASH PRODUCTION VERIFIED; STORE BALANCE VERIFICATION EXPOSED A SALE-POLICY DEFECT** through PR #2320 / squash merge `df70660f`; final head `91cde8c5` passed PR CI #5619 and merge SHA `df70660f` passed post-merge CI #5620. The slice wires Accounting to the already-public Orders change facts, Orders original SALE facts, Payments financial/reversal facts and Loyalty financial facts only through owner `public-api` modules. Accounting continues to own only the matching/classification/Journal-draft policy and its own original-SALE Journal-anchor lookup; it does not read Orders, Payments or Loyalty Prisma internals.
...
A subsequent pure Store Balance sale/full-refund test proved the owner evidence chain but exposed a pre-existing SALE-posting semantic defect before any wrong Journal could be written. Orders froze `paymentTotalCents=846` for an `$8.46` payable sale, while Loyalty correctly recorded `STORE_BALANCE_REDEEMED=846` and `STORE_BALANCE_RETURNED=846`; C1-A first blocked the reversal for the missing original SALE anchor as designed. The canonical SALE replay preview then blocked the original SALE with `JOURNAL_POLICY / TENDER_INVARIANT`, because the B1B mapper had incorrectly treated `OrderFinancialFactV1.paymentTotalCents` as external tender instead of the owner contract's **payable total after card surcharge**. Fail-closed behavior therefore prevented an incorrect Store Balance SALE Journal.

### 18.18 5D production-verification hotfix — Store Balance payable / external-tender semantics

The local branch `phase9-store-balance-tender-semantic-fix` corrects only the Accounting-owned SALE Journal mapper and its focused tests. Orders and Loyalty contracts/persistence remain unchanged: `paymentTotalCents` continues to mean **customer payable total after card surcharge**, while Loyalty remains the owner of Store Balance principal. A read-only production compatibility scan found **15** immutable `order.financial_sale.v1` facts and **0** cases where `paymentTotalCents != orderTotalCents + cardSurchargeCents`; the only current Store Balance SALE is the observed `846 / 846 / 0` payable-principal-surcharge case. The mapper now first requires `paymentTotalCents == orderTotalCents + cardSurchargeCents`, then derives `externalTenderCents = paymentTotalCents - storeBalanceRedeemedCents`; Store Balance principal greater than the payable total fails closed. This makes a pure Store Balance sale produce zero external tender and makes mixed Store Balance + CARD/CASH derive only the remaining external amount.

Regression fixtures that previously encoded `paymentTotalCents` as external tender are corrected across the SALE policy, posting-service, range-replay and Store Balance reversal characterization tests. A production-shaped regression locks the observed `$8.46` sale semantics: `$8.49` nominal revenue, `$1.00` sales discount, `$0.97` HST, `$8.46` Store Balance liability debit, and **zero** external tender. No public contract, module ownership, dependency direction, Prisma/schema/migration, provider execution path, Web Clover behavior, C1-A write authority or compatibility path changes. Per repository workflow, local lint/build/test are intentionally deferred until user review and GitHub Actions after remote-delivery authorization.
