...
C1-A.1 is **LOCALLY IMPLEMENTED / REVIEW PENDING / NOT YET REMOTE** on the dedicated feature branch. Legacy in-store CARD refund/reversal drafts may use `LEGACY_ORDER_DECLARED` authority without synthesizing a Payments reversal; explicit `UNIFIED_PAYMENT_CORE` provenance or a Payments-owned CARD SALE fact remains strict and requires canonical Payments/provider reversal evidence, with missing evidence classified `WAITING_FOR_PAYMENT_EVIDENCE`. The relaxation is deliberately one-way: **post-sale CARD collection/additional charge still requires change-scoped Payments money evidence**. Preview output exposes resolved `cardSettlementEvidenceMode`, Orders `posCardExecutionEvidence`, and Payment source/method provenance; CARD draft hashes include the resolved evidence mode, while non-CARD draft hashing remains unchanged. No POS public compatibility token, new env variable, canonical-change POST/write path, Prisma/schema/migration, Web Clover execution/refund change or provider-traffic change is added.

Focused coverage is updated for: older unprovenanced in-store CARD SALE with no Payments SALE fact -> legacy READY; explicit legacy execution provenance -> legacy READY; Payments/Unified provenance -> strict; exact strict Payments reversal behavior; conflicting/ambiguous provider evidence fail-closed; legacy CARD collection still blocked; CASH and Store Balance behavior unchanged. New route provenance also keeps future legacy cutback vs Unified transactions deterministic without relying on a wall-clock cutover rule.

### 18.20 Phase 9 closeout gate — Production test-fixture cleanup
