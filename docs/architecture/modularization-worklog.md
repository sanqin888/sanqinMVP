...
**Branch/State:** `refactor/phase9-6d-a-legacy-settlement-contraction` / **SOURCE / LOCAL REVIEW / MIGRATION REQUIRED / NO LOCAL TEST EXECUTION PER `AGENTS.md`**.  
...
**Migration gate:** dropping the table/type is destructive schema contraction even though production currently has zero legacy rows. MCP does not create/edit migration files. After this source change is reviewed/merged to `dev`, the user must generate a create-only migration locally and review that it drops only the retired table/type plus table-owned indexes/constraints before any production promotion.  
