...
**PR/SHA:** local branch `refactor/phase8-slice8.5-test-era-compat-cleanup` from `origin/dev@466ae633`; PR not created  
**State:** REMOTE DELIVERY AUTHORIZED — local lint/build/test/scanner not run per repository workflow; no Slice 8.5 data migration or production data mutation  
...
**Tests/verification scope:** Focused application coverage preserves canonical availability/store-status retries and rejects legacy `OFFLINE` ticket context; persistence/availability coverage pins canonical Store identity. Architecture coverage/scanner must reject closed compatibility annotations, legacy OpsTicket scope symbols, provider-ID ticket dedupe and menu-availability provider aliases. After source deployment, verify canonical `4750_Yonge_Street` operations queries still work, item availability sync still reaches Uber, pause/resume still reaches Uber, and API/worker logs contain no compatibility/missing-store errors. After Uber Production Verification passes, separately inventory all Uber test tables and prepare the complete test-data cleanup for review before any destructive database action.  
