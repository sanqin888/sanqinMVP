...
**Branch/State:** `refactor/accounting-expense-payment-allocations` / **LOCAL SOURCE / REVIEW GATE / MIGRATION REQUIRED**. No commit/push/PR/remote CI is claimed.  
**Read-only evidence:** current Accounting business data is empty for this contraction: `AccountingExpenseDocument = 0` and `AccountingTransaction = 0`. Under explicit user authorization, the Expense single-payment-account model therefore does not retain compatibility, backfill, dual-read, dual-write, deprecated fields or a primary-account fallback.  
**Source/result:** replaces `AccountingExpenseDocument.accountId/account` plus `AccountingAccount.expenseDocuments` with Accounting-owned `AccountingExpensePaymentAllocation` rows. Expense create/confirm accepts optional `paymentAllocations`; an empty set is the formal unknown-payment-account state, while a non-empty set must use unique active CAD accounts, positive integer amounts and exact CAD-total closure. The retired top-level Expense `accountStableId` payload now fails explicitly instead of being silently ignored. Expense category `AccountingTransaction` splits no longer carry payment-account ownership. The account-balance read model derives confirmed Expense outflow from allocations while preserving unrelated generic Transaction/TRANSFER/Journal account semantics.  
**Web/result:** Inbox Expense Review and manual no-evidence Expense entry share a payment-allocation editor with one-row single-account UX, optional additional account rows and allocated/remaining totals. The quick-category calculator remains category/HST-only. Its aggregate button stays clickable so the existing source-currency/CAD guard explains an unsafe aggregation instead of silently disabling the action. Expense Review no longer stores a full `AccountingInboxItem` snapshot: the page stores only `reviewingInboxItemStableId` and derives the review item from the latest `items`. Existing `load()` already returns the current Inbox row plus latest ParseRun, so refresh/re-recognition/acquisition updates replace the Review Panel evidence automatically with no polling or extra endpoint/request. If the stable ID disappears from refreshed items, `load()` clears the selection; confirmation also clears it before reload.  
**Architecture effect:** Expense/allocations remain Accounting-owned; Web remains an adapter and Expense -> Journal cutover is explicitly not part of this slice. Task C changes no API contract, Prisma shape or dependency direction. No new context direction, public edge, direct-import allowance or scanner baseline is introduced; expected Accounting baseline remains Foundation **1** / External **1** / Identity **2** / Runtime **6**, total **10**, public SCC empty pending remote CI. Focused source regressions pin schema ownership, API semantics, balance-reader behavior, aggregate-button behavior and stable-ID-derived Review freshness.  
**Migration gate:** `apps/api/prisma/schema.prisma` is changed by the multi-account-payment portion, but MCP created/edited no migration and made no database write. Task C itself has no Prisma/migration effect. Suggested later user-generated create-only migration: `pnpm --filter api exec prisma migrate dev --create-only --name phase9_expense_payment_allocations`. `main`/production promotion remains blocked until that migration is generated, reviewed and merged.  
