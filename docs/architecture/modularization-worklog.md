...
**Branch/State:** `feat/phase9-slice8p-b3-payroll-lifecycle` from `origin/dev@bf175d2a` / **LOCAL SOURCE COMPLETE / REVIEW PENDING / MIGRATION REQUIRED**.  
...
**Migration gate:** **MIGRATION REQUIRED.** MCP did not create/edit `apps/api/prisma/migrations/**`. After source merge, generate locally with `pnpm --filter api exec prisma migrate dev --create-only --name phase9_slice8p_b3_payroll_opening_non_periodic_evidence`. The three new columns are intentionally required with no defaults; if any opening rows exist in the target database, do not apply generated SQL until exact evidence/backfill treatment is reviewed. Promotion to main/production remains blocked until the companion migration is reviewed and merged into dev.  
**Validation:** per `AGENTS.md`, no local Prisma generate/validate, scanner, lint, build, strict TypeScript or Jest execution is claimed before remote delivery.  
