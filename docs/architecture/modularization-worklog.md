...
**Branch/State:** `refactor/phase9-slice8a1-period-journal-capabilities` from `origin/dev@17b48d7c` / **8A-1 SOURCE READY FOR REMOTE CI / NO MIGRATION**.  
**Audit/result:** Slice 7 is source-closed and full Slice 8 remains deliberately decomposed as `8A -> 8P -> 8B -> closeout`. 8A-1 is now implemented locally: `AccountingPeriodService` owns Store-local timezone, `accountingStartDate`, date clamping, month close/reopen, year hard lock and Period editability/status; `AccountingJournalService` owns Journal mutation/idempotency, canonical-change authority, provider-settlement replacement-group authority/revalidation and Journal audit writes. Those implementations were physically removed from `AccountingService`, not wrapped. Canonical SALE/change/provider-settlement services consume the narrow capabilities, and `AccountingOperationsService` consumes Period directly. `pnlReport()` and `dashboard()` remain on legacy `AccountingTransaction` facts for 8A-4.  
...
**Architecture effect:** the split uses one Accounting-local `ACCOUNTING_DB` token bound with `useExisting: PrismaService`, avoiding a new Prisma import seam per capability. Journal mutations are pinned to the Journal capability and Period-close mutations to the Period capability; focused architecture coverage blocks return to the broad facade. Initial PR CI #5831 failed only at the architecture gate and revealed two monotonic metadata updates required by the ownership move: the migrated Brand/Store config consumer follows `accounting.service.ts -> accounting-period.service.ts`, and Accounting -> Runtime direct debt contracts **6 -> 5**. The source/baseline follow-up records that contraction rather than widening an allowance. No public edge, Prisma schema/migration, HTTP contract, report fact source or financial semantics change. Authoritative Accounting limits are now Foundation **1** / External **1** / Identity **2** / Runtime **5**, total **9**, public SCC empty. The MCP workspace has no local command runner, so no local lint/build/Jest/scanner execution is claimed; remote CI is the executable merge gate.  
**Details:** `apps/api/src/accounting/{accounting-period.service.ts,accounting-journal.service.ts,accounting-db.ts,accounting-audit-writer.ts,accounting-internal-capability-boundary.architecture.spec.ts}`, affected canonical/settlement/Operations services and characterization tests, `docs/architecture/{phase-9-accounting-reporting-analytics.md,phase-9-payroll-design.md,current-dependency-graph.md}`, this worklog.
