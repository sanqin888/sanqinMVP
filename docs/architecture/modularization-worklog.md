...
**Validation:** initial PR CI #5505 failed at the architecture gate before lint/build/test because the first remote head still exposed a public `brand-store -> accounting-reporting-analytics` edge through Homepage, forming an SCC with the existing reverse Accounting/Reporting -> Brand/Store dependency. The follow-up moves Homepage business logic onto its own outbound ranking port and confines the Reporting adapter to `HomepageContentModule`; no local validation command is claimed. GitHub Actions remains authoritative for the follow-up head.  
