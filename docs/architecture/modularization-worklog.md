...
**Branch/State:** `refactor/phase9-slice8a5-accounting-controller-vertical-split` from merged `origin/dev@66a1873b` / **SOURCE READY FOR USER REVIEW / NO MIGRATION**.  
...
**Architecture effect:** no Prisma schema/migration, package dependency, public context edge, direct-import allowance, compatibility path, HTTP contract or scanner-baseline widening. Expected upper bound remains Foundation **1** / External **1** / Identity **2** / Runtime **4**, total **8**, public SCC empty pending authoritative PR CI. Per repository workflow, no local lint/build/Jest/scanner execution is claimed before user review.  
