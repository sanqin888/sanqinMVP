...
**Branch/State:** `feat/phase9-slice8p-d3b-cra-remittance-settlement` from `origin/dev@73a70839` / **LOCAL SOURCE REVIEW PENDING / MIGRATION REQUIRED**.  
...
**Validation:** focused evidence characterization covers deterministic ordering/hash, component reconciliation, duplicate-run rejection and period bounds. Service characterization covers remitter-config transitions, unremitted POSTED selection, empty preview and incomplete frozen evidence. Architecture guard pins both models, stable scalar account/Journal references, preview route and the deliberate absence of the D3-B2 Journal writer. Per repository workflow no local lint/build/strict/Jest/scanner run is claimed before user review.  
**Migration:** **REQUIRED** because schema adds two persisted models and relations. Suggested name `phase9_slice8p_d3_cra_remittance`; user-local command after merge to dev: `pnpm --filter api exec prisma migrate dev --create-only --name phase9_slice8p_d3_cra_remittance`. Expected SQL is additive CREATE TABLE/unique/index/RESTRICT-FK only, with no drop/rename/backfill/enum rewrite. Main/production promotion is blocked until that user-generated migration is reviewed and merged back into dev.  
