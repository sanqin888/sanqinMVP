...
**State:** MERGED / CI GREEN / DEPLOYED / CASH PRODUCTION VERIFIED — Store Balance verification exposed a separate pre-existing SALE-policy semantic defect now handled by the follow-up hotfix below  
...
**Validation:** PR CI #5619 and post-merge CI #5620 passed the architecture baseline, API/Web lint/build/strict/tests and shared strict checks. Production CASH verification then exercised one `VOID_ITEM` adjustment and one `FULL_REFUND`: both first blocked on missing SALE anchors, became `READY` after exact-hash SALE replay, produced balanced shadow totals **2936 / 2936 cents**, zero unmatched Payment reversals, and identical plan/draft hashes across two repeated GETs. Store Balance verification subsequently exposed the separate SALE-policy semantic defect recorded in the follow-up hotfix entry below; C1-A itself continued to fail closed and wrote no change Journal.  
...
**PR/SHA:** local branch `phase9-store-balance-tender-semantic-fix` from `origin/dev@df70660f`; local review ready  
**State:** SOURCE / LOCAL REVIEW READY — implementation/docs/diff review complete; no remote CI, merge, redeploy or post-fix runtime verification claimed  
...
**Validation:** per `AGENTS.md`, local lint/build/test/architecture scanner are intentionally not run before user source review. GitHub Actions is the authoritative validation gate after remote-delivery authorization. Required post-deploy active verification is to rerun SALE replay preview for the observed Store Balance order, confirm it becomes READY/balanced, create its original SALE Journal through the exact-hash replay gate, then confirm the existing full-refund C1-A shadow candidate transitions from `WAITING_FOR_ORIGINAL_SALE_JOURNAL` to `READY` using the already-observed Loyalty return facts.  
