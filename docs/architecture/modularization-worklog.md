...
**PR/SHA:** local branch `refactor/phase5-print-dispatch-idempotency` based on `origin/dev@39bfc09a`  
**State:** SOURCE COMPLETE / LOCAL REVIEW PENDING — PRINT OWNS JOB IDENTITY/ROUTING/DISPATCH/ACK; ORDERS PRINT-PERSISTENCE PROBE REMOVED; POS AMENDMENT KITCHEN/LABEL/CUSTOMER PRINTING REPAIRED  
...
- Phase 5: Slice 0 is **MERGED / CI GREEN** via PR #2193 / `07311f74`, including merged dev CI #5194. Slice 1A is
  **MERGED / PR CI GREEN** via PR #2194 / `db7a1de9`, with CI #5195 passing. Slice 1B is **MERGED / PR CI GREEN**
  ...
  first-print event is removed while `order.paid.verified` remains for deferred Uber Direct work. Slice 2 is **SOURCE
  COMPLETE / LOCAL REVIEW PENDING** on `refactor/phase5-print-dispatch-idempotency`: Print owns job identity/routing,
  ...
  snapshot builder. No graph count changes; the public SCC baseline remains empty. Under the 2026-09-06 repository-wide
  cadence, Phase 5 runtime verification is performed once against the final merged Phase state immediately before
  closeout rather than after each Slice.
