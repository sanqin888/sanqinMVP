...
**Branch/State:** `phase9-6c-b3-provider-settlement-replay-gate` / **SOURCE IMPLEMENTED / LOCAL REVIEW PENDING / REAL REPLAY NOT EXECUTED / NO LOCAL TEST EXECUTION PER `AGENTS.md`**.  
...
**Post-merge verification gate:** deploy first, run a fresh June statement shadow, visually review the top-level hash and exact 84-Journal summary, then obtain a separate explicit operator authorization before the first real POST. After that POST, require the UI fresh-preview reconciliation plus read-only DB counts before 6C-B is marked replay-verified.  
