...
**Branch/State:** `phase9-6c-b1-uber-settlement-components` / **SOURCE IMPLEMENTED / MIGRATION REQUIRED / REMOTE SUBMISSION IN PROGRESS / NO LOCAL TEST EXECUTION PER `AGENTS.md`**.  
...
**Architecture/schema effect:** Accounting ownership, GET shadow/POST execution boundary, expected-plan-hash authority, Serializable replacement group, payout control-only policy, provider wire contracts, context directions and scanner allowances are unchanged. Direct-import baseline remains expected at Foundation **1** / External **1** / Identity **2** / Runtime **6**, total **10**, public SCC empty. Prisma enum expansion is persisted schema contract work, therefore **MIGRATION REQUIRED**; no migration was authored. Suggested user-local command after the source/schema PR reaches `dev`: `pnpm --filter api exec prisma migrate dev --create-only --name phase9_slice6c_b1_uber_settlement_components`. Production promotion remains blocked until the generated SQL is reviewed and merged.  
**Verification after migration/deploy:** confirm/materialize the real June statement, verify revision/review/coverage linkage, then run only the full-period `2026-06-01 -> 2026-07-01` Uber shadow preview. Require one READY provider document, zero blocked provider documents, provider draft `294164 / 294164`, Uber Pending `122285`, all 83 June historical reversals READY, repeated deterministic `planHash`, and zero provider-settlement/reversal Journals before any separately authorized POST replay.  
