...
**Deferred semantic decision:** authenticated generic `POST /accounting/tx` plus `assertNoLegacyOrderRevenueAccrual()` stay intact. They are not a Slice 7 stable-ID/Prisma blocker; the generic single-entry `INCOME / ADJUSTMENT / TRANSFER` authority decision becomes Slice **8A-0** before capability splitting.  
