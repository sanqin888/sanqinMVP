...
**PR/SHA:** local branch `refactor/phase9-slice5d-a-canonical-financial-facts` from `origin/dev@4fe76c49`  
**State:** LOCAL SOURCE IMPLEMENTATION / REVIEW PENDING — no remote CI/deploy/runtime verification claimed  
**Confirmed policy:** Points redemption = Sales Discount; Daily Special = nominal/original Sales Revenue plus separate Sales Discount; Store Balance top-up = LIABILITY and spend reduces that liability; WECHAT_ALIPAY = Cash; legacy refund/amendment rows without authoritative occurrence time remain 5D-C exceptions and are not guessed. For legacy Daily Special history, same-stable-product current Catalog base price is an approved controlled reconstruction because base prices have remained stable since the February 2026 launch; changed product structures require manual mapping and unresolved identities are not guessed.  
...
**Historical evidence discovered during implementation:** after the 2026-06-01 Toronto floor, 175 Orders contain Daily Special items; 129 (2026-06-03 through 2026-08-21) have no immutable `promotionSnapshot` original-price evidence. Of the 46 with snapshots, 45 record Daily Special `discountCents=0` but retain base/override evidence that can be reconstructed and cross-checked against persisted Daily Special item quantities/effective base prices. Orders facts expose `DAILY_SPECIAL_NOMINAL_UNKNOWN` when their own immutable evidence is absent. 5D-B may then classify a same-stable-product Catalog reconstruction as `CATALOG_STABLE_MATCH`; the former Roujiamo pork/beef option shape and other product-identity changes require `MANUAL_OVERRIDE`, and unmatched rows remain `UNRESOLVED`. The two Store Balance top-up Orders since 2026-06-01 also have null Store identity; 5D-A does not invent it. These remain explicit 5D-B historical backfill/data-quality gates without adding a 5D-A Orders -> Catalog dependency.  
**Architecture/schema effect:** additive public contracts/modules plus one Orders-owned durable financial-snapshot write inside existing Order create/finalize transactions; no Prisma/schema/migration, no Journal/AccountingTransaction writes, no Accounting consumer import, no package/dependency change, no provider execution/wire change and no production Web Clover payment-semantics change. The Accounting/Reporting/Analytics direct baseline is expected to remain Foundation **2**, External **1**, Identity **2**, Runtime **6**, total **11**, with `legacyPublicCycleComponents=[]`. Focused characterization and architecture tests are added, but per repository workflow local lint/build/test/scanner execution is not claimed; remote CI remains authoritative after submission.  
