...
**PR/SHA:** local branch `phase9-5d-b2b-approved-historical-override`; not pushed / no PR  
**State:** SOURCE COMPLETE / LOCAL REVIEW GATE — CI, deployment and post-change production preview not yet run  
**Decision/evidence:** user explicitly authorized the exact 105 `PRICING_MANUAL_OVERRIDE` Order stable IDs from the verified B2A production exception inventory to use historical accounting reconstruction `approved nominal base = MAX(current Catalog base, persisted historical effective base)`. Historical `OrderItem` evidence separately preserves `baseUnitPriceCents`, `optionsUnitPriceCents`, `unitPriceCents` and option snapshots, so current Catalog options are not needed or substituted.  
...
**Compatibility/architecture effect:** this is a user-authorized, exact-ID exception to the general no-current-Catalog historical-repricing rule, not a new general fallback. `accounting.order-revenue-journal-cutover.v1` remains shadow-only; the legacy accrual route remains the sole possible Revenue writer and canonical Journal replay is still disabled. No Prisma/schema/migration, package dependency, new context direction, direct-import debt, scanner allowance, provider/Web Clover behavior or Reporting read model changes. Baseline remains Foundation **2**, External **1**, Identity **2**, Runtime **6**, total **11**, public SCC empty.  
**Verification scope:** focused tests pin the approval manifest at 105, reject an unlisted future ID, verify current-base-higher and historical-base-higher branches of the MAX formula, preserve option value, and verify an approved legacy candidate becomes replay-eligible while the existing unapproved MANUAL_OVERRIDE tests remain intact. Repository workflow defers local lint/build/test to GitHub Actions after user review. After merge/deploy, rerun the same read-only production range; expected direction is 1210 READY / 5 POST_SALE_MUTATION BLOCKED / 0 pricing override blocks, but only the actual preview may establish the new plan hash and zero-delta/balanced evidence.  
