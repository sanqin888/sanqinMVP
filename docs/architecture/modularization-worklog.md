...
**PR/SHA:** local branch `phase9-c1b-canonical-change-write-cutover` from merged `origin/dev` `35ece2da`; no remote PR yet  
...
**Architecture/schema effect:** no new owner-context import direction, deep import, reverse dependency, scanner allowance, Prisma/schema/migration or package dependency. The shadow remains the sole Orders/Payments/Loyalty reconciliation reader and the new execution service imports only Accounting-local capabilities. `assertNoLegacyOrderRevenueAccrual()` remains a pre-write gate; no legacy `AccountingTransaction` revenue/refund writer is restored. Production Web Clover execution/refund behavior is untouched. Expected direct baseline remains Foundation **2**, External **1**, Identity **2**, Runtime **6**, total **11**, public SCC empty, subject to authoritative GitHub CI after operator review.  
**Validation prepared:** focused source coverage now includes stale-plan rejection before writer access; READY-only execution with READY_NOOP/BLOCKED/unmatched evidence excluded; balanced-total/original-SALE-anchor fail-closed behavior; canonical-change authority persisted into idempotency + CREATE Audit; source-identity mismatch rejection; identical replay convergence and authority-drift conflict; architecture guards that keep preview pure and restrict the new execution role. Per repository policy no local lint/build/test/CI reproduction has been run before operator review.  
**Production verification scope after merge/deploy:** obtain a fresh C1-B shadow hash, then write the existing four READY facts: CASH partial **1355 / 1355**, CASH full **1581 / 1581**, Store Balance full **946 / 946**, legacy CARD full **846 / 846**, total **4728 / 4728**. Current drafts imply four JournalEntries, approximately thirteen JournalLines and four CREATE Audits. A second identical replay must produce no additional Journal/Line/Audit rows. The historical C1-A.1 plan hash must not be reused because C1-B intentionally adds matched-evidence stable IDs to plan hashing.  
