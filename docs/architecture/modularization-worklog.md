...
**PR/SHA:** PR #2254 / final source head `d7d61660`; final docs evidence head pending  
**State:** PR OPEN / CI GREEN — CI #5392 passed Prisma generation, Architecture baseline, API lint/build/strict/shared strict/full tests, and Web lint/build/strict/tests; earlier #5390/#5391 passed Architecture and failed only focused new-file lint findings; no deployment/active verification yet  
**Result:** Implements the authorized additive half of the Phase 7 closeout blocker without changing Uber admission truth. Prisma adds POS-owned `PosConnectivityReadModel`, keyed only by `storeStableId`, with heartbeat-capable-device presence, latest heartbeat and derived `validUntil`; migration `20260909173000_expand_pos_connectivity_read_model` only creates the table and performs no destructive change/backfill. `PosDeviceService` remains the sole writer and maintains the projection from existing heartbeat-capable ACTIVE device activity plus device lifecycle recomputation, while projection failures are non-authoritative/log-only in 5A. `UberOrderImportPrismaAdapter` keeps the legacy `PosDevice` + `common/pos-connectivity` result as the returned truth under new compatibility `pos-connectivity.read-model-shadow.v1`, reads the projection only for `uber_pos_connectivity_read_model_shadow_compare`, and logs shadow failures without changing admission. Architecture guards prohibit Uber writes to the read model and prohibit a reverse `Uber -> POS` source/public dependency, so source inspection introduces no new public-cycle direction. This expand slice intentionally leaves the configured Phase 7 Store Operations direct-debt baseline at **9**; final public-SCC/scanner confirmation is deferred to GitHub Actions after review. Slice 5B is mandatory before source closeout to cut admission to the read model, delete the direct Uber `PosDevice`/Foundation connectivity reads, move connectivity policy into POS ownership, and close the temporary compatibility after deliberate parity evidence is clean.  
