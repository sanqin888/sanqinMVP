...
**State:** MERGED / CI GREEN — PR CI #5476 and merged-head CI #5477 passed API + Web; closeout active verification in progress  
...
**PR/SHA:** local branch `fix/phase8-menu-publish-store-schedule` from merged Phase 8 closeout baseline; PR not created  
**State:** LOCAL SOURCE / USER REVIEW PENDING — no local lint/build/test/scanner run per repository workflow  
**Trigger/evidence:** Consolidated 8.2A verification found Admin Dry Run Publish displaying all seven days as `00:00–23:59`. Read-only production DB inspection confirmed canonical Store data is not 24-hour: `4750_Yonge_Street` uses `America/Toronto`, `salesTaxRate=0.13`, and seven explicit BusinessHour rows including Monday-Friday `08:00–22:30` and Sunday `12:00–21:00`. Uber Store timezone remained absent from the provider response and therefore correctly stayed behind the existing manual-confirmation gate; that absence was not the schedule mismatch cause.  
