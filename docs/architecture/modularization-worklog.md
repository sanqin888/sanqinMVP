...
**Branch/State:** `feat/phase9-slice8p-d2-payroll-employee-payment` from `origin/dev@3bae5682` / PR #2392 / head `fdb44919` / **SOURCE + MIGRATION REVIEWED / CI RE-RUN PENDING**.  
...
**Validation:** focused pure-authority, orchestration and Journal-writer characterization plus controller/Payroll architecture guards are added. PR CI #5887 passed Prisma generation, Architecture, API lint/build/strict and the full Web job, but API Jest exposed one stale source-text assertion that expected the net-pay account literal after production code was changed to reuse canonical `PAYROLL_ACCOUNT_IDS.netPayPayable`. The local follow-up updates that architecture assertion; per repository workflow no local lint/build/strict/Jest/scanner run is claimed.  
