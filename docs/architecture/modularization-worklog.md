...
**Branch/State:** `refactor/phase9-slice7b-accounting-actor-identities` from latest `origin/dev` / **LOCAL SOURCE / REVIEW GATE / MIGRATION REQUIRED**. No push, PR, remote CI or database mutation is claimed.  
...
**Migration gate:** `schema.prisma` changes are source-only. MCP created/edited no migration and made no database write. The required user-generated migration must preserve all existing strings exactly; Journal/Audit columns and the Audit index should be represented as renames rather than destructive drop/add. Suggested command: `pnpm --filter api exec prisma migrate dev --create-only --name phase9_slice7b_accounting_actor_identities`. `main`/production promotion remains blocked until that migration is reviewed and merged to `dev`.  
