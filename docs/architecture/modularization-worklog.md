...
**Result:** Closeout testing confirmed a POS in-store full refund moved the Order to `refunded` and persisted a confirmed `FULL_REFUND` amendment, but produced no kitchen cancellation print after the original INITIAL customer/kitchen job. The hotfix preserves refund/payment ownership and extends the existing Orders/Fulfillment -> POS/Print business-purpose handoff with `CANCELLATION`. Full-refund orchestration emits cancellation printing only after canonical refund success; Fulfillment independently requires the persisted Order to be `refunded`, creates an `整单取消 / ORDER CANCELLED` kitchen snapshot with every original item cancellation-marked, and hands it to Print. Print owns routing and durable identity: `CANCELLATION` is kitchen-only, no label/customer auto-print, and uses one stable `kind=CANCELLATION` per Order for replay idempotency. The same local batch fixes the 70x30 mm food-label header: the printer agent now passes the already-owned `pickupCode` instead of `orderNumber` (which may fall back to full `orderStableId`), and the PowerShell label renderer centers `#<pickupCode>` across the header while preserving the existing left pair-code and bilingual item layout; no stable-ID fallback is printed when pickup code is absent. Focused orchestration/Fulfillment/gateway/printer-agent architecture characterizations cover confirmed-only emission, non-refunded suppression, cancellation payload shape/routing/unique job identity and centered pickup-code label identity. No Prisma schema/migration, dependency/lockfile, public route, Clover provider protocol, refund amount/state rule, pricing/promotion policy or architecture allowance changes. Per AGENTS.md no local lint/build/test/scanner run is claimed before user review.  
