...
**State:** SOURCE MERGED / CI GREEN — PR CI #5238 PASSED; PRODUCTION DATA REPAIR NOT EXECUTED AND REMAINS SEPARATELY GATED  
**Result:** Fixes the pre-existing Customer address identity mismatch discovered during Slice 4C. `CustomerService.createAddress()` no longer rewrites the first character of a generated CUID to `a`; it now writes the canonical `c...` value produced by the shared `generateStableId()`. Focused address coverage asserts the generated identifier is accepted by the same shared `normalizeStableId()` consumed by Orders, and Customer order-context fixtures now use canonical address IDs. Orders validation remains strict and is not widened to accept historical `a...` values. Read-only production audit found exactly **2** `UserAddress` rows, both in the legacy `a + 24` shape; no other typed `addressStableId` column exists, all **12** current `CheckoutIntent.metadataJson` rows contain no `addressStableId` key, and none references either current address ID. Because the legacy generator only replaced the first character, the pending production correction is deterministic: restore each existing ID's first character from `a` to `c`, with exact pre/post counts and collision checks. No Prisma schema/migration, dependency/lockfile, route, context edge, scanner allowance, payment/provider, pricing, lifecycle, or Benefits transaction semantics change. The two-row mutation is intentionally deferred until after source review, remote CI/merge, deployment readiness, and explicit production-mutation approval.  
...
**PR/SHA:** local branch `fix/phase5-full-refund-kitchen-cancel-print`  
**State:** SOURCE / LOCAL REVIEW PENDING  
**Result:** Closeout testing confirmed a POS in-store full refund moved the Order to `refunded` and persisted a confirmed `FULL_REFUND` amendment, but produced no kitchen cancellation print after the original INITIAL customer/kitchen job. The first hotfix iteration covered that POS path with a local EventEmitter, then production re-test of an Uber scheduled cancellation exposed the missing cross-process boundary: Uber cancellation is confirmed/persisted in `ubereats-worker`, so no API-local cancellation event was emitted. The implementation is now normalized onto the existing durable `orders.lifecycle` OpsEvent path. Confirmed POS/internal full refunds and confirmed Uber cancellations append the same idempotent `order.cancelled` fact in the same transaction that sets `Order.status=refunded`; API `OrderLifecycleOutboxProcessor` consumes it only when `order.initial_print_handoff` already exists and no `order.cancellation_print_handoff` exists, then calls Fulfillment to create an `整单取消 / ORDER CANCELLED` kitchen-only `CANCELLATION` job and checkpoints only after the Print handoff succeeds. `order.prep_started` is excluded once the Order is already refunded, preventing an initial ticket from being replayed after cancellation. Print keeps one stable `kind=CANCELLATION` per Order, so worker retries/replayed webhooks do not duplicate the ticket. The same local batch fixes the 70x30 mm food-label header: the printer agent now passes the already-owned `pickupCode` instead of `orderNumber` (which may fall back to full `orderStableId`), and the PowerShell label renderer centers `#<pickupCode>` across the header while preserving the existing left pair-code and bilingual item layout; no stable-ID fallback is printed when pickup code is absent. Focused Orders full-refund, Uber persistence, lifecycle-outbox, Fulfillment, gateway and printer-agent characterizations cover durable confirmed-only production, initial-handoff gating, refunded-state gating, replay checkpointing, cancellation payload/routing/unique job identity and centered pickup-code label identity. No Prisma schema/migration, dependency/lockfile, public route, Clover provider protocol, refund amount/state rule, pricing/promotion policy or architecture allowance changes. Per AGENTS.md no local lint/build/test/scanner run is claimed before user review.  
