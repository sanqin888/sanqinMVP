...
**PR/SHA:** local branch `refactor/phase9-slice5c-c-provider-financial-history` from `origin/dev@6f1093a8`  
...
**Uber/API boundary:** Existing `UberEatsReportingPort` is additively extended with validated READY/IMPORTED artifact read + imported-state capability. Accounting never deep-imports Uber persistence or artifact storage. `ORDERS_AND_ITEMS_REPORT` is excluded from financial history. PAYMENT/FINANCE CSV artifacts are registered into Unified Inbox but remain parser-pending until a real provider CSV schema is observed and fixture-pinned; no guessed CSV mapping is allowed.  
...
**Architecture/schema effect:** no Prisma/schema/migration change and no intended new dependency direction; current merged Accounting baseline remains Foundation **2**, External **1**, Identity **2**, Runtime **6**, total **11**, pending remote CI confirmation for this slice. No `AccountingTransaction`, `Order`, Journal or settlement posting is created by 5C-C.  
**Details:** `apps/api/src/accounting/accounting-provider-financial*`, `apps/api/src/accounting/accounting-inbox-*`, `apps/api/src/integrations/ubereats/{public-api.ts,application/operations,infrastructure}`, `apps/web/src/app/[locale]/(site)/accounting/{inbox,settings}`, `docs/architecture/phase-9-accounting-reporting-analytics.md`, `docs/architecture/current-dependency-graph.md`, this worklog.
