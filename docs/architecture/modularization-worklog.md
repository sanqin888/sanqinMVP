...
**Branch/State:** `feat/phase9-slice8p-d2-payroll-employee-payment` from `origin/dev@3bae5682` / **SOURCE REVIEWED / MIGRATION REQUIRED**.  
...
**Migration:** schema adds `PayrollEmployeePayment`; MCP created no migration file. After source merges to dev, generate locally with `pnpm --filter api exec prisma migrate dev --create-only --name phase9_slice8p_d2_payroll_employee_payment`, review for additive create-table/unique/index/FK-only SQL, then merge the user-generated migration before main/production.  
**Validation:** focused pure-authority, orchestration and Journal-writer characterization plus controller/Payroll architecture guards are added. Per repository workflow no local lint/build/strict/Jest/scanner run is claimed before review.  
