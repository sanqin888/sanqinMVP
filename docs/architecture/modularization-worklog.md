...
**Branch/State:** `phase9-expense-inbox-cad-boundary` / **SOURCE IMPLEMENTED / LOCAL REVIEW GATE / NO LOCAL TEST EXECUTION PER `AGENTS.md`**.  
...
**Legacy data audit:** read-only production inspection found exactly **7** pre-refactor `GMAIL / PENDING_REVIEW` ExpenseDocuments. Each has zero active AccountingTransactions and no current Inbox materialization link; current pending Inbox items materialized as Expenses = **0**. They are safe cleanup candidates under the already-approved pre-formal-bookkeeping data-disposal policy, but this source batch does not mutate production data.  
**Architecture/schema effect:** no Prisma schema/migration, package dependency, Textract integration, context edge, scanner allowance, Journal authority, provider-settlement policy or revenue-posting change. Native-text PDF/CSV/email recognition remains local; the separately approved Textract adapter is deferred to the next slice after this financial boundary review. Expected baseline remains Foundation **1** / External **1** / Identity **2** / Runtime **6**, total **10**, public SCC empty pending GitHub Actions after review.  
