...
**PR/SHA:** local branch `refactor/phase9-slice5d-b0-revenue-posting-hardening` from `origin/dev@7f35878f`  
**State:** LOCAL SOURCE IMPLEMENTATION / REVIEW PENDING — no remote CI/deploy/runtime verification claimed  
...
**Architecture/schema effect:** no Prisma/schema/migration, Journal/AccountingTransaction posting, compatibility path, dependency-manifest or provider/Web Clover behavior change. Orders already depends on Catalog through the public order-facts boundary, so no new context direction is introduced; Accounting is still not wired to Orders/Loyalty/Payments. The expected Accounting direct baseline remains Foundation **2**, External **1**, Identity **2**, Runtime **6**, total **11**, with `legacyPublicCycleComponents=[]`; remote CI remains authoritative after user review.  
