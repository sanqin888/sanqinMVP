...
**PR/SHA:** local branch `refactor/phase9-slice5d-b1b-canonical-sale-journal-engine` from `origin/dev@91e1c6e7`  
**State:** LOCAL SOURCE IMPLEMENTATION / REVIEW PENDING — no remote CI/deploy/Journal posting/historical backfill claimed  
...
**Architecture/runtime effect:** Accounting now composes `OrderFinancialFactsModule` and `LoyaltyFinancialFactsModule` through public surfaces; no owner internals or Payments implementation are imported. This intentionally adds a public Accounting -> Orders capability edge and reuses the public Identity/Loyalty direction, while expected direct debt remains Foundation **2**, External **1**, Identity **2**, Runtime **6**, total **11**, with no public SCC. The new post service is deliberately not exposed through `AccountingController` or scheduler; the existing `automation/order-accrual` path remains the sole active revenue-posting entry, so B1B deploy alone cannot dual-write Revenue or trigger historical backfill. No Prisma/schema/migration, provider/Web Clover behavior, Reporting read model, or old `AccountingTransaction` path is changed. Per repository workflow no local lint/build/test/scanner is run before user review.  
**Next:** after review/CI/merge, implement controlled range preview/replay/parity from the configured Toronto start date, define the explicit workflow for 101 Daily-Special `MANUAL_OVERRIDE` rows plus five post-sale-mutation exceptions, and only then retire/demote the provisional `Order.totalCents -> AccountingTransaction` accrual path. Refund/reversal posting remains 5D-C.  
