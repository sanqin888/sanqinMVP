...
**PR/SHA:** local branch `refactor/phase6-slice2b-pos-transport-public-surface` / base `origin/dev@b1051c24`  
**State:** LOCAL / REVIEW PENDING  
**Result:** Contracts five Payments -> POS implementation-path imports without changing ownership or runtime behavior. `pos/public-api.ts` now exports the existing `PosDeviceGuard` and `PosModule` alongside the already-public `PosDeviceModule`; the three Payment/POS controllers consume `PosDeviceGuard` from that public surface, and `PosCardPaymentOrchestrationModule` consumes the two existing Nest modules from the same surface. No wrapper/facade, token, module or guard is introduced, and guard ordering plus the composition `imports` array remain unchanged. `payments-architecture.spec.ts` guards these transport/composition consumers against returning to `pos-device.guard`, `pos-device.module` or `pos.module` deep imports. The local monotonic baseline contracts `payments-clover -> store-operations-pos-print` **9 -> 4**, reducing Payments/Clover total direct debt **51 -> 46**; public SCC remains expected empty pending CI. `PosCardPaymentFeatureConfig`, full-refund `PosOrdersService`/`PosCreateFullRefundInput`, refund policy, confirmed-payment atomicity, provider/reconciliation/refund truth, Prisma schema/migrations, dependencies and Web Clover are explicitly unchanged. No local lint/build/test/scanner execution is claimed per repository workflow.  
