...
**PR/SHA:** local source only on `phase9-5d-c0b-payments-reversal-facts`; review pending  
**State:** SOURCE / LOCAL REVIEW PENDING — no CI/deploy/runtime verification claimed  
**Scope/result:** extends the Payments owner-public financial boundary without wiring Accounting. Final managed REFUND/VOID transactions now recover stable Order/Store identity through the original provider-correlated SALE and the existing checkout binding. A new `PAYMENT_REVERSAL_FINANCIAL_FACTS_READER` exposes normalized managed reversal money facts plus provider-webhook reversal deltas. The existing durable `payment.reverse-sync.completed` OpsEvent now freezes provider event identity, provider `occurredAt`, previous/current cumulative refunded base amount and exact per-event delta after reverse-sync orchestration succeeds. Managed reversal facts retain canonical completed time, exact base/additional-charge/customer refund totals and provider refund identity; external webhook facts intentionally leave unproven additional-charge/customer-total refund values null.  
...
**Validation:** per `AGENTS.md`, no local lint/build/test is run before user review. Focused specs cover managed REFUND identity correlation, reversal fact precision, webhook delta persistence, zero-delta suppression and managed/webhook deduplication; GitHub Actions remains the validation gate after remote-delivery authorization.  
