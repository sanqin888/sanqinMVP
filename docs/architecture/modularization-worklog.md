...
**Branch/State:** `phase9-textract-expense-recognition` / **SOURCE IMPLEMENTATION / LOCAL REVIEW GATE PENDING DEPENDENCY LOCK REGENERATION / NO LOCAL TEST EXECUTION PER `AGENTS.md`**. The user explicitly authorized the new npm dependency before source modification.  
**Recognition ownership/routing:** Accounting gains an internal AWS Textract `AnalyzeExpense` adapter behind opt-in `ACCOUNTING_TEXTRACT_ENABLED` (default false). Receipt images submit a transient auto-oriented, high-confidence horizontally cropped, bounded JPEG; original SourceArtifact bytes remain unchanged and still feed the existing Tesseract fallback when Textract/preparation fails. Native-text PDFs remain Poppler/provider-parser first and do not call AWS; only an empty-text scanned PDF may try synchronous Textract, with multi-page/oversize cases failing back to the existing review path rather than adding S3/async infrastructure. CSV and email-body parsing remain local. Image-recognized text now runs provider-financial recognition before generic expense suggestion, matching the established PDF ordering.  
**Financial safety:** structured Textract `SUBTOTAL/TAX/TOTAL` are editable recognition candidates and gain arithmetic consistency diagnostics; `AMOUNT_PAID` is evidence-only and cannot override total. Provider-inferred currency is evidence-only; only explicit source text promotes `CAD/USD/EUR/GBP` into `sourceCurrency`. Conflicting normalized date candidates are not resolved by highest provider confidence. `LineItemGroups` never become durable SKU rows; only count/price-sum/reconciliation diagnostics survive, so confirmed Expense facts remain category-level. The persisted evidence is the bounded Accounting projection rather than raw AWS JSON. Generic Inbox parser version advances to `2` for the new recognition metadata/routing semantics.  
**Dependency/architecture effect:** adds explicitly authorized `@aws-sdk/client-textract` to `apps/api`; `pnpm-lock.yaml` still requires pnpm regeneration before any remote submission and must not be hand-edited. No Prisma schema/migration, public API, image-retention writer, Journal authority, Expense posting policy, context import or scanner allowance change. Expected direct-import baseline remains Foundation **1** / External **1** / Identity **2** / Runtime **6**, total **10**, public SCC empty pending GitHub Actions after user review.  
**Verification scope:** focused source regressions cover real-observed failure modes: Foody-style wrong high-confidence `AMOUNT_PAID`, incorrect provider currency suggestion, reconciled vs unreconciled LineItem prices, conflicting dates, original-image Textract preference/fallback, native PDF local routing and scanned-PDF synchronous fallback. Production enablement additionally requires IAM `textract:AnalyzeExpense`, `ACCOUNTING_TEXTRACT_ENABLED=true`, then re-upload the two known Foody receipts plus one explicit-USD Cloudflare-style bill; confirm no AWS request for a normal native-text provider PDF.  
