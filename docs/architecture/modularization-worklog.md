...
**Validation:** focused evidence characterization covers deterministic ordering/hash, component reconciliation, duplicate-run rejection and period bounds. Service characterization covers remitter-config transitions, unremitted POSTED selection, empty preview and incomplete frozen evidence. Architecture guard pins both models, stable scalar account/Journal references, preview route and the deliberate absence of the D3-B2 Journal writer. PR CI #5894 passed Prisma generation, Architecture and Web but API lint failed one `no-unsafe-assignment` from a redundant nested Jest matcher; commit `6f0929f4` removed that matcher. CI #5895 again passed Prisma generation, Architecture and Web, then failed only because the same test still destructured an unused `prisma` fixture. The local follow-up now destructures only `service`; no production/migration behavior changes and no local CI run is claimed.  
