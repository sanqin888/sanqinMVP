...
**Architecture effect:** the split uses one Accounting-local `ACCOUNTING_DB` token bound with `useExisting: PrismaService`, avoiding a new Prisma import seam per capability. Journal mutations are pinned to the Journal capability and Period-close mutations to the Period capability; focused architecture coverage blocks return to the broad facade. No public edge, scanner allowance, Prisma schema/migration, HTTP contract, report fact source or financial semantics change. Authoritative Accounting allowance remains Foundation **1** / External **1** / Identity **2** / Runtime **6**, total **10**, public SCC empty. The MCP workspace has no local command runner, so no local lint/build/Jest/scanner execution is claimed; remote CI is the executable merge gate.  
