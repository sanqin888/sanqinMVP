...
- Phase 5: Slice 0 is **MERGED / CI GREEN** via PR #2193 / `07311f74`, including merged dev CI #5194. Slice 1A is
  **MERGED / PR CI GREEN** via PR #2194 / `db7a1de9`, with CI #5195 passing. Slice 1B is **MERGED / PR CI GREEN /
  PRODUCTION UNVERIFIED** via PR #2195 / `e4a783a5`, final head `c8ed5579`, CI #5200 passing: ordinary in-store POS
  creation owns durable `order.accepted`, durable `prep_started` materializes the sole AUTO first print, and the POS
  payment page no longer performs first `printOrderCloud()` / `advanceOrder()`. The user explicitly authorized both
  no old-PWA compatibility and proceeding to Slice 1C before active 1B testing; this does not mark 1B verified.
  Slice 1C is **SOURCE COMPLETE / LOCAL REVIEW PENDING** on
  `refactor/phase5-slice1c-web-durable-lifecycle`: Web payment still creates only a paid Order, while the existing
  store-side auto/manual acceptance now writes durable `order.accepted`; immediate Web acceptance then synchronously reuses the same
  idempotent preparation materializer to write `making + order.prep_started`, with the accepted-event poll as recovery; scheduled Web
  acceptance waits for the existing prepStartAt scheduler, and Web/in-store
  first AUTO printing is durable-origin only. Context debt/SCC baselines remain unchanged; 1B/1C production verification
  remains pending as a later deployment batch.
