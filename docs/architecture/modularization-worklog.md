...
**Branch/State:** `refactor/phase9-slice8a3-generic-transaction-contraction` from merged `origin/dev@eaff4c0a` / **SOURCE READY FOR USER REVIEW / NO MIGRATION**.  
**Readiness evidence:** read-only production verification still finds `AccountingTransaction = 0` and zero retired `AUTO_ORDER:` / `AUTO_ORDER_DAILY:` idempotency rows. Repository search finds no Accounting Web `/accounting/tx` consumer.  
**Source/result:** removes the authenticated generic `POST/GET/PUT/DELETE /accounting/tx` surface, Controller `TxBody`, `AccountingService` `UpsertTxDto` / generic validator / public Transaction select / `createTx` / `listTx` / `updateTx` / `deleteTx`, and the generic CRUD characterization suite. `AccountingTransaction` stays in Prisma unchanged because Expense category splits still persist through `AccountingExpenseService`; P&L/dashboard/Transaction CSV export reads also remain unchanged until 8A-4.  
...
**Architecture effect:** no Prisma schema/migration, package dependency, public context edge, direct-import allowance, compatibility path or report fact-source cutover. Current expected upper bound remains Foundation **1** / External **1** / Identity **2** / Runtime **4**, total **8**, public SCC empty; any scanner reduction in remote CI must lower the baseline rather than add an allowance. Per repository workflow, no local lint/build/Jest/scanner execution is claimed.  
