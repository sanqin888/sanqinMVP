...
**Architecture effect:** `AccountingGmailIngestService` no longer imports `PrismaService`; acquisition/query helpers avoid new runtime persistence boundaries. The monotonic baseline therefore contracts Accounting / Reporting / Analytics Runtime debt **7 -> 6**, total **13 -> 12** while Foundation **3**, External **1**, Identity **2** remain unchanged. No new public SCC or scanner allowance is expected; remote CI remains authoritative.  
