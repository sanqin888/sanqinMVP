...
**Validation:** per `AGENTS.md`, no local lint/build/test was run before user review. Focused characterization/architecture tests were added for immutable fact serialization/idempotency, reader fail-closed behavior, full-refund/external-cancellation facts, settlement-method enforcement and the no-non-RETENDER Order-tender rewrite rule; GitHub Actions remains the validation gate after user authorizes remote delivery.  
**Details:** `apps/api/src/orders/{order-financial-change-fact.ts,order-financial-change-facts-reader.contract.ts,order-financial-change-facts-reader.service.ts,order-financial-change-facts.module.ts,orders.service.ts,order-external-cancellation.service.ts,public-api.ts}`, related focused specs, POS orders page, `docs/architecture/phase-9-accounting-reporting-analytics.md`, `docs/architecture/current-dependency-graph.md`, Uber Eats architecture note, this worklog.
