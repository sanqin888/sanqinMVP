...
**State:** MERGED / PR CI GREEN / POST-MERGE CI GREEN — PR CI #5679 and post-merge CI #5680 green / NO LOCAL TEST EXECUTION PER `AGENTS.md`  
...
**Verification state:** GitHub Actions is green. Runtime verification remains pending: rebuild the API image so Poppler is present, then re-upload the same Uber statement and confirm readable text, provider-financial review routing, period `2026-06-01..2026-06-30`, `Marketplace Fees`, and the real `Net Total` control amount.  
...
**Branch/State:** `phase9-accounting-image-retention` from `origin/dev@9c358405` / SOURCE IMPLEMENTED / LOCAL REVIEW GATE / NO LOCAL TEST EXECUTION PER `AGENTS.md`  
...
**Architecture/schema effect:** retention writes live in a TransactionClient-only Accounting writer and are orchestrated without another `PrismaService` boundary. Initial PR CI #5694 correctly rejected a third direct Accounting -> architecture-foundation upload-path import. The same-scope fix introduces one Accounting-local storage-path adapter and routes acquisition/controller/retention through it, contracting that pair **2 -> 1**; `tools/architecture/context-baseline.json` is tightened accordingly. Expected direct-import baseline is therefore Foundation **1** / External **1** / Identity **2** / Runtime **6**, total **10**, public SCC empty pending rerun. No new package dependency, external/public context edge, provider wire behavior, Journal authority or posting policy. Prisma adds `AccountingArtifactBinaryRetentionState` and `AccountingArtifactBinaryRetention`, so **MIGRATION REQUIRED** before `main` / production promotion; the user explicitly authorized a dev-only sequence where this CI-green source PR may merge first, then the migration is generated locally from updated `dev` and returned in a follow-up before any production promotion.  
