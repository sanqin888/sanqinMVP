...
**Branch/State:** `refactor/phase9-slice7-accounting-order-contract-contraction` / **SOURCE / LOCAL REVIEW / MIGRATION REQUIRED / NO LOCAL TEST EXECUTION PER `AGENTS.md`**.  
...
**Migration gate:** **MIGRATION REQUIRED**. Suggested name `phase9_slice7_a_contract_legacy_accounting_transaction_order_sources`; generate user-locally with `pnpm --filter api exec prisma migrate dev --create-only --name phase9_slice7_a_contract_legacy_accounting_transaction_order_sources`. Review must show only the `AccountingTransaction.orderId` column drop plus a safe PostgreSQL `AccountingSourceType` contraction to `MANUAL / OTHER`, with no unrelated drift. Production has zero rows, so no backfill/value conversion is required.  
**Details:** `apps/api/prisma/schema.prisma`, `apps/api/src/accounting/{accounting.controller.ts,accounting.service.ts,accounting-ledger.characterization.spec.ts,canonical-financial-facts-boundary.architecture.spec.ts}`, `docs/architecture/{phase-9-accounting-reporting-analytics.md,current-dependency-graph.md}`, this worklog.
