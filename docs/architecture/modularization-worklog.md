...
**PR/SHA:** local branch `phase9-c1a1-legacy-pos-card-accounting` from merged `origin/dev` `878e85b9`; no remote PR yet  
...
**Accounting policy:** legacy/order-declared mode permits refund/negative settlement drafting without manufacturing Payments reversal evidence and exposes `LEGACY_ORDER_DECLARED` provenance; CARD collection/additional charge still requires change-scoped Payments money truth. Strict mode keeps exact reversal matching and `WAITING_FOR_PAYMENT_EVIDENCE`. Preview surfaces the resolved evidence mode, owner SALE execution provenance and Payment source/method; CARD draft hash includes the evidence mode. No canonical-change write endpoint is added.  
**Architecture/schema effect:** no new bounded-context edge, no deep import, no direct-import allowance, no reverse dependency, no Prisma/schema/migration and no public SCC is expected. Existing Accounting -> Orders/Payments public fact reads are reused. The direct baseline remains Foundation **2**, External **1**, Identity **2**, Runtime **6**, total **11**, subject to authoritative CI after review/push. Production Web Clover, provider traffic, current rollout value and C1-A write authority remain unchanged.  
**Validation prepared:** focused tests cover older legacy CARD refund READY without Payment reversal, explicit legacy provenance, Payments/Unified strict provenance, exact strict reversal behavior, legacy CARD collection still blocked, immutable sale-fact old-payload compatibility, and existing architecture guards. Per repository policy no local lint/build/test/CI reproduction has been run before operator review.  
