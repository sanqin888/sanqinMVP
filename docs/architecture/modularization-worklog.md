...
**PR/SHA:** local branch `refactor/phase9-slice4-reporting-orders-boundary`; no PR / remote SHA yet  
**State:** SOURCE COMPLETE / LOCAL REVIEW PENDING  
**Scope/result:** Introduces a Reporting-owned `REPORTING_ORDER_FACTS_QUERY` outbound contract with versioned V1 metric/item facts and an Orders-owned public `ORDER_REPORTING_FACTS_READER`. Orders now owns the existing Order/OrderItem Prisma queries and immutable `componentsJson` parsing; `ReportsService` consumes only Reporting contracts. `ReportsModule` is explicitly registered as the composition root that adapts the Orders public reader to the Reporting outbound port. Homepage switches from deep `ReportsService` / `ReportsModule` imports to the public `REPORTING_TOP_ITEMS_QUERY` capability. Existing report timezone, status eligibility, KPI/payment/fulfillment/timeline semantics and historical Top Items behavior are preserved. No persisted Reporting projection/read model, migration/backfill/replay worker or Revenue Posting semantics are introduced.  
**Architecture effect:** source baseline contracts `accounting-reporting-analytics -> commerce-orders-fulfillment` **1 -> 0**, `accounting-reporting-analytics -> runtime-data-ci-ops` **9 -> 7**, and `brand-store -> accounting-reporting-analytics` **2 -> 0**. Accounting / Reporting / Analytics direct debt becomes Foundation **3**, External **1**, Identity **2**, Runtime **7**, total **13**. The Orders binding is confined to the registered composition root, so `legacyPublicCycleComponents` is expected to remain empty rather than introducing `Brand -> Reporting -> Orders -> Brand`.  
**Validation:** repository review-first policy applies; no local lint/build/test/architecture command is claimed. Reporting characterization is retained at the new port boundary, Orders-reader tests pin the reportable status set/current queries/component decoding, and a focused architecture spec guards the cycle-safe ownership shape. GitHub Actions validation is deferred until user authorizes remote delivery.  
