...
**State:** MERGED / CI GREEN — PR CI `#5453` and merged-head CI `#5454` / `#5455` passed API + Web; staged production verification and destructive migration application remain pending  
...
**Schema/data contraction:** Source/DB audit found `UberOrderCancellation` had one production writer and zero readers. Read-only DB inventory before contraction found **21 rows / 21 distinct events / 21 distinct orders**; all 21 events had durable `UberWebhookInbox` evidence and corresponding `OrderAmendment`, with zero missing inbox/amendment/order references in the parity checks. The final pre-commit read-only check still reports the same **21 / 21 / 21** counts and all 21 corresponding inbox rows are already `PROCESSED`, so there is no currently queued legacy cancellation replay. Because current Uber records are Test Store data and the user explicitly authorized destructive removal, Prisma relation `Order.uberCancellations` and model `UberOrderCancellation` are removed. Migration `20260910131300_contract_uber_order_cancellation` contains only `DROP TABLE "UberOrderCancellation";` without `CASCADE`; it has not been applied to production. `docs/architecture/id-inventory.md` contracts the UUID-backed model count **65 -> 64**. Production rollout is deliberately staged: deploy/recreate the new API + Uber worker code first while the legacy table still exists, verify no legacy-table access and re-check non-terminal `orders.failure` work, then apply the drop only after separate production-migration authorization and normal backup/readiness checks.  
...
**Tests/verification scope:** Added Orders finalizer characterization for canonical amount/payment ownership, stable/external identity matching, same-event replay/idempotency and rollback propagation; Uber application tests assert the dedicated cancellation port while persistence characterization no longer contains a cancellation writer; architecture guards require application-port/composition-root routing, forbid reintroducing `UberOrderCancellation` or canonical cancellation delegates in Uber persistence, and require the non-cascading migration. Phase 8 closeout must actively verify `orders.failure` causes one canonical refund/amendment + `order.cancelled`, cancellation printing still occurs exactly as expected, replay remains convergent, and sanitized logs/DB show no legacy-table access.  
...
**PR/SHA:** PR #2271; merged `dev` SHA `466ae633`  
