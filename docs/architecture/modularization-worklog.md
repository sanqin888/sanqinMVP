...
**Branch/State:** PR #2394 / head `ec10eb60` / **SOURCE + MIGRATION REVIEWED / CI RE-RUN PENDING**.  
...
**Validation:** focused evidence characterization covers deterministic ordering/hash, component reconciliation, duplicate-run rejection and period bounds. Service characterization covers remitter-config transitions, unremitted POSTED selection, empty preview and incomplete frozen evidence. Payroll architecture guard pins both models, stable scalar account/Journal references, preview route and the deliberate absence of the D3-B2 Journal writer. PR CI #5894/#5895 exposed only two sequential test-lint cleanup issues; `6f0929f4` and `e017cd85` resolved them. CI #5896 then passed API lint/build/strict/shared-strict, Prisma generation, Architecture scanner and the full Web job; Jest failed only because the pre-existing 8A Accounting vertical-boundary exact inventories had not listed `AccountingPayrollCraRemittanceService` or the new CRA preview GET route. The local follow-up adds exactly those two expected entries, without widening scanner rules or changing production/migration behavior. No local CI run is claimed.  
**Migration:** user-generated `20260918215432_phase9_slice8p_d3_cra_remittance` is reviewed against `schema.prisma` and is additive create-table/unique/index/RESTRICT-FK-only SQL. It contains no drop/rename/backfill/enum rewrite or alteration of existing Payroll rows; nullable Journal stable-ID uniqueness is valid for PostgreSQL. Main/production promotion remains blocked until PR #2394 is green and merged to `dev`.  
