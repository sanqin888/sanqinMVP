...
**State:** MERGED / CI GREEN — CI #5516 passed; deployment/runtime verification not yet claimed  
...
**PR/SHA:** local docs branch `docs/phase9-slice5c-audit` from `origin/dev@cbe8ad6f`; no remote PR yet  
**State:** READ-ONLY AUDIT / LOCAL DOCS — 5C implementation not started; no new Prisma/migration/source behavior, CI, deploy or runtime verification claimed  
...
**Implementation recommendation:** split 5C into 5C-A Unified Inbox Core, 5C-B Acquisition Cutover, and 5C-C Provider Financial Parsing + History. Gmail body and supported attachments become independent artifacts; `MANUAL_UPLOAD` joins the same pipeline; trusted sender governs automatic intake eligibility but never provider/document classification. Because 5B's persisted migration has never been applied on the running VM and 5C-A will add another migration on top, the preferred rollout gate is to deploy/apply 5B first and smoke-verify migration/API startup before implementing the 5C-A schema.
