...
**Branch/State:** `phase9-6c-b-pdf-extractor-hardening` / SOURCE IMPLEMENTED / LOCAL REVIEW GATE / NO LOCAL TEST EXECUTION PER `AGENTS.md`  
...
**Verification state:** source/tests/docs are updated but intentionally not run locally. GitHub Actions becomes authoritative after user review and explicit remote-delivery authorization. Post-merge runtime verification must rebuild the API image so Poppler is present, then re-upload the same Uber statement and confirm readable text, provider-financial review routing, period `2026-06-01..2026-06-30`, `Marketplace Fees`, and the real `Net Total` control amount.  
