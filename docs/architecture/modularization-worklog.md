...
**Branch/State:** `feat/phase9-slice8p-d-payroll-financial-posting` from `origin/dev@b252382d` / **LOCAL SOURCE COMPLETE / REVIEW PENDING / PAYROLL COA DATA MIGRATION HANDOFF REQUIRED / NO PRISMA SCHEMA MIGRATION**.  
...
**CoA gate:** seven Payroll account IDs are defined by the D1 owner policy but intentionally are **not yet** added to `DEFAULT_ACCOUNTING_ACCOUNTS`. The required CoA provisioning is data-only and cannot be generated from `schema.prisma` by `prisma migrate dev --create-only`; per `AGENTS.md`, MCP does not create/edit migration files. The operator must add a durable data migration locally (suggested name `phase9_slice8p_d0_payroll_coa`) following the Tip Revenue pattern. Until reviewed migration evidence exists, posting fails closed because all seven active CAD account prerequisites are required. After that migration is present, this same D1 branch must add the accounts to TypeScript defaults and extend the cumulative CoA migration guard before merge.  
...
**Validation:** focused source characterization covers exact Journal mapping, incoherent frozen amounts, missing/changed CoA facts, stale run authority, one-transaction posting and idempotent POSTED replay. Per repository workflow no local lint/build/strict/Jest/scanner run is claimed before review.  
