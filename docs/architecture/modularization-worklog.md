...
**PR/SHA:** local branch `fix/customer-address-stable-id-cuid` / base `origin/dev@1f58f1a3`  
**State:** LOCAL / REVIEW PENDING — SOURCE FIX COMPLETE; PRODUCTION DATA REPAIR NOT EXECUTED; NO LOCAL LINT/BUILD/TEST RUN PER REPOSITORY WORKFLOW  
...
- Phase 5: Slice 0 is **MERGED / CI GREEN** via PR #2193 / `07311f74`, including merged dev CI #5194. Slice 1A is
  **MERGED / PR CI GREEN** via PR #2194 / `db7a1de9`, with CI #5195 passing. Slice 1B is **MERGED / PR CI GREEN**
  ...
  scanner rejects any protected `.menuItem.` delegate. Slice 4C is **MERGED / CI GREEN** via PR #2204 / `1f58f1a3`, final head `3efd8930`, with PR CI #5236 passing: Orders User/UserAddress persistence reads are zero and Customer runtime facts now cross only the stable-ID public capability while Commerce remains **22**. Slice 4C-A is **LOCAL / REVIEW PENDING** on `fix/customer-address-stable-id-cuid`: new Customer addresses now use canonical `c...` StableIds, while the two historical production `a...` rows remain a separately gated data repair.
