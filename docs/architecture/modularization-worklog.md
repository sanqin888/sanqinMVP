...
**PR/SHA:** local branch `phase9-b2b-write-cutover`; remote delivery not yet started  
**State:** SOURCE COMPLETE / USER APPROVED FOR REMOTE DELIVERY — CI not run; no canonical replay executed  
...
**Compatibility/architecture effect:** `accounting.order-revenue-journal-cutover.v1` moves from shadow-only to cutover-source state: old Revenue invocation is removed in source and canonical Journal becomes the only Revenue automation writer, but compatibility is not closed until the exact B2A.1 preview evidence and first canonical replay/idempotent rerun are production-verified. No Prisma/schema/migration, dependency, provider/Web Clover, Orders/Loyalty/Store public-contract, direct-import or scanner-allowance change; expected graph baseline stays Foundation **2**, External **1**, Identity **2**, Runtime **6**, total **11**, public SCC empty.  
**Verification scope:** focused source tests cover stale-plan refusal, exact mutation acknowledgement, unresolved-pricing hard block, canonical execution delegation and legacy-row guard; architecture coverage pins removal of the old route/method and presence of the gated replay route. Per repository workflow, local lint/build/test are deferred to GitHub Actions after this user-approved remote delivery. The post-B2A.1 preview body has been user-reviewed with the expected 1210 replay-ready records; first production execution still requires the exact reviewed `planHash` and runtime parity/balance/block rechecks. After first replay, verify Journal counts/amounts/source facts, zero legacy Revenue rows, old endpoint unavailability and a second same-plan idempotent replay before closing compatibility.  
