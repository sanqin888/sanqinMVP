...
**Architecture/schema effect:** retention writes live in a TransactionClient-only Accounting writer and are orchestrated without another `PrismaService` boundary. No new dependency, context direction, scanner allowance, provider wire behavior, Journal authority or posting policy. Prisma adds `AccountingArtifactBinaryRetentionState` and `AccountingArtifactBinaryRetention`, so **MIGRATION REQUIRED** before `main` / production promotion. Recorded direct-import baseline remains Foundation **2** / External **1** / Identity **2** / Runtime **6**, total **11**, public SCC empty pending GitHub CI after user review.  
