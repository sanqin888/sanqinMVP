...
**Compatibility/architecture effect:** registers `accounting.order-revenue-journal-cutover.v1` as active shadow-only compatibility. The provisional `/accounting/automation/order-accrual` / `Order.totalCents -> AccountingTransaction` path remains the sole active Revenue writer during B2A; canonical Journal replay is disabled. B2B must retire/demote that old write before enabling canonical replay, so simultaneous Revenue dual-write is explicitly forbidden. The replay service is architecture-pinned to Orders/Loyalty/Store public APIs only; no new context direction or direct-import allowance is expected, so baseline should remain Foundation **2**, External **1**, Identity **2**, Runtime **6**, total **11**, public SCC empty pending remote CI. Per repository workflow no local lint/build/test/scanner is run before user review.  
**Next:** after reviewed PR/CI/merge and deployment, run production preview from 2026-06-01 through an explicit exclusive end date scoped to `4750_Yonge_Street`; require `parityDeltaCents=0`, aggregate Journal debits=credits, and explain every blocked row against the historical exception inventory before authorizing B2B cutover/write replay. Refund/reversal posting remains 5D-C.  
