...
**Architecture/schema effect:** intentionally adds the one-way public Accounting -> Payments financial-fact capability and composes Orders change/Payments fact public modules in Accounting. The Orders change reader gains only an additive batch operation on the already-existing Accounting -> Orders public edge. `PaymentFinancialFactsModule` is narrowed to Prisma-backed fact-reader composition instead of importing the full `PaymentsModule`, so this Accounting read edge does not activate Clover provider/execution infrastructure. No Accounting deep import of Orders/Payments/Loyalty internals, no reverse Payments/Orders/Loyalty -> Accounting dependency, no new direct-import allowance, no Prisma/schema/migration, no Journal/AccountingTransaction write, no Clover execution/canonicalization change and no Web Ecommerce cutover. Direct debt/public-SCC results await GitHub Actions and are not claimed locally.  
