...
**PR/SHA:** local branch `refactor/phase9-slice5d-b1a-store-balance-liability-coa` from `origin/dev@fd32edcf`  
**State:** LOCAL SOURCE IMPLEMENTATION / REVIEW PENDING — no remote CI/deploy/migration application claimed  
**Authorization/readiness:** user explicitly authorized Prisma/migration work. `AccountingAccount` already supports nullable operational `type`, `AccountingAccountClass.LIABILITY`, CAD and active state, so no structural `schema.prisma` change is required. Production read-only pre-migration evidence shows 20 Accounting accounts, no `account_store_balance_liability`, 0 `AccountingJournalEntry` and 0 `AccountingJournalLine`; the cleaned Store Balance opening/post-cutover principal baseline remains zero.  
**Scope/result:** adds `account_store_balance_liability` / `储值余额负债` to `DEFAULT_ACCOUNTING_ACCOUNTS` as a null-type LIABILITY account and adds additive data-only migration `20260913010000_phase9_slice5d_b1a_store_balance_liability_coa`. The migration inserts the reserved stable ID as active CAD liability and on stable-ID conflict only repairs its account class; it does not alter Orders/Loyalty history, user-created accounts, Journal rows or create an opening balance. The CoA architecture test now treats the 5B seed plus later additive CoA migrations cumulatively and pins the new account semantics plus no Journal insert.  
**Architecture/schema effect:** no structural Prisma model change, new API/public boundary, runtime context edge, provider behavior or posting behavior. Expected Accounting direct baseline remains Foundation **2**, External **1**, Identity **2**, Runtime **6**, total **11**, with `legacyPublicCycleComponents=[]`. Per repository workflow no local lint/build/test/scanner is run before user review; the migration SQL is manually authored because a data-only seed has no Prisma schema diff for `migrate dev` to generate.  
**Next:** after review/CI/merge and production migration application, 5D-B1B can implement the canonical SALE Journal mapper/preview-post path without historical backfill.  
