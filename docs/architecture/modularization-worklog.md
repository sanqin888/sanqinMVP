...
**PR/SHA/State:** PR #2356 / final head `42d9e160` / squash merge `6f4e9f4d`; PR CI #5753 green. **MERGED / CI GREEN / DEPLOYED / OPERATOR GATE VERIFIED / REAL REPLAY DEFERRED BY OPERATOR / REAL REPLAY NOT EXECUTED / NO LOCAL TEST EXECUTION PER `AGENTS.md`**.  
...
**Operator decision / deferred cutover:** the operator explicitly chose not to begin formal bookkeeping yet, so the first real replay is intentionally **DEFERRED BY OPERATOR**. This is not a technical blocker and does not require compatibility work now because the ledger has not been mutated. Before any later first POST, deploy the then-current code, generate a fresh June shadow, revalidate revision/review/coverage/CoA/83 anchors and zero prior target Journals, review the new current `planHash` and write summary, and obtain a new explicit authorization. The hash recorded above is verification evidence only and must not be reused as future authorization. Downstream work must not assume June settlement has already been posted.  
