...
**PR/SHA:** local branch `refactor/phase8-8-4-report-artifact-idempotency` from `origin/dev@982b4de1`; PR not created  
**State:** LOCAL SOURCE / USER REVIEW PENDING — local lint/build/test/scanner deferred to GitHub Actions per repository workflow  
...
**Tests/verification scope:** New infrastructure characterization covers identical replay -> one CSV URL/file, changed content -> distinct artifact, and corrupt deterministic-path mismatch -> fail closed. Application characterization covers retry after artifact download succeeded but READY persistence failed and the existing already-READY early return. GitHub CI is the validation gate after user review. Phase closeout should verify a real `eats.report.success` when the `eats.report` capability becomes available; production scope/whitelist remains provider-gated.  
