...
**Branch/State:** `refactor/phase9-6d-a-legacy-settlement-contraction` / **MERGED / PR CI GREEN / MIGRATION MERGED TO `dev` / NOT YET PRODUCTION APPLIED**. Source merged through PR #2367 / squash `895cd35b` after CI #5790 passed; user-generated migration `20260917044832_phase9_slice6d_a_drop_legacy_platform_settlement` is merged at `cb384c82`.  
...
**Migration gate:** the destructive contraction migration is now merged to `dev`; review confirms it contains only `DROP TABLE "PlatformSettlementRecord"` and `DROP TYPE "SettlementPlatform"`. Production application remains a separate rollout action.  
...
**Branch/State:** `refactor/phase9-slice7-accounting-order-contract-contraction` / **MERGED / PR CI GREEN / MIGRATION MERGED TO `dev` / NOT YET PRODUCTION APPLIED**. Source merged through PR #2369 / squash `e5997fb9` after CI #5793 passed; user-generated migration `20260917053632_phase9_slice7_a_contract_legacy_accounting_transaction_order_sources` is merged at `af6fbc06`.  
...
**Migration gate:** user-generated migration review is complete. SQL creates replacement enum `AccountingSourceType_new` with only `MANUAL / OTHER`, casts `AccountingTransaction.source` into it, renames/retires the old enum type, and drops only `AccountingTransaction.orderId`; no unrelated schema drift or backfill is present. Production application remains a separate rollout action.  
...
**Branch/State:** `refactor/accounting-expense-payment-allocations` / **MERGED / PR CI GREEN / MIGRATION MERGED TO `dev` / NOT YET PRODUCTION APPLIED**. Source merged through PR #2372 / squash `38984b1a`; PR CI #5804 passed API/Web gates. User-generated migration `20260917122856_phase9_expense_payment_allocations` merged at `9d14212d`.  
...
**Migration gate:** migration review confirms the generated SQL only drops the authorized empty legacy Expense payment-account FK/index/column and creates `AccountingExpensePaymentAllocation` plus its indexes/FKs. Production application is not claimed here.  
...
**Branch/State:** `refactor/phase9-slice7b-accounting-actor-identities` / **MERGED / PR CI GREEN / MIGRATION PRESENT IN `dev` / PRODUCTION PROMOTION BLOCKED** through PR #2375 / squash `141fd362`; PR CI #5817 passed API/Web.  
...
**Branch/State:** `refactor/phase9-slice7d-accounting-reporting-boundary` from `origin/dev@412dc66b` / **LOCAL SOURCE / REVIEW GATE / NO MIGRATION**. No push, PR, remote CI, schema change or database mutation is claimed.  
