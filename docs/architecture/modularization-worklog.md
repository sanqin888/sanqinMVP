...
**State:** MERGED / PR CI GREEN / POST-MERGE CI GREEN / DEPLOYED / LIVE RE-VERIFICATION BLOCKED BY DUPLICATE LIFECYCLE — PR CI #5679 and post-merge CI #5680 green / production is running a later head that contains `ff97871d` / NO LOCAL TEST EXECUTION PER `AGENTS.md`  
...
**Verification state:** GitHub Actions is green and the PDF hardening is deployed in production. The 2026-09-15 re-verification attempt used the same real Uber statement, but the file was rejected into `DUPLICATE` because an earlier copy remained in Inbox history; the Web UI then misleadingly said it had entered the Inbox while the default review list showed zero items. The PDF extractor/parser therefore has **not yet been functionally re-verified after deployment**. The follow-up Upload-library slice fixes the duplicate lifecycle/visibility problem so the prior copy can be permanently removed and the same statement can be uploaded as a new source; after that, verification must still confirm readable text, provider-financial review routing, period `2026-06-01..2026-06-30`, `Marketplace Fees`, and the real `Net Total` control amount.  
...
**Branch/State:** `phase9-inbox-provider-recognition-config` / SOURCE IMPLEMENTED / LOCAL REVIEW GATE / NO LOCAL TEST EXECUTION PER `AGENTS.md`  
...
**Branch/State:** `phase9-accounting-filename-encoding` from `dev@376cea32` / SOURCE IMPLEMENTED / LOCAL REVIEW GATE / NO LOCAL TEST EXECUTION PER `AGENTS.md`  
