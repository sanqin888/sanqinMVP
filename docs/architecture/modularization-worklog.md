...
**Branch/State:** `feat/phase9-slice8p-b1-payroll-foundation-dev2386` from merged `origin/dev@5e968136` / **LOCAL SOURCE COMPLETE / REVIEW PENDING / MIGRATION REQUIRED**.  
...
**Migration:** **MIGRATION REQUIRED** because B1 adds persisted enums/models/relations and `AccountingJournalSource.PAYROLL`. MCP did not touch `apps/api/prisma/migrations/**`. Suggested migration: `phase9_payroll_b1_foundation`; after schema/source merges to dev, generate locally with `pnpm --filter api exec prisma migrate dev --create-only --name phase9_payroll_b1_foundation`, then return the generated SQL for review. Main/production promotion remains blocked until that migration is reviewed and merged into dev.  
