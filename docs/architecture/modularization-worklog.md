...
**PR/SHA:** local branch `refactor/phase5-orders-usecase-decomposition-a` / base `origin/dev@3cc775f1`  
**State:** SOURCE / LOCAL REVIEW PENDING — INVOICE ROUTES CALL `OrderInvoiceUseCase`; `OrdersService` NO LONGER OWNS INVOICE DELIVERY OR CONCRETE PRINT-PAYLOAD DEPENDENCIES; COMMERCE TOTAL REMAINS 20; NO LOCAL LINT/BUILD/TEST RUN PER REPOSITORY WORKFLOW  
