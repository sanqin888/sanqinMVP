...
**Branch/State:** `feat/phase9-slice8p-c-payroll-ui-pdf` from merged `origin/dev@dca67435` / **LOCAL SOURCE COMPLETE / REVIEW PENDING / DEPENDENCY LOCK HANDOFF REQUIRED / NO MIGRATION**.  
...
**Dependency/runtime packaging:** user explicitly authorized the new dependency. Source declares `pdfkit@^0.20.2`, `@types/pdfkit@^0.17.6`, and runner package `font-noto-cjk`. MCP has no package-manager execution tool and the existing lockfile lacks PDFKit's complete transitive graph, so `pnpm-lock.yaml` is intentionally not hand-edited. Before remote delivery the user must run real pnpm on this branch to generate the lockfile, then return it for review.  
**Architecture effect:** no Prisma schema/migration, context direction, public edge, direct-import allowance, Payroll CoA/settlement persistence or Journal authority change. Payroll production code still has zero direct `@prisma/client` imports and no direct AccountingTransaction/Journal delegate mutation. Expected baseline remains Foundation **1** / External **1** / Identity **2** / Runtime **4**, total **8**, public SCC empty pending CI.  
**Validation:** per `AGENTS.md`, no local package install/lint/build/strict/Jest/scanner execution is claimed by MCP before user review.  
