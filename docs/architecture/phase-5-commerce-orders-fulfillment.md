...
The former same-process `order.prep_started` / compatibility-named `emitOrderAccepted()` path was a Slice 0 baseline fact and is retired by Slice 1E. Initial AUTO printing now consumes only durable `orders.lifecycle/order.prep_started`. The later post-modularization P0 hygiene audit confirmed `NotificationProcessor` had no event consumer, scheduler, dynamic registration or runtime responsibility beyond a startup log, so the class and its `OrdersModule` provider registration are deleted atomically. Manual thank-you/invoice behavior remains unchanged. The obsolete uncalled `OrdersService.notifyDeliveryDispatchFailureAlert()` helper was removed in the pre-Slice 3 Uber Direct alert hardening batch. Slice 4A then removed the remaining verified-dead `dispatchPriorityDelivery()` / `buildUberPickupOverride()` delivery tail together with `ensureLoyaltyAccountWithTx()`; none is part of the current source graph.
...
Status: **MERGED / CI GREEN / PRODUCTION RE-VERIFIED** — PR #2216, squash merge `81b79148`.

Production verification surfaced one POS payment-routing regression: a member order fully covered by loyalty points displayed `0` due, but the browser kept `paymentMethod=cash`, opened the cash-received dialog with `0`, and then rejected confirmation because the cash dialog requires a positive received amount. The server quote and loyalty redemption amount were already correct; the defect was the browser-side classification `totalAfterPointsCents > 0 && remainingTotalCents === 0`, which recognized full Store Balance coverage but excluded the pure-points case where `totalAfterPointsCents` itself becomes zero.

The merged fix broadens the existing internal-benefits classification to any zero remaining external tender actually covered by loyalty points and/or Store Balance. Such orders use the existing unified payment flow, which already treats `externalAmountCents === 0` as succeeded without starting a Clover provider sale and finalizes the immutable payment snapshot through the existing Benefits COMMIT + Order transaction. The request payment method remains the repository's current `STORE_BALANCE` internal-benefits classification; no new enum, public contract, API behavior, Clover provider path, Prisma schema/migration, refund rule, pricing rule or architecture edge is introduced. Cash orders with a positive external amount still use the cash dialog unchanged.

Production re-verification on 2026-09-07 confirmed a pure-loyalty zero-external POS order completed without the cash dialog or Clover sale, committed loyalty points, entered the normal durable lifecycle and completed AUTO customer/kitchen/label printing plus ready notification.
...
The first hotfix iteration kept payment/refund ownership unchanged and added `CANCELLATION`, but used an API-local `order.cancellation.print` EventEmitter after POS full-refund success. Production verification of an Uber scheduled cancellation exposed the cross-process gap: Uber cancellation is confirmed and persisted in `ubereats-worker`, so the API-local event is never emitted. The follow-up normalizes cancellation onto the existing durable `orders.lifecycle` OpsEvent path. Confirmed internal full refunds append idempotent `order.cancelled` in the same Orders transaction that moves the Order to `refunded`; confirmed Uber cancellations append the same fact in the worker transaction that persists the Uber cancellation/amendment and refunded status. `OrderLifecycleOutboxProcessor` consumes the fact only when the Order is still `refunded`, `order.initial_print_handoff` already exists, and no `order.cancellation_print_handoff` exists; after Fulfillment successfully creates the kitchen-only `CANCELLATION` job, the outbox writes the handoff checkpoint. Refunded orders are excluded from later INITIAL handoff replay. Print still owns the stable `kind=CANCELLATION` identity, so retries/replayed webhooks cannot duplicate the cancellation ticket. The Uber producer imports these lifecycle symbols through the Orders `public-api.ts` boundary, keeping the architecture direct-import baseline unchanged.

The same local closeout batch also corrects the existing 70x30 mm food-label header identity: the stable print payload already contains `pickupCode`, but the Windows printer agent previously passed `orderNumber` into the label script, and `orderNumber` can fall back to the full `orderStableId`. The agent now passes `pickupCode` explicitly; the PowerShell renderer draws `#<pickupCode>` horizontally centered across the label header instead of drawing `orderNumber` right-aligned. The existing left-side pair code and bilingual item layout remain unchanged, and labels with no pickup code do not fall back to exposing the stable ID.

No Prisma schema/migration, dependency/lockfile, public HTTP route, Clover provider protocol, refund amount/state rule, pricing/promotion behavior or context-import allowance changes are introduced. Focused orchestration, Fulfillment, Print gateway and architecture characterization cover confirmed-only emission, non-refunded suppression, cancellation payload shape, kitchen-only routing, stable cancellation job identity, and pickup-code-only centered label identity. Per repository workflow, local lint/build/test/scanner execution is intentionally not run before user review; GitHub Actions remains the validation gate after remote authorization.
...
The action button reuses the existing `/membership/marketing-consent` command. The existing Customer false-to-true transition remains the only opt-in mutation and still triggers the existing Benefits issuance path. Prompt dismissal is browser-local per authenticated user and local calendar day: closing the prompt records the current local date, so later reloads or fresh customer-site mounts on the same day stay quiet; on a later local date the prompt may appear again while the Customer remains unsubscribed and Benefits still reports an eligible offer. Admin/Accounting routes and incomplete login/MFA flows do not render the prompt. This adds one explicit Customer-facing composition over a Benefits public read capability and does not add a deep import, scanner allowance, dependency package, Prisma schema/migration, or persistence ownership change.
