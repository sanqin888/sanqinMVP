...
The action button reuses the existing `/membership/marketing-consent` command. The existing Customer false-to-true transition remains the only opt-in mutation and still triggers the existing Benefits issuance path. Prompt dismissal is render-local only: closing the prompt hides it for the current mounted customer-site shell, but no browser-local 30-day cooldown is persisted. A later full reload or fresh customer-site mount may prompt again while the Customer remains unsubscribed and Benefits still reports an eligible offer. Admin/Accounting routes and incomplete login/MFA flows do not render the prompt. This adds one explicit Customer-facing composition over a Benefits public read capability and does not add a deep import, scanner allowance, dependency package, Prisma schema/migration, or persistence ownership change.
