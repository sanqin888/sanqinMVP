...
The action button reuses the existing `/membership/marketing-consent` command. The existing Customer false-to-true transition remains the only opt-in mutation and still triggers the existing Benefits issuance path. A 30-day "not now" cooldown is stored only in browser local storage, avoiding a Phase-closeout Prisma schema/migration. Admin/Accounting routes and incomplete login/MFA flows do not render the prompt. This adds one explicit Customer-facing composition over a Benefits public read capability and does not add a deep import, scanner allowance, dependency package, or persistence ownership change.
