...
The local hotfix keeps payment/refund ownership unchanged and extends the existing Orders/Fulfillment -> POS/Print handoff purpose from `INITIAL | REPRINT | AMENDMENT` to `INITIAL | REPRINT | AMENDMENT | CANCELLATION`. Full-refund orchestration emits `order.cancellation.print` only after the result is canonically confirmed as `refunded`; PROCESSING/UNKNOWN/RECONCILING/failed managed refunds do not emit. Fulfillment independently re-reads the Order and requires `status=refunded`, builds a kitchen snapshot headed `整单取消 / ORDER CANCELLED`, prefixes every original item with cancellation markers, and hands it to Print as `CANCELLATION`. Print owns the durable identity and routes this purpose kitchen-only with one stable `kind=CANCELLATION` per Order, so replay cannot create multiple UUID cancellation jobs. No customer receipt or label is auto-requested by this cancellation purpose.
