...
Status: **LOCAL / REVIEW PENDING** on `fix/phase5-full-refund-kitchen-cancel-print`.
...
The local hotfix keeps payment/refund ownership unchanged and extends the existing Orders/Fulfillment -> POS/Print handoff purpose from `INITIAL | REPRINT | AMENDMENT` to `INITIAL | REPRINT | AMENDMENT | CANCELLATION`. The first local iteration used an API-process EventEmitter after POS full-refund success; production verification of an Uber scheduled cancellation proved that this was not a valid cross-process boundary because Uber cancellation is confirmed and persisted in `ubereats-worker`. The implementation is therefore normalized onto the existing durable `orders.lifecycle` OpsEvent path already used by Uber ACCEPT and Orders preparation. Canonically confirmed internal full refunds append `order.cancelled` in the same Orders transaction that moves the Order to `refunded`; Uber cancellation appends the same fact in the same worker transaction that persists `UberOrderCancellation`, the confirmed amendment and `Order.status=refunded`.

`OrderLifecycleOutboxProcessor` consumes `order.cancelled` only when the Order is still `refunded`, the original order already has `order.initial_print_handoff`, and no `order.cancellation_print_handoff` exists. It then calls Fulfillment, which builds a kitchen snapshot headed `整单取消 / ORDER CANCELLED`, prefixes every original item with cancellation markers, and hands it to Print as `CANCELLATION`; only after the durable Print handoff succeeds does the outbox append `order.cancellation_print_handoff`. Print owns the durable identity and routes this purpose kitchen-only with one stable `kind=CANCELLATION` per Order, so retries/replayed Uber webhooks cannot create multiple UUID cancellation jobs. Orders cancelled before their initial kitchen handoff do not produce a cancellation-only ticket, and `order.prep_started` is no longer eligible for INITIAL handoff after the Order is already refunded. No customer receipt or label is auto-requested by this cancellation purpose.
