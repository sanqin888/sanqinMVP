...
**Slice 4D implementation status (2026-09-08): MERGED / CI GREEN / DEPLOYED / NON-DEVICE CLOVER SANDBOX VERIFIED.** PR #2245 final head `c665b469` 通过 PR CI #5361，squash merge 为 `1cc4a829`，post-merge CI #5362 的 API/Web 均通过。Terminal static token path 已在 source/deployment config 中删除；`CloverTerminalTransport` 注入现有 `CloverMerchantAccessTokenService`，以 `CLOVER_UNIFIED_MERCHANT_ID` 获取数据库 OAuth token，并在 HTTP 401 时只进行一次 force-refresh + 同 idempotency request retry。Transport 保留同步的 Terminal static config predicate，不改 `PaymentProvider` / `PaymentTerminalProvider` / POS public contract；pre-send credential unavailable 与 post-send network uncertainty 已分离，前者让 Sale/Refund/Void fail closed 且不发 HTTP，后者继续 `UNKNOWN` 并要求 reconciliation。Focused tests 与 architecture guards 锁定 DB token、401 refresh、no-Web-fallback 与 static-token removal。部署后的 production SanQ runtime 已完成 Test Merchant OAuth 并形成 `ACTIVE` `4750_Yonge_Street` binding；当前 sandbox 权限为 Merchant READ + Payments READ + Orders READ，Ecommerce enabled，未增加写权限。Canonical Platform v3 read 在补齐 Orders READ 后返回 HTTP 200；自然 access-token 到期后自动 refresh/rotation 将 `tokenVersion 2 -> 3` 并继续读成功，API recreate 后也能从数据库解密并复用 rotated credential。`POS_CLOVER_TERMINAL_PAYMENT_ENABLED` 仍未开启；Cloud Pay Display/device availability 与所有 Terminal financial acceptance 继续等待 Clover Dev Kit。
...
**Phase 9 Slice 5D-C0B reversal-fact boundary (2026-09-13):** final managed REFUND/VOID transactions resolve their business Order/Store context through the original provider-correlated SALE and existing checkout binding; reversal rows are not required to become a second checkout owner. Payments additionally exposes a read-only `PAYMENT_REVERSAL_FINANCIAL_FACTS_READER` that normalizes managed reversals and provider reverse-sync evidence. Managed facts use canonical reversal `completedAt` and may expose provider-proven base/additional-charge/customer refund totals. Provider-webhook facts use provider `occurredAt` and the exact delta between previous/current cumulative refunded base amount persisted in the durable reverse-sync completion event; because the current Platform payment read does not prove externally refunded surcharge/additional-charge allocation, those fields must remain null rather than be inferred. A managed reversal and its later webhook confirmation must not create duplicate money facts, zero-delta webhook observations are not reversal facts, and ambiguous/over-refund correlation fails closed. This remains a Payments read boundary only and does not alter Clover execution, POS refund orchestration, Web Ecommerce cutover or Accounting write authority.

**Phase 9 Slice 5D-C1-A Accounting shadow-consumer support (2026-09-14):** the two Payments financial-fact readers add stable Order-scoped batch reads so Accounting can obtain final sale/reversal owner facts without choosing evidence by temporal proximity. The batch capability still exposes only the existing versioned public fact shapes and resolves Order/Store identity through Payments-owned checkout/provider correlation; it does not expose PaymentTransaction UUIDs or provider implementation objects. `PaymentFinancialFactsModule` continues to compose the existing `PAYMENT_TRANSACTION_REPOSITORY` through `PaymentsModule`; C1-A does not introduce a new Payments -> runtime-data direct dependency or compatibility allowance. This is a read-only consumer support change: it does not backfill historical PaymentTransaction data, initiate or alter a Clover charge/refund/void, change Platform canonicalization, change POS refund orchestration, or advance the protected production Web Ecommerce cutover.
