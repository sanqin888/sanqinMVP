...
**Phase 9 Slice 5D-C1-A Accounting shadow-consumer support (2026-09-14):** the two Payments financial-fact readers add stable Order-scoped batch reads so Accounting can obtain final sale/reversal owner facts without choosing evidence by temporal proximity. The batch capability still exposes only the existing versioned public fact shapes and resolves Order/Store identity through Payments-owned checkout/provider correlation; it does not expose PaymentTransaction UUIDs or provider implementation objects. `PaymentFinancialFactsModule` is also narrowed to its Prisma-backed reader composition instead of importing the full `PaymentsModule`, so an Accounting read consumer does not activate Clover provider/execution infrastructure merely to inspect facts. This is a read-only consumer support change: it does not backfill historical PaymentTransaction data, initiate or alter a Clover charge/refund/void, change Platform canonicalization, change POS refund orchestration, or advance the protected production Web Ecommerce cutover.
