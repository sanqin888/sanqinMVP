...
export type DraftNode = { id: string; type: DraftNodeType; name: string; sourceStableId?: string | null; source: 'SOURCE' | 'AUTO-MAPPED' | 'OVERRIDDEN'; status?: 'UNPUBLISHED' | 'ERROR'; priceCents?: number; priceDeltaCents?: number; isAvailable?: boolean; minSelect?: number; maxSelect?: number; children?: DraftNode[] };
export type UberDraftOptionNode = { id: string; sourceOptionChoiceStableId: string; displayName: string; priceDeltaCents: number; isAvailable: boolean; childGroups: Array<{ id: string; sourceTemplateGroupStableId: string; name: string; minSelect: number; maxSelect: number }> };
export type UberDraftGroupNode = { id: string; sourceTemplateGroupStableId: string; name: string; minSelect: number; maxSelect: number; options: UberDraftOptionNode[] };
