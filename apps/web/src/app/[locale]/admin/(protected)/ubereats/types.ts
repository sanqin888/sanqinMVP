...
export type UberMenuConfigImportPreview = { fingerprint: string; sourceStoreId: string; targetStoreId: string; mode: 'SKIP_EXISTING' | 'OVERWRITE'; counts: Record<'items' | 'options' | 'groups' | 'categories' | 'bindings', { create: number; update: number; unchanged: number; conflicts: number }>; conflicts: Array<{ kind: string; stableId: string; source: Record<string, unknown>; target: Record<string, unknown> }>; warnings: string[] };
