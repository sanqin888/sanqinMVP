...
function buildPaymentErrorMessage(locale: Locale, error: unknown) {
  ...
  return locale === "zh"
    ? "Apple Pay 支付失败，请返回结算页重试。"
    : "Apple Pay failed. Please go back and try again.";
}
...
export default function ApplePayWalletPage() {
  ...
  useEffect(() => {
    ...
    try {
      const raw = window.sessionStorage.getItem(APPLE_PAY_CTX_KEY);
      if (!raw) {
      ...
      if (parsed.paymentMethod && parsed.paymentMethod !== "APPLE_PAY") {
      ...
      console.error("[AP] ctx parse error", toSafeErrorLog(readError));
  ...
  useEffect(() => {
    ...
    const init = async () => {
      try {
        ...
        const applePay = clover.elements().create("PAYMENT_REQUEST_BUTTON_APPLE_PAY", {
          applePaymentRequest: appleReq,
        ...
        console.error("[AP] init error", toSafeErrorLog(initError));
    ...
    const onPaymentMethod = async (event: Event) => {
      ...
      const token =
        ...
        typeof (detail as { tokenRecieved?: { id?: unknown } }).tokenRecieved?.id === "string"
          ? (detail as { tokenRecieved: { id: string } }).tokenRecieved.id
          : undefined) ??
        (typeof detail === "object" &&
        detail &&
        "tokenReceived" in detail &&
        typeof (detail as { tokenReceived?: { id?: unknown } }).tokenReceived?.id === "string"
      ...
      try {
            ...
            body: JSON.stringify({
              ...
              cardholderName:
                typeof customer?.firstName === "string" || typeof customer?.lastName === "string"
                  ? `${typeof customer?.firstName === "string" ? customer.firstName : ""} ${typeof customer?.lastName === "string" ? customer.lastName : ""}`.trim() || "Apple Pay"
                  : "Apple Pay",
              customer: customer ?? {},
        ...
        window.sessionStorage.removeItem(APPLE_PAY_CTX_KEY);
        router.replace(`/${locale}/thank-you/${paymentResponse.orderStableId}`);
      } catch (payError) {
        console.error("[AP] pay error", toSafeErrorLog(payError));
    ...
    <main className="mx-auto flex min-h-screen w-full max-w-lg flex-col px-4 py-10">
      <div className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
        <h1 className="text-xl font-semibold text-slate-900">
          {locale === "zh" ? "Apple Pay 支付" : "Apple Pay"}
        </h1>
        {loading ? (
        ...
        <button
          type="button"
          className="mt-6 w-full rounded-full border border-slate-300 px-4 py-2 text-sm font-semibold text-slate-700 hover:bg-slate-50"
          onClick={() => {
