...
export function UberEatsConsole() {
  ...
  const [operationsStoreStableId, setOperationsStoreStableId] = useState('');
  const [ticketStatusFilter, setTicketStatusFilter] = useState<TicketStatus | ''>('');
  const connectionData = useUberConnectionStores(active === 'dashboard' || active === 'auth' || active === 'store-menu' || active === 'reconciliation-tickets');
  const { connectUrl, connection, stores } = connectionData.data;
  const operationStores = useMemo(
    () =>
      stores.filter(
        (store) => store.isProvisioned && Boolean(store.posExternalStoreId?.trim()),
      ),
    [stores],
  );
  useEffect(() => {
    if (
      !operationsStoreStableId ||
      !operationStores.some(
        (store) => store.posExternalStoreId === operationsStoreStableId,
      )
    ) {
      setOperationsStoreStableId(operationStores[0]?.posExternalStoreId ?? '');
    }
  }, [operationStores, operationsStoreStableId]);
  const ordersData = useUberOrders(active === 'orders-ops');
  const operationsData = useUberOperations(active === 'reconciliation-tickets', operationsStoreStableId, ticketStatusFilter);
  const reportsData = useUberReports(active === 'reconciliation-tickets', operationsStoreStableId);
  ...
  return <div className="flex gap-6">
    ...
    <main className="min-w-0 flex-1 space-y-4">
      ...
      {active === 'reconciliation-tickets' ? <section className="space-y-4"><div className="rounded-xl border border-slate-200 bg-white p-4"><label className="text-sm font-medium" htmlFor="ubereats-operations-store">SanQ Store</label><select id="ubereats-operations-store" className="mt-2 w-full rounded border px-2 py-1 text-sm" value={operationsStoreStableId} onChange={(event) => setOperationsStoreStableId(event.target.value)} disabled={operationStores.length === 0}><option value="">选择门店</option>{operationStores.map((store) => <option key={store.storeId} value={store.posExternalStoreId ?? ''}>{store.storeName ?? store.posExternalStoreId} ({store.posExternalStoreId})</option>)}</select></div><div className="grid gap-4 lg:grid-cols-2"><ReconciliationPanel storeStableId={operationsStoreStableId} reports={reportsData.data} resource={reportsData} retry={() => void reportsData.retry()} runAction={mutation.runAction} /><OperationsTicketsPanel tickets={operationsData.data} resource={operationsData} retry={() => void operationsData.retry()} statusFilter={ticketStatusFilter} onStatusFilterChange={setTicketStatusFilter} runAction={mutation.runAction} /></div></section> : null}
    </main>
  </div>;
}
