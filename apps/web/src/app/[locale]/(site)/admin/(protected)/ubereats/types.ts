...
export type DraftNode = { id: string; type: DraftNodeType; name: string; source: 'SOURCE' | 'AUTO-MAPPED' | 'OVERRIDDEN'; status?: 'UNPUBLISHED' | 'ERROR'; priceCents?: number; priceDeltaCents?: number; isAvailable?: boolean; minSelect?: number; maxSelect?: number; children?: DraftNode[] };
export type UberDraftOptionNode = { id: string; displayName: string; priceDeltaCents: number; isAvailable: boolean; childGroups: Array<{ id: string; name: string; minSelect: number; maxSelect: number }> };
export type UberDraftGroupNode = { id: string; name: string; minSelect: number; maxSelect: number; options: UberDraftOptionNode[] };
export type UberDraftItemNode = { id: string; displayName: string; displayDescription?: string | null; priceCents: number; isAvailable: boolean; imageUrl?: string | null; groups: UberDraftGroupNode[] };
