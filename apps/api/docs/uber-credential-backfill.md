...
2. 在包含旧明文字段的数据库上分批执行：
   `pnpm --filter api exec ts-node src/integrations/ubereats/infrastructure/scripts/backfill-uber-credentials.ts --batch-size=100`
3. 可重复执行。每行更新在事务内完成，写入后会立即解密并与内存中的原值比较；失败会回滚该批次。
4. 在切换应用版本前执行只读门禁：
   `pnpm --filter api exec ts-node src/integrations/ubereats/infrastructure/scripts/backfill-uber-credentials.ts --verify-only`
