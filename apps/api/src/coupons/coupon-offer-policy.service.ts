...
import type {
  CouponJsonObject,
  CouponOfferAdminPort,
  CouponOfferPolicyPort,
...
function normalizeStackingPolicy(
  value: CouponTemplateAdminInput['stackingPolicy'],
): 'EXCLUSIVE' | 'STACKABLE' {
...
function normalizeDistributionType(
  value: CouponProgramAdminInput['distributionType'],
): 'AUTOMATIC_TRIGGER' | 'MANUAL_CLAIM' | 'PROMO_CODE' | 'ADMIN_PUSH' {
  return value ?? 'AUTOMATIC_TRIGGER';
}

function normalizeTriggerType(
  distributionType: CouponProgramAdminInput['distributionType'],
  triggerType: CouponProgramAdminInput['triggerType'],
):
  | 'SIGNUP_COMPLETED'
  | 'REFERRAL_QUALIFIED'
  | 'MARKETING_OPT_IN'
...
@Injectable()
export class CouponOfferPolicyService
  implements CouponOfferPolicyPort, CouponOfferAdminPort
{
  constructor(private readonly prisma: PrismaService) {}

  validateUseRule(value: unknown): CouponJsonObject {
    return validateUseRule(value);
  }

  async parseAndValidateProgramItems(
    value: unknown,
  ): Promise<CouponProgramItem[]> {
    const items = parseProgramItems(value);
  ...
  async createTemplate(input: CouponTemplateAdminInput) {
    const useRule = this.validateUseRule(input.useRule);
    ...
    return this.prisma.couponTemplate.create({
      data: {
        ...
        validFrom,
        validTo,
        useRule: useRule as Prisma.InputJsonValue,
        issueRule,
      },
    });
  }
  ...
  async createProgram(input: CouponProgramAdminInput) {
    const items = await this.parseAndValidateProgramItems(input.items);
