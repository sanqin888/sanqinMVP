...
describe('AccountingProviderSettlementPreviewService', () => {
  ...
  it(
    'blocks pre-live Uber Order reversals until a READY authoritative statement covers the same date',
    async () => {
      const operations = {
        ...
        readProviderFinancialCoverage: jest.fn().mockResolvedValue([
          {
            ...
            storeStableId: '4750_Yonge_Street',
            financialHistoryRequiredFrom: new Date(
  ...
  it(
    'blocks a statement tip draft until the dedicated tip revenue account is provisioned',
    async () => {
      const operations = {
        readProviderSettlementDocuments: jest.fn().mockResolvedValue([
          {
