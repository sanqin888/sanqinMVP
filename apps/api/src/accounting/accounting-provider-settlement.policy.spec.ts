...
describe('Accounting provider settlement shadow policy', () => {
  it(
    'uses statement authority before live Uber Order cutover and maps tip as non-taxable store revenue',
    () => {
  ...
  it(
    'keeps Uber sales and sales tax reconciliation-only after live Order cutover while fees remain postable',
    () => {
      const authority = resolveProviderSalesAuthority({
        provider: AccountingFinancialProvider.UBER_EATS,
        documentType: AccountingFinancialDocumentType.STATEMENT,
  ...
  it(
    'blocks sales posting for a provider document that crosses a non-midnight live cutover',
    () => {
      const authority = resolveProviderSalesAuthority({
        provider: AccountingFinancialProvider.UBER_EATS,
        documentType: AccountingFinancialDocumentType.STATEMENT,
      ...
      expect(plan.blockReasons).toContain(
        'DOCUMENT_CROSSES_LIVE_ORDER_CUTOVER',
      );
    },
  );

  it(
    'fails closed on unknown provider components instead of defaulting them into general income or expense',
    () => {
      const plan = buildProviderSettlementDocumentPlan({
        document: uberDocument([
          {
  ...
  it(
    'inverts every line of a pre-cutover Uber Order journal without changing historical occurrence time',
    () => {
