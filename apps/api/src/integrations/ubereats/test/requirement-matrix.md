...
Accounting 财务历史采集范围，也不得用于合成 Uber-only 历史订单/商品事实。未获批/未配置时必须安全跳过，
不能把 Reporting scope 变成订单/门店链路的硬依赖，也不得把 provider Store UUID 暴露为 Accounting public contract。
