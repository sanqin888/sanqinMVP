...
describe('Uber operations application workflows', () => {
  ...
  it('generates reconciliation through semantic repositories', async () => {
    ...
    expect(tickets.countOpen).toHaveBeenCalledWith('store-stable-1');
  ...
  it('normalizes operation queries and caps report limit', async () => {
    ...
    expect(tickets.list).toHaveBeenCalledWith('store-stable-1', undefined);
  });

  it('retries a canonical availability ticket without re-reading Catalog', async () => {
    const tickets = {
  ...
  it('retries a canonical PAUSED store-status ticket through validated store mapping', async () => {
    const tickets = {
      find: jest.fn().mockResolvedValue({
  ...
  it('rejects legacy OFFLINE store-status ticket context', async () => {
    const tickets = {
      find: jest.fn().mockResolvedValue({
        ticketStableId: 'ticket-old-offline',
        storeStableId: 'store-stable-1',
        type: UberOpsTicketType.STORE_STATUS_SYNC,
        status: UberOpsTicketStatus.OPEN,
        priority: UberOpsTicketPriority.HIGH,
        title: 'Legacy store status sync',
        description: null,
        externalOrderId: null,
        menuItemStableId: null,
        context: {
      ...
      finishRetry: jest.fn().mockResolvedValue({
        ...
        status: UberOpsTicketStatus.OPEN,
        retryCount: 1,
        lastError: '门店状态工单的 targetStatus 非法',
        resolvedAt: null,
      }),
    };
    const unitOfWork = {
    ...
    const storeStatusSync = { syncStoreStatusToUber: jest.fn() };
    const mappings = { listMappings: jest.fn() };
    const retry = new RetryUberOpsTicketUseCase(
      unitOfWork as never,
      {} as never,
      {} as never,
    ...
    await expect(retry.execute('ticket-old-offline')).rejects.toThrow(
      '门店状态工单的 targetStatus 非法',
    );
    expect(storeStatusSync.syncStoreStatusToUber).not.toHaveBeenCalled();
    expect(mappings.listMappings).not.toHaveBeenCalled();
    expect(tickets.finishRetry).toHaveBeenCalledWith(
      'ticket-old-offline',
      '门店状态工单的 targetStatus 非法',
    );
  });
