...
describe('OrderLifecycleOutboxProcessor durable lifecycle replay', () => {
  ...
  it(
    'materializes prep_started exactly once through the existing AUTO print path',
    async () => {
      const queryRaw = jest
  ...
  it(
    'turns an accepted immediate order into prep_started before printing',
    async () => {
      const queryRaw = jest
        .fn<ReturnType<RawTag>, Parameters<RawTag>>()
  ...
  it(
    'never claims scheduled accepted orders in the immediate lifecycle stage',
    async () => {
      const queryRaw = jest
        .fn<ReturnType<RawTag>, Parameters<RawTag>>()
        .mockResolvedValueOnce([])
  ...
  it(
    'replays prep_started after a worker crash or transient fulfillment failure',
    async () => {
  ...
  it(
    'stops when every prep_started event already has its durable AUTO print materialization',
    async () => {
      const queryRaw = jest
        .fn<ReturnType<RawTag>, Parameters<RawTag>>()
        .mockResolvedValueOnce([])
