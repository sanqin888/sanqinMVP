...
describe('PosOrderReadService', () => {
  ...
  it('keeps internal order DB ids inside Orders while returning POS financial projections', async () => {
  ...
  it('skips amendment aggregation when the store has no orders in the window', async () => {
    const { service, orderFindMany, amendmentGroupBy } = setup();
    orderFindMany.mockResolvedValue([]);
  ...
  it('checks store-scoped access before resolving amendment persistence ids', async () => {
