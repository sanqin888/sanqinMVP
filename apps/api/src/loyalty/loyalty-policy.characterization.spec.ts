...
describe('Loyalty policy characterization', () => {
  ...
  it(
    'returns the default policy without creating legacy config on a missing row',
    async () => {
      const prisma = {
        brandConfig: {
  ...
  it(
    'preserves existing normalization rules for invalid loyalty policy values',
    () => {
      expect(
        normalizeLoyaltyPolicy({
          earnPtPerDollar: -1,
  ...
  it(
    'reads membership program rules from the Benefits policy snapshot backed by BrandConfig',
    async () => {
