...
- Admin menu CRUD/read-model decisions are owned by Catalog through
  ...
  Prisma access is exclusive to that Offers service. Catalog exposes only item stable
  IDs/base-price facts, while Admin composition, Public Menu and Orders consume the
  Offers public capability. The Catalog availability module reuses the narrow
  `CatalogAdminModule` so this HTTP-side Offers wiring does not expand the Uber worker
